<#
.SYNOPSIS
    Azure Infra IQ App Service deployment with Azure SQL and no AI provider.
    
.DESCRIPTION
    Separate variant of deploy-appservice-healthsector-qatarcentral.ps1 for FinOps
    customers who do not use AI. Deploys the unchanged application from the parent
    repository to Azure App Service and always provisions/reuses Azure SQL.

    Does not create, discover, connect to, or modify an Azure OpenAI resource, model,
    private endpoint, DNS zone, or AI role assignment. No AI parameters are accepted.
    Uses the application's existing AI_PROVIDER=none setting and clears AI endpoints
    and credentials in this Web App. Other deployment scripts retain their AI behavior.

    Use a dedicated Web App, SQL server, and database for this customer. Reusing a
    SQL server retains the source script's administrator-password reset behavior.
    The original script, backend, frontend, and startup code are not changed.
    This is configuration-only:
    AI controls may remain visible as unconfigured and can be changed in Settings.
    It is NOT an immutable AI policy or a FinOps-only UI. Existing Advisor and
    deterministic savings recommendations remain available without a model.
    Do not reuse a database or Redis cache containing an AI deployment's saved output.
    
    DEPLOYMENT MODES:
    ─────────────────
    PUBLIC MODE (Default):
      - App Service accessible from internet
        - Azure SQL reachable through its firewall rules
      - Simplest deployment option
      
    PRIVATE MODE (-DeploymentMode Private):
      - App Service behind Private Endpoint (no public access)
        - Azure SQL behind Private Endpoint (no public access)
      - Requires existing VNet with appropriate subnets
        - Outbound access to Entra ID and Azure management APIs is still required
    
    WHAT THIS SCRIPT DOES:
    ──────────────────────
    1. Creates or reuses Azure SQL for FinOps costs and application persistence
    2. Creates App Service Plan (Linux, Python 3.11)
    3. Creates Web App with System-Assigned Managed Identity
    4. Configures no-AI settings, Entra sign-in, and SQL persistence
    5. Deploys application code with pip package installation
    6. [Private Mode] Creates Private Endpoints for SQL and Web App
    7. [Private Mode] Configures Private DNS Zones
    8. Retains the original non-AI RBAC and Microsoft Graph permissions
...
.PARAMETER ResourceGroupName
    ...
    Default: rg-azure-cost-optimizer
...
.PARAMETER AppServicePlanName
    ...
    Default: asp-azure-cost-optimizer
    
.PARAMETER WebAppName
    Name for the Web App (must be globally unique).
    Default: app-azure-cost-optimizer

.PARAMETER SqlDatabaseName
    Azure SQL database used by the application, including its FinOps warehouse.
    Default: prompts (kept compatible with the source script).

.PARAMETER SqlServiceObjective
    SQL sizing is unchanged from the source: GP_Gen5_4, 4 GB maximum size by default.
    Review sizing for the customer's retention and resource count before deployment.
...
.EXAMPLE
    ...
    .\Az-Infra-IQ-Deployment-NoAI.ps1 `
...
.NOTES
    ...
    - Managed Identity for Azure API reads; SQL login authentication as in the source
    - Private networking option for enterprise security
    - Reuses named resources; review settings and SQL login changes before rerunning
#>
...
Write-Host "║     Azure Infra IQ - No-AI App Service Deployment                 ║" -ForegroundColor Cyan
Write-Host "║                                                                  ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "    2. Create / reuse Azure SQL for the FinOps warehouse and app data" -ForegroundColor Gray
...
Write-Host "    Azure SQL server + database (no AI service)" -ForegroundColor Gray
Write-Host "    • App Service Plan (Linux) + Web App with managed identity" -ForegroundColor Gray
Write-Host "    Role assignments (Reader, Cost Management Reader, etc.; no AI roles)" -ForegroundColor Gray
...
Write-Host "    SQL server/database settings (no AI inputs)" -ForegroundColor Gray
...
Write-Host "    Private - Web App + Azure SQL behind Private Endpoints; the" -ForegroundColor Gray
...
Write-Host ""
Write-Host "  Supply deployment values as parameters; subscription confirmation remains interactive." -ForegroundColor DarkGray
Write-Host "  Use a separate Web App and database. AI UI controls are not removed or policy-locked." -ForegroundColor Yellow
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey('DeploymentMode')) {
    ...
    Write-Host "    [2] Private - App Service + Azure SQL behind Private Endpoints (requires existing VNet)" -ForegroundColor Gray
...
if ($DeploymentMode -eq "Private") {
    ...
    if ([string]::IsNullOrWhiteSpace($AppServiceIntegrationSubnetName)) {
        Write-Host "  (VNet integration lets the Web App reach PRIVATE Azure SQL - needs a DEDICATED subnet," -ForegroundColor Gray
    ...
    if (-not $PSBoundParameters.ContainsKey('PrivateDnsZoneSubscriptionId') -and -not $PSBoundParameters.ContainsKey('PrivateDnsZoneResourceGroupName')) {
        Write-Host ""
        Write-Host "  Private DNS Zones needed: privatelink.database.windows.net, privatelink.azurewebsites.net" -ForegroundColor Gray
...
# ============================================
# AZURE SQL CONFIGURATION (FinOps warehouse and application persistence)
# ============================================
if ($DeploySql) {
...
# ============================================
# CREATE AZURE SQL (FinOps warehouse and application persistence)
# ============================================
if ($DeploySql) {
...
if ($LASTEXITCODE -eq 0) {
    ...
    # safety floor. Grab the highest tier with headroom, then
...
$settings = @(
    ...
    "SETTINGS_DIR=/home/site/wwwroot/config-noai",
    "WEBSITES_PORT=8000",
    "SCM_DO_BUILD_DURING_DEPLOYMENT=true",
    "WEBSITE_PYTHON_VERSION=3.11"
)

# FinOps warehouse / application persistence -> Azure SQL (pyodbc, SQL auth).
if ($DeploySql) {
    $settings += @(
...
# (needed by pyodbc for the SQL login connection) before launching
...
# Required so the app's outbound SQL connections can reach the private endpoint.
# ============================================
if ($DeploymentMode -eq "Private") {
...
Write-Host ""
Write-Host "  Role: Management Group Reader" -ForegroundColor CyanWrite-Host "    Scope: Tenant Root Management Group" -ForegroundColor White
...
Write-Host "  AI Provider:           none (no AI service provisioned or connected)" -ForegroundColor White
...
if ($DeploymentMode -eq "Private") {
    ...
    Write-Host "  Azure SQL PE:          ${SqlServerName}-pe" -ForegroundColor White
    Write-Host "  Web App PE:            ${WebAppName}-pe" -ForegroundColor White
    Write-Host "" -ForegroundColor White
    Write-Host "  🌐 PRIVATE DNS ZONES" -ForegroundColor Magenta
    Write-Host "  privatelink.database.windows.net -> linked to $VNetName" -ForegroundColor White
...
Write-Host "  Azure API Authentication: Managed Identity (no AI credentials)" -ForegroundColor White
Write-Host "  SQL Authentication:       SQL login stored in Web App settings" -ForegroundColor White
Write-Host ""
...
Write-Host "  AI provider set to none; no model deployment or AI access grant" -ForegroundColor Green
...
if ($DeploymentMode -eq "Private") {
    ...
    Write-Host "  Configured Private Endpoint for Azure SQL" -ForegroundColor Green
...
Write-Host "─────────────────────────────────────────────────────────────────" -ForegroundColor Gray
Write-Host "  AI generation is unconfigured; AI controls may remain visible in the unchanged portal." -ForegroundColor DarkGray
Write-Host "  Settings can reconfigure AI. An immutable no-AI policy needs application changes." -ForegroundColor DarkGray
Write-Host "  FinOps ingestion is unchanged. Resource deployment is not proof of cost-data readiness." -ForegroundColor DarkGray
