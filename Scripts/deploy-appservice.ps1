...
.PARAMETER AppServiceSku
    App Service Plan SKU. Default: "Auto" - the script deploys the most capable Premium plan
    the subscription/region can support, trying this preferred order and falling back ONLY on
    quota/capacity/region limits:
        P4mv3 -> P4mv4 -> P3mv4 -> P3mv3 -> P3v4 -> P3v3   (with a P1v3 safety floor)
    The create call is the authoritative quota/capacity check; a region pre-filter skips SKUs not
    offered in the target region. All these SKUs are >=32 GB RAM, so the Oryx build cannot OOM
    (unlike the 1.75 GB S1/B-series). Pass an explicit SKU (e.g. -AppServiceSku P3v3) to pin one;
    it is tried first, then the premium list as fallback.
...
param(
    ...
    [string]$SqlBackupStorageRedundancy = "Local",
    ...
    # Optional Redis L2 cache (Azure Cache for Redis). Provisioned by default.
    [Parameter(Mandatory = $false)]
...
if ($modelExists) {
    ...
    Write-Info "Strategy: newest model first (GPT-5.5 > GPT-5.x > GPT-5 > GPT-4.1 > GPT-4o) + SKU (GlobalStandard > DataZoneStandard > Standard); REAL TPM quota pre-check picks the HIGHEST capacity available, then falls back"
    ...
    # Robust ladder (same logic style as deploy-automated.ps1):
    #   - Newest GA model + version first so we always get the latest model benefits
    #     (GPT-5.5 is the current top GA chat model as of 2026)
    #   - GlobalStandard (best latency) -> DataZoneStandard -> Standard
    #   - Falls back GPT-5.x -> GPT-5 -> GPT-4.1 -> GPT-4o -> gpt-4o-mini
    #   - Each combination is retried across decreasing capacity levels (quota discovery)
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($AppServiceSku -and $AppServiceSku -ne 'Auto') {
        # Operator pinned a SKU: honor it first, then fall back through the premium list.
        $desiredLadder = @($AppServiceSku) + $premiumPreference
    } else {
        $desiredLadder = $premiumPreference
    }
    # Safety floor: P1v3 (8 GB) guarantees the deploy never fails if every premium SKU is blocked.
    # NEVER add S1/B-series here (they OOM during the build).
    $desiredLadder = @($desiredLadder + 'P1v3') |
