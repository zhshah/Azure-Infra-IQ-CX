<#
.SYNOPSIS
    Automated Azure App Service deployment for Infra IQ — FinOps only, NO AI
    
.DESCRIPTION
    Deploys Infra IQ to Azure App Service for customers who want the FinOps and
    dashboard capability WITHOUT any Azure OpenAI dependency.

    This is a standalone variant of deploy-appservice-healthsector-qatarcentral.ps1.
    That script remains the full-featured deployment and is NOT affected by anything
    here. Keep the two in step when the shared steps change.

    WHAT IS DIFFERENT FROM THE FULL SCRIPT
    ──────────────────────────────────────
      - No Azure OpenAI account is created, and no existing one is used.
      - No OpenAI model deployment, and no OpenAI Private Endpoint / DNS zone.
      - No "Cognitive Services OpenAI User" role assignment.
      - The app is configured with AI_PROVIDER=none, so every AI code path is
        skipped at runtime rather than failing. The app logs
        "AI provider set to 'none' — skipping AI scoring" and carries on.
      - No -OpenAI* parameters are accepted or prompted for.

    EVERYTHING ELSE IS IDENTICAL: Cost Management exports to ADLS Gen2, the Azure SQL
    warehouse, the estate scan, dashboards, RBAC, VNet integration, Private Endpoints
    for the Web App / SQL / cost-export storage, and Microsoft Graph permissions.

    WHAT STILL WORKS WITHOUT AI
    ───────────────────────────
      FinOps overview and summary, cost allocation and chargeback, budgets, the cost
      warehouse and its ETL, the estate scan, resource inventory and scores, waste and
      orphan detection, Azure Advisor recommendations, utilisation and right-sizing,
      reservations, Log Analytics per-table cost, exports and reports.

    WHAT DOES NOT WORK WITHOUT AI
    ─────────────────────────────
      These are NOT available: the AI summary on the home page, AI resource verdicts,
      AI deep-dive analysis, the executive briefing, and every other narrative that
      would otherwise be written by Azure OpenAI. Those views render empty; they do
      not error, and nothing else on the page is affected.
    ...
    ──────────────────────
    1. Creates App Service Plan (Linux, Python 3.11)
    2. Creates Web App with System-Assigned Managed Identity
    3. Creates Azure SQL + the FinOps cost-export storage account
    4. Configures environment variables (AI disabled)
    5. Deploys application code with pip package installation
    6. [Private Mode] Creates Private Endpoints for the Web App, SQL and cost storage
    7. [Private Mode] Configures Private DNS Zones
    8. Assigns RBAC roles (Reader, Cost Management Reader/Contributor, etc.)
...
param(
    ...
    # NOTE: every -OpenAI* parameter from the full deployment script is intentionally
    # absent here. This variant never creates or contacts an Azure OpenAI resource, and
    # accepting the parameters would imply otherwise. See $AIEnabled below.
...
# ── AI is OFF for this deployment variant ────────────────────────────────────
# This is the single switch every AI-related block below is gated on, so the shared
# steps stay line-for-line comparable with the full deployment script. Do not make it
# a parameter: a customer who needs AI should run the full script instead, which
# creates the OpenAI resource, its Private Endpoint and the data-plane role assignment.
# At runtime the app reads AI_PROVIDER; "none" makes every AI path log
# "AI provider set to 'none' — skipping AI scoring" and return empty.
$AIEnabled = $false
...
# WELCOME / OVERVIEW
# ============================================
Write-Host "  Welcome! This script deploys Infra IQ to Azure using the APP SERVICE" -ForegroundColor White
Write-Host "  hosting model (a managed Linux Web App) — FinOps only, WITHOUT AI." -ForegroundColor White
Write-Host ""
Write-Host "  ⚠  AI ANALYSIS IS DISABLED IN THIS DEPLOYMENT" -ForegroundColor Yellow
Write-Host "     No Azure OpenAI resource is created or used. Cost, FinOps, estate scan," -ForegroundColor Gray
Write-Host "     inventory, waste/orphan detection, Advisor, utilisation, budgets, exports" -ForegroundColor Gray
Write-Host "     and reports all work normally. Only the AI-written narratives (home-page" -ForegroundColor Gray
Write-Host "     summary, resource verdicts, deep dive, executive briefing) are switched off." -ForegroundColor Gray
Write-Host "     Need AI? Use deploy-appservice-healthsector-qatarcentral.ps1 instead." -ForegroundColor Gray
Write-Host ""
...
Write-Host "    2. Create Azure SQL + the FinOps cost-export storage account" -ForegroundColor Gray
...
Write-Host "    • Azure SQL logical server + database (the cost warehouse)" -ForegroundColor Gray
Write-Host "    • Storage account + container for the Cost Management daily exports" -ForegroundColor Gray
Write-Host "    • Role assignments (Reader, Cost Management Reader/Contributor, etc.)" -ForegroundColor Gray
...
Write-Host "    • NO Azure OpenAI account and NO model deployment" -ForegroundColor DarkGray
Write-Host ""
...
Write-Host "    • Storage account name for the FinOps cost exports" -ForegroundColor Gray
...
Write-Host "    🔒 Private - Web App + Azure SQL locked behind Private Endpoints; the" -ForegroundColor Gray
...
if (-not $PSBoundParameters.ContainsKey('DeploymentMode')) {
    ...
    Write-Host "    [2] Private - App Service behind a Private Endpoint (requires existing VNet)" -ForegroundColor Gray
...
# 3a) Azure OpenAI — NOT PROMPTED IN THIS VARIANT.
# The full deployment script asks here whether to create a new OpenAI resource or reuse an
# existing PTU/Provisioned one, then asks for its region, resource name and model deployment
# name. None of that applies: no OpenAI resource is created or contacted, so there is nothing
# to choose. The app is configured with AI_PROVIDER=none in Step 7.
Write-Success "AI analysis: DISABLED (no Azure OpenAI resource will be created or used)"
...
# 4) and 5) Azure OpenAI resource name + model deployment name — NOT PROMPTED IN THIS VARIANT.
# The full script checks global subdomain availability for a new Cognitive Services account and
# asks for the model alias. Neither is created here.
...
if ($DeploymentMode -eq "Private") {
    ...
    if ([string]::IsNullOrWhiteSpace($AppServiceIntegrationSubnetName)) {
        Write-Host "  (VNet integration lets the Web App reach the PRIVATE Azure SQL and storage endpoints - needs a DEDICATED subnet," -ForegroundColor Gray
    ...
    if (-not $PSBoundParameters.ContainsKey('PrivateDnsZoneSubscriptionId') -and -not $PSBoundParameters.ContainsKey('PrivateDnsZoneResourceGroupName')) {
        Write-Host ""
        Write-Host "  Private DNS Zones needed: privatelink.azurewebsites.net, privatelink.database.windows.net" -ForegroundColor Gray
...
if ($DeploymentMode -eq "Private") {
    ...
    # NOTE: the full deployment script also looks up privatelink.openai.azure.com here.
    # This variant never creates an OpenAI Private Endpoint, so that zone is not needed.
...
# ============================================
# AZURE OPENAI - INTENTIONALLY NOT DEPLOYED
# ============================================
# The full deployment script runs three steps here:
#   Step 3  - create a new Azure OpenAI (Cognitive Services) account, or resolve an
#             existing customer-provided one, and read its endpoint + key.
#   Step 4  - create the model deployment (newest GPT with graceful fallback) and
#             derive the AI throughput tuning values.
#   Step 4b - create the OpenAI Private Endpoint and its privatelink.openai.azure.com
#             DNS zone + zone group, then lock the account down.
#
# None of that happens in this variant ($AIEnabled = $false). No Cognitive Services
# account is created, none is contacted, and no OpenAI Private Endpoint or DNS zone is
# made. Step 7 sets AI_PROVIDER=none so the application skips its AI paths at runtime
# instead of failing, and Step 9 omits the "Cognitive Services OpenAI User" role.
#
# Step numbering below is deliberately UNCHANGED (the next step is still 4c) so this
# script stays easy to diff against the full one.
Write-Step "Steps 3, 4, 4b: Azure OpenAI - SKIPPED (AI disabled for this deployment)"
Write-Info "No Azure OpenAI resource, model deployment or OpenAI Private Endpoint will be created."
Write-Info "The app will run with AI_PROVIDER=none - all FinOps and estate features work; AI narratives are off."
Write-Host ""
# ============================================
...
#
# AI IS DISABLED HERE. AI_PROVIDER=none is the app's own default and is what
# services/ai_service.py checks first — it logs "AI provider set to 'none' — skipping
# AI scoring" and returns an empty result rather than raising, so every AI-backed view
# renders empty and nothing else is affected. The AZURE_OPENAI_* settings are omitted
# ENTIRELY rather than set to empty strings: an absent setting cannot be mistaken for a
# misconfigured one when someone later reads the app's configuration.
$settings = @(
    "AI_PROVIDER=none",
...
$miRbacRef = @(
    ...
    # NOTE: the full deployment script also grants "Cognitive Services OpenAI User" on the
    # OpenAI resource here. AI is disabled in this variant, so that grant is omitted — the
    # managed identity is given NO Cognitive Services access at all.
...
# ── Azure OpenAI access — NOT GRANTED IN THIS VARIANT ──
# The full deployment script grants the Web App's managed identity "Cognitive Services
# OpenAI User" on the OpenAI resource here. AI is disabled ($AIEnabled = $false), there is
# no OpenAI resource, and the identity is deliberately given NO Cognitive Services access.
Write-Host ""
Write-Info "Skipping 'Cognitive Services OpenAI User' — AI is disabled for this deployment."
...
Write-Host "  AI Analysis:           DISABLED (AI_PROVIDER=none, no Azure OpenAI resource)" -ForegroundColor Yellow
...
Write-Host "  ℹ  AI analysis NOT deployed (no Azure OpenAI resource, AI_PROVIDER=none)" -ForegroundColor Gray
