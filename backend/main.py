...
# ── User-directed AI scope / context / resource filter ────────────────────────
# Applied centrally so EVERY /api/ai/* module analysis honours the same controls
# without each endpoint re-implementing them.
import contextvars as _contextvars

_AI_RESOURCE_FILTER: "_contextvars.ContextVar" = _contextvars.ContextVar("ai_resource_filter", default=None)


def _apply_ai_resource_filter(rows: List[dict]) -> List[dict]:
    """Narrow a resource list to the caller's AI scope (no-op outside /api/ai/*)."""
    f = _AI_RESOURCE_FILTER.get()
    if not f or not rows:
        return rows
    sub, rg, rtype = f.get("sub"), f.get("rg"), f.get("rtype")
    res = f.get("res")
    out = rows
    if sub:
        out = [r for r in out if str(r.get("subscription_id", "")).lower() == sub]
    if rg:
        out = [r for r in out if str(r.get("resource_group", "")).lower() == rg]
    if rtype:
        out = [r for r in out if str(r.get("resource_type", "")).lower() == rtype]
    if res:
        out = [r for r in out
               if str(r.get("resource_id", "")).lower() == res
               or str(r.get("id", "")).lower() == res
               or str(r.get("resource_name", "")).lower() == res
               or str(r.get("name", "")).lower() == res]
    # An over-narrow filter that matches nothing would make the AI analyse an empty
    # estate and invent findings; fall back to the full set instead.
    return out or rows


@app.middleware("http")
async def _ai_scope_gate(request: Request, call_next):
    """Read scope/context/filter query params for AI routes and publish them.

    `sub`/`rg`/`rtype`/`res` narrow the resources every analysis sees; `scope` and
    `context` are injected into the prompt by ai_module_analysis_service.
    """
    if not request.url.path.startswith("/api/ai/"):
        return await call_next(request)
    q = request.query_params
    sub, rg, rtype = q.get("sub"), q.get("rg"), q.get("rtype")
    res, res_name = q.get("res"), q.get("res_name")
    scope, ctx_raw = q.get("scope"), q.get("context")
    if not any([sub, rg, rtype, res, scope, ctx_raw]):
        return await call_next(request)

    labels = []
    if sub:
        labels.append(f"Subscription: {_sub_name_map().get(sub, sub)}")
    if rg:
        labels.append(f"Resource group: {rg}")
    if rtype:
        labels.append(f"Resource type: {rtype}")
    if res:
        labels.append(f"Single resource: {res_name or res}")
    _AI_RESOURCE_FILTER.set({
        "sub": sub.lower() if sub else None,
        "rg": rg.lower() if rg else None,
        "rtype": rtype.lower() if rtype else None,
        "res": res.lower() if res else None,
    })
    ctx = None
    if ctx_raw:
        try:
            ...
            ctx = None
    try:
        from services import ai_module_analysis_service as _aims
        _aims.set_user_directive(scope=scope or "", context=ctx,
                                 filter_label="; ".join(labels))
    except Exception as exc:
        logger.debug("AI scope gate: directive not set (%s)", exc)
    return await call_next(request)
...
def _resources_first_payload(resources: List[Dict], sub_ids: List[str],
    ...
    for r in resources:
        ...
        tags  = r.get("tags") or {}
        missing = _check_tag_compliance(tags)
        # "Untagged" = carrying NONE of the mandatory tag keys (case/hyphen-insensitive via
        # _check_tag_compliance). A resource missing only SOME mandatory tags is tagged-but-
        # non-compliant, not untagged. (Single definition used everywhere — fixes the
        # Overview-vs-AI 310-vs-183 contradiction.)
...
async def _build_dashboard(
    ...
    savings_recs = sorted(
        [SavingsRecommendation(
            ...
            resource_type=r.resource_type, resource_group=r.resource_group,
            subscription_id=getattr(r, "subscription_id", "") or "",
...
async def get_settings_endpoint():
    ...
    return AppSettings(
        ...
        azure_openai_deployment  = s.get("AZURE_OPENAI_DEPLOYMENT","gpt-5.6-sol"),
        has_azure_openai_key     = bool(settings_svc.get_value("AZURE_OPENAI_KEY","")),
        ai_cache_ttl_hours       = s.get("ai_cache_ttl_hours", 12),
        AI_REASONING_EFFORT      = settings_svc.get_reasoning_effort(),
...
async def test_ai(body: dict):
    ...
    if provider == "claude":
        ...
        deployment = body.get("AZURE_OPENAI_DEPLOYMENT") or settings_svc.get_value("AZURE_OPENAI_DEPLOYMENT", "gpt-5.6-sol")
...
async def _finops_warehouse_scheduler() -> None:
    ...
    try:
        ...
        elif _FINOPS_WAREHOUSE_AVAILABLE:
            # Cost rows can already exist while budgets, Sentinel/LA table costs and the
            # utilisation history are still empty — those only fill from an ETL pass, so a
    ...
    while True:
        ...
        try:
            # Every other tick (~daily) reaches all the way back over the full window
            # instead of only topping up recent days. Without this a deployment that
            # started on a short window never deepens its history, so year-on-year and
            # long-range trends stay permanently empty.
...
try:
    ...
    import services.finops_advanced_service as finops_adv_svc
...
@app.get("/api/metrics/summary", tags=["FinOps"])
async def metrics_summary(scope: Optional[str] = None, period: Optional[str] = None):
    """
    THE canonical metrics object. Every KPI card, dashboard tile, Cost Studio
    strip, and AI narrative must read from here so no number is ever computed two
    different ways. Returns spend / forecast / resources / reservations / budgets /
    savings / anomalies with a single definition per metric, one forecast model
    (linear-mtd), and explicit dataThroughDate + currency.

    `scope`  — optional comma-separated subscription IDs (defaults to all scanned).
    `period` — reserved for future {start,end}; MTD is the default window.
    """
    _require_finops()
    from services import finops_metrics_service as metrics_svc

    sub_ids: Optional[List[str]] = None
    if scope:
        sub_ids = [s.strip() for s in scope.split(",") if s.strip()]

    period_obj: Optional[Dict[str, str]] = None
    if period:
        try:
            import json as _json
            period_obj = _json.loads(period)
        except Exception:
            period_obj = None
    ...
    return await loop.run_in_executor(
        _pool,
        lambda: metrics_svc.get_metrics_summary(subscription_ids=sub_ids, period=period_obj),
    )
...
async def list_subscriptions():
    ...
    for sid in scanned:
        if sid not in all_ids:
            all_ids.append(sid)
    # FLEXIBLE + RESILIENT scope list: surface every subscription the identity can currently
    # access (so a newly-granted or re-enabled subscription appears automatically) and DROP any
    # that are disabled / unknown-state, so a disabled subscription never lingers in the scope
    # picker (it would otherwise error per-sub calls and stall dashboards). Fail-open: if the
    # enabled set can't be resolved, leave the list untouched so we never show nothing.
    try:
        enabled_names = _resolve_all_subscription_names()  # accessible + Enabled only
        if enabled_names:
            for sid in enabled_names:
                if sid not in all_ids:
                    all_ids.append(sid)
            filtered = [sid for sid in all_ids if sid in enabled_names]
            all_ids = filtered or all_ids
    except Exception:
        pass
    if not all_ids:
        return [
...
def _dashboard_cost_explorer(query):
    """Dashboard-cache fallback for Cost Explorer so the tab still returns a real
    breakdown (by the primary group-by dimension) when the live Cost Management query
    is throttled/times out. Total granularity only (cache has month totals, not a daily
    series), which is a graceful degrade vs a spinner/timeout."""
    from models.schemas import FinOpsCostExplorerResult, FinOpsCostDataPoint
    from collections import defaultdict as _dd
    dash = _cache.get("data:*") or _cache.get("data")
    resources = list(getattr(dash, "resources", None) or []) if dash else []
    if not resources:
        return None
    gb = list(getattr(query, "group_by", None) or [])
    dim = gb[0] if gb else "ServiceName"
    dim_l = dim.lower()
    tag_key = dim.split(":", 1)[1] if dim.startswith("TagKey:") else None
    field_map = {
        "subscriptionid": "subscription_id", "resourcegroupname": "resource_group",
        "resourcegroup": "resource_group", "resourcetype": "resource_type",
        "resourcelocation": "location", "location": "location",
    }
    field = field_map.get(dim_l)
    agg = _dd(float)
    for r in resources:
        if tag_key:
            tags = getattr(r, "tags", None) or {}
            v = tags.get(tag_key)
            if v is None:
                v = next((vv for k, vv in tags.items() if str(k).lower() == tag_key.lower()), "")
            key = str(v or "") or "(untagged)"
        elif field:
            key = str(getattr(r, field, "") or "") or "(none)"
        else:
            key = str(getattr(r, "resource_type", "") or "") or "(none)"
        agg[key] += float(getattr(r, "cost_current_month", 0) or 0)
    if not agg:
        return None
    subs = {s.subscription_id: (s.subscription_name or s.subscription_id)
            for s in (getattr(dash, "subscriptions", None) or [])}
    total = round(sum(agg.values()), 2)
    points, contributors = [], []
    for k, c in sorted(agg.items(), key=lambda x: -x[1]):
        label = subs.get(k, k) if dim_l == "subscriptionid" else k
        points.append(FinOpsCostDataPoint(date=None, label=label, cost_usd=round(c, 2), breakdown={label: round(c, 2)}))
        contributors.append({"label": label, "cost": round(c, 2), "pct": round(c / total * 100, 1) if total else 0.0})
    return FinOpsCostExplorerResult(
        data_points=points, total_usd=total, top_contributors=contributors[:20],
        dimensions_used=[dim], cost_type=getattr(query, "cost_type", "ActualCost"),
        date_from="", date_to="", granularity="None", currency="USD", data_source="dashboard_cache",
    )
...
async def finops_cost_explorer(query: FinOpsCostExplorerQuery):
    ...
    actual & amortized cost, and tag-based grouping. Falls back to the dashboard resource
    cache (throttle-immune) when the live Cost Management query is slow/throttled.
    """
    _require_finops()
    loop = asyncio.get_event_loop()
    try:
        ...
        logger.warning("finops_cost_explorer live failed/slow: %s", exc)
        fb = await loop.run_in_executor(_pool, lambda: _dashboard_cost_explorer(query))
        if fb is not None and fb.data_points:
            return fb
        raise HTTPException(status_code=503, detail="Cost explorer temporarily unavailable — retry shortly")
...
def _dashboard_allocation(dimension: str) -> Optional[FinOpsAllocationReport]:
    """Cost-allocation from the dashboard resource cache (per-resource
    cost_current_month / cost_previous_month) — instant and immune to Cost Management
    throttling. Fallback so the Cost Allocation tab never 503s when the warehouse is
    empty and the live query is throttled. Resource-attribute dimensions (subscription /
    resource group / resource type / region / tag) are exact; billing-only dimensions
    (ServiceName / ServiceFamily / MeterCategory) approximate with resource type."""
    from collections import defaultdict as _dd
    ...
    tag_key = dimension.split(":", 1)[1] if dimension.startswith("TagKey:") else None
    field_map = {
        "subscriptionid": "subscription_id", "resourcegroupname": "resource_group",
        "resourcegroup": "resource_group", "resourcetype": "resource_type",
        "resourcelocation": "location", "location": "location",
    ...
    field = field_map.get(dim_l)
    cur, prev, cnt = _dd(float), _dd(float), _dd(int)
    for r in resources:
        if tag_key:
            tags = getattr(r, "tags", None) or {}
            val = tags.get(tag_key)
            if val is None:
                val = next((v for k, v in tags.items() if str(k).lower() == tag_key.lower()), "")
            key = str(val or "") or "(blank)"
        elif field:
            key = str(getattr(r, field, "") or "") or "(blank)"
        else:
            key = str(getattr(r, "resource_type", "") or "") or "(blank)"
        cur[key] += float(getattr(r, "cost_current_month", 0) or 0)
        prev[key] += float(getattr(r, "cost_previous_month", 0) or 0)
        cnt[key] += 1
    total = round(sum(cur.values()), 2)
    unalloc = round(cur.pop("(blank)", 0.0), 2)
    prev.pop("(blank)", None)
    cnt.pop("(blank)", None)
    items = []
    for k, c in sorted(cur.items(), key=lambda x: -x[1]):
        p = prev.get(k, 0.0)
        mom = ((c - p) / p * 100.0) if p > 0 else 0.0
        items.append(FinOpsAllocationItem(
            dimension_value=k, cost_usd=round(c, 2),
            cost_pct=round(c / total * 100.0, 1) if total else 0.0,
            resource_count=cnt.get(k, 0), mom_delta_pct=round(mom, 1),
        ))
    if not items:
        return None
    dim_labels = {
        "SubscriptionId": "Subscription", "ResourceGroupName": "Resource Group",
        "ResourceType": "Resource Type", "ServiceFamily": "Service Family",
        "ServiceName": "Service Name", "ResourceLocation": "Region",
    }
    return FinOpsAllocationReport(
        dimension=dimension,
        dimension_label=dim_labels.get(dimension, dimension.replace("TagKey:", "Tag: ")),
        items=items, total_usd=total, unallocated_usd=unalloc,
        unallocated_pct=round(unalloc / (total + unalloc) * 100.0, 1) if (total + unalloc) else 0.0,
        period_label="current month (from resource cache)", date_from="", date_to="",
        data_source="dashboard_cache",
    )
...
def _warehouse_tag_analytics(required_tags: Optional[List[str]] = None) -> Optional[FinOpsTagAnalyticsResult]:
    ...
    try:
        ...
        worst_covered = total_res       # resources carrying EVERY required tag (approx)
        ...
        for tag_key in req:
            ...
            worst_covered = min(worst_covered, covered)
            if not covered:
        ...
        return FinOpsTagAnalyticsResult(
            ...
            compliant_resources=max(0, min(int(worst_covered), int(total_res))),
            non_compliant_resources=max(int(total_res) - max(0, int(worst_covered)), 0),
            untagged_spend_usd=round(untagged_cost, 2),
            # EVERY discovered key, not just the required ones, so the coverage table
            # matches the tag chips above it instead of rendering empty.
            tag_stats=[
                FinOpsTagKeyStats(
                    tag_key=rstats["casing"].get(lk, lk),
                    covered_resources=cov_n,
                    total_resources=int(total_res),
                    coverage_pct=round(cov_n / total_res * 100, 1) if total_res else 0.0,
                    total_cost_usd=round(rstats["cost_by_key"].get(lk, 0.0), 2),
                    distinct_values=len(rstats["values"].get(lk, set())),
                    top_values=[],
                    is_required=lk in {str(t).lower() for t in req},
                )
                for lk, cov_n in sorted(rstats["covered"].items(), key=lambda kv: -int(kv[1] or 0))
            ],
...
async def finops_allocation(
    ...
    dash_alloc = await loop.run_in_executor(_pool, lambda: _dashboard_allocation(dimension))
    if dash_alloc is not None and dash_alloc.items:
        return _apply_sub_names(dash_alloc, dimension)
    # 3. Bounded live Cost Management query; fall back to cache, else 503.
    try:
...
async def finops_top_movers(
    ...
    try:
        ...
        logger.warning("finops_top_movers live failed/slow: %s", exc)
        raise HTTPException(status_code=503, detail="Top movers temporarily unavailable — retry shortly")
...
# A deep reasoning pass (AI_REASONING_EFFORT=high) measures 70-300s on gpt-5.6-sol.
# The previous 60s cap 503'd every uncached generation, so Refresh / Scope / Context /
# Ask never returned anything.
_FINOPS_AI_TIMEOUT_SECS = 300


def _finops_ai_grounding(filters: Optional[dict]) -> Optional[dict]:
    """Build authoritative, resource-level grounding facts for the FinOps AI from the
    dashboard cache, best-effort scoped by the caller's filters (subscription / RG /
    region / type / tags). Every AI panel is grounded on this so recommendations cite
    real resources + dollars instead of generic advice. Instant + throttling-immune."""
    ...
    def _aslist(*keys):
        for k in keys:
            v = f.get(k)
            if v:
                return [str(x).lower() for x in v if x] if isinstance(v, list) else [str(v).lower()]
        return []

    sub_f, rg_f = _aslist("subscription_id", "subscriptions"), _aslist("resource_group", "resource_groups")
    reg_f, type_f = _aslist("region", "regions", "location"), _aslist("resource_type", "resource_types")
    tag_f = []
    tf = f.get("tags")
    if isinstance(tf, list):
        tag_f = [(str(t.get("key", "")).lower(), str(t.get("value", "")).lower()) for t in tf if isinstance(t, dict) and t.get("key")]
    ...
    def _match(r):
        if sub_f and str(getattr(r, "subscription_id", "") or "").lower() not in sub_f: return False
        if rg_f and str(getattr(r, "resource_group", "") or "").lower() not in rg_f: return False
        if reg_f and str(getattr(r, "location", "") or "").lower() not in reg_f: return False
        if type_f and str(getattr(r, "resource_type", "") or "").lower() not in type_f: return False
        if tag_f:
            tags = {str(k).lower(): str(v).lower() for k, v in (getattr(r, "tags", None) or {}).items()}
            for tk, tv in tag_f:
                if tk not in tags or (tv and tags[tk] != tv): return False
        return True

    sel = [r for r in resources if _match(r)] or resources
    _c = lambda r: float(getattr(r, "cost_current_month", 0) or 0)
    attributed = sum(_c(r) for r in sel)
    ...
    _seen = set()
    waste_src = []
    for r in orphans + oversized:
        rid = getattr(r, "resource_id", None) or id(r)
        if rid in _seen:
            continue
        _seen.add(rid)
        waste_src.append(r)
    waste = [{
        "name": getattr(r, "resource_name", "") or "", "type": (getattr(r, "resource_type", "") or "").split("/")[-1],
        "rg": getattr(r, "resource_group", "") or "", "cost": round(_c(r), 2),
        "issue": "orphaned" if getattr(r, "is_orphan", False) else "oversized",
        "current_sku": getattr(r, "sku", None), "recommended_sku": getattr(r, "rightsize_sku", None),
    } for r in sorted(waste_src, key=_c, reverse=True)[:10]]
    # ---- Authoritative COST basis: the warehouse, never per-resource costs. -------------
    # Per-resource `cost_current_month` is Cost-Management-throttled and attributes only a
    # fraction of real spend (often ~2-20%), so using it for totals/breakdowns made the AI
    # quote figures that contradicted the very view it was analysing. The warehouse
    # (finops_daily_dimension_costs) is a complete, throttle-immune partition of spend.
    from services import cost_analytics_service as _ca
    wh_subs = sub_f or None
    other_filters = bool(rg_f or reg_f or type_f or tag_f)
    try:
        d_from, d_to = _ca.resolve_period("last_30d", None, None)
        span = (datetime.fromisoformat(d_to) - datetime.fromisoformat(d_from)).days + 1
        p_to = datetime.fromisoformat(d_from) - timedelta(days=1)
        p_from = p_to - timedelta(days=span - 1)
        cur = _ca.estate_total(date_from=d_from, date_to=d_to, subscription_ids=wh_subs)
        prev = _ca.estate_total(date_from=p_from.strftime("%Y-%m-%d"),
                                date_to=p_to.strftime("%Y-%m-%d"), subscription_ids=wh_subs)

        def _wh(group_by: str, n: int = 8):
            r = _ca.analyze(subscription_ids=wh_subs, group_by=group_by,
                            date_from=d_from, date_to=d_to, top=n) or {}
            return [{"name": b.get("key"), "cost": round(float(b.get("cost") or 0), 2)}
                    for b in (r.get("breakdown") or [])][:n]

        by_svc, by_rg, by_sub, by_reg = (_wh("service_family"), _wh("resource_group"),
                                         _wh("subscription"), _wh("location"))
        wh_ok = cur > 0
    except Exception as _we:
        logger.warning("FinOps AI grounding: warehouse cost basis unavailable (%s)", _we)
        d_from = d_to = ""
        cur = prev = 0.0
        by_svc = by_rg = by_sub = by_reg = []
        wh_ok = False

    coverage_pct = round(attributed / cur * 100, 1) if cur > 0 else 0.0
    return {
        "cost_basis": {
            "source": ("Azure cost warehouse (finops_daily_dimension_costs) — complete, "
                       "throttle-immune daily actuals"
                       if wh_ok else "UNAVAILABLE — no authoritative cost figures for this request"),
            "window": f"{d_from} → {d_to} (last 30 days)" if wh_ok else "n/a",
            "currency": "USD",
            "authoritative_spend_usd": round(cur, 2),
            "prior_equal_window_spend_usd": round(prev, 2),
            "change_pct": round((cur - prev) / prev * 100, 1) if prev > 0 else None,
            "scoped_to_subscriptions": wh_subs or "all configured subscriptions",
            "filters_applied_to_these_totals": (
                "subscription only — resource group / region / type / tag filters are NOT applied "
                "to these authoritative totals, so do not present them as the filtered total"
                if other_filters else "subscription scope only (no other filters active)"),
        },
        # Warehouse-derived: these SUM to authoritative_spend_usd (top-N + remainder).
        "cost_by_service": by_svc, "cost_by_resource_group": by_rg,
        "cost_by_subscription": by_sub, "cost_by_region": by_reg,
        # Inventory facts are COMPLETE (they come from Resource Graph, not Cost Management).
        "resource_inventory": {
        ...
        # Everything below is PARTIAL. It exists to name real resources, not to total spend.
        "resource_level_costs": {
            "warning": (
                f"INCOMPLETE — per-resource costs account for only {coverage_pct}% of "
                f"authoritative spend (${attributed:,.2f} of ${cur:,.2f}) because Azure Cost "
                "Management per-resource queries are rate-limited. Use these ONLY to rank and "
                "name resources. NEVER sum them into a total, never present them as estate "
                "spend, and never contradict authoritative_spend_usd with them."),
            "coverage_pct": coverage_pct,
            "attributed_usd": round(attributed, 2),
...
async def finops_ai_insights(req: FinOpsAIRequest):
    ...
    Every request is grounded on real resource-level facts from the dashboard cache.
    """
    _require_finops()
    from services import finops_ai_service as _ai
    loop = asyncio.get_event_loop()
    # Attach authoritative resource-level grounding so the AI cites real resources.
    data_for_ai = dict(req.data or {})
    try:
        grounding = _finops_ai_grounding(req.filters)
        if grounding:
            data_for_ai["_grounding"] = grounding
    except Exception as _ge:
        logger.debug("FinOps AI grounding skipped: %s", _ge)
    try:
        result = await asyncio.wait_for(
            loop.run_in_executor(
                _ai_pool,
                lambda: _ai.get_finops_insights(req.view, data_for_ai, req.filters, req.force_refresh, req.scope, req.context, req.question),
            ),
            timeout=_FINOPS_AI_TIMEOUT_SECS,
        )
    except Exception as exc:
...
# ── Advanced FinOps: cost drill-down · dependencies/workloads · period compare ───

def _finops_dash_dict() -> Optional[dict]:
    """Dashboard resource cache as a plain dict (for finops_advanced_service)."""
    dash_model = _cache.get("data:*") or _cache.get("data")
    if not dash_model:
        return None
    try:
        return json.loads(dash_model.model_dump_json())
    except Exception:
        return None


@app.get("/api/finops/cost-resources", tags=["FinOps"])
async def finops_cost_resources(
    subscription_id: Optional[str] = None,
    resource_group: Optional[str] = None,
    ...
    """Cost drill-down leaf: the underlying resources (with individual monthly cost)
    behind a Cost-Explorer slice. Instant — served from the dashboard resource cache."""
    _require_finops()
    dash = _finops_dash_dict()
    if dash is None:
        return {"resources": [], "count": 0, "total_cost": 0.0, "prev_total_cost": 0.0,
                "delta_usd": 0.0, "data_source": "cache_unavailable",
                "generated_at": datetime.now(timezone.utc).isoformat()}
    loop = asyncio.get_event_loop()
    out = await loop.run_in_executor(_pool, lambda: finops_adv_svc.filter_cost_resources(
        dash, subscription_id=subscription_id, resource_group=resource_group,
        resource_type=resource_type, region=region, tag_key=tag_key, tag_value=tag_value,
        environment=environment, search=search, min_cost=min_cost, max_cost=max_cost, limit=limit))
    out["data_source"] = "dashboard_cache"
    return out


@app.get("/api/finops/workloads", tags=["FinOps"])
async def finops_workloads(group_by: str = "dependency", tag_key: Optional[str] = None,
                           subscription_id: Optional[str] = None, region: Optional[str] = None,
                           limit: int = 200):
    """Cost dependencies / workload roll-up.
    group_by = 'dependency' (connected-component clusters: a VM + its disks + NIC +
    public IP + backup as one application) | 'resource_group' | 'tag'.
    Optional subscription_id / region scope filters."""
    _require_finops()
    dash = _finops_dash_dict()
    g = _cache.get("dependency_graph")
    # Lazily build + cache the dependency graph if missing/empty. Startup builds it
    # from the persisted snapshot, which may be empty on a fresh local instance or
    # before the first scan completes. Build offline (no ARG call) for speed.
    if group_by == "dependency" and (g is None or getattr(g, "node_count", 0) == 0):
        try:
            _res = _cache.get("data:*") or _cache.get("data")
            _rl = [r.model_dump() for r in (_res.resources or [])] if _res else []
            if _rl:
                g = build_dependency_graph(_rl, use_resource_graph=False)
                _cache["dependency_graph"] = g
                logger.info("FinOps workloads: built dependency graph on demand — %d nodes, %d edges, %d clusters",
                            g.node_count, g.edge_count, g.cluster_count)
        except Exception as _ge:
            logger.warning("FinOps workloads: on-demand dependency graph build failed: %s", _ge)
    graph = None
    if g is not None:
        try:
            graph = g.model_dump()
            ...
            graph = None
    if dash is None:
        return {"mode": group_by, "workloads": [], "workload_count": 0, "total_cost": 0.0,
                "untagged_cost": 0.0, "graph_available": graph is not None,
                "data_source": "cache_unavailable",
                "generated_at": datetime.now(timezone.utc).isoformat()}
    loop = asyncio.get_event_loop()
    out = await loop.run_in_executor(_pool, lambda: finops_adv_svc.workload_rollup(
        dash, graph, group_by=group_by, tag_key=tag_key,
        subscription_id=subscription_id, region=region, limit=limit))
    out["data_source"] = "dependency_graph" if group_by == "dependency" else "dashboard_cache"
    out["graph_available"] = graph is not None
    return out


@app.get("/api/finops/compare", tags=["FinOps"])
async def finops_compare(dimension: str = "ResourceGroupName", tag_key: Optional[str] = None, limit: int = 100):
    """Period-over-period: current month (MTD) vs previous month, grouped by dimension.
    Instant — uses cost_current_month / cost_previous_month already on each resource."""
    _require_finops()
    dash = _finops_dash_dict()
    if dash is None:
        return {"dimension": dimension, "rows": [], "row_count": 0, "total_current": 0.0,
                "total_prior": 0.0, "data_source": "cache_unavailable",
                "generated_at": datetime.now(timezone.utc).isoformat()}
    loop = asyncio.get_event_loop()
    out = await loop.run_in_executor(_pool, lambda: finops_adv_svc.period_compare(
        dash, dimension=dimension, tag_key=tag_key, limit=limit))
    out["data_source"] = "dashboard_cache"
    return out
...
async def finops_analyze(req: FinOpsAnalyzeRequest):
    ...
    from services import cost_analytics_service as _ca
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(_pool, lambda: _ca.analyze(
    ...
    # Attach provenance. Without it a chart is just numbers — the reader cannot tell
    # which subscriptions, window or cost basis produced them, and an export is useless.
    if isinstance(result, dict):
        d_from, d_to = result.get("date_from"), result.get("date_to")
        span = ""
        if d_from and d_to:
            try:
                span = f" ({(datetime.fromisoformat(d_to) - datetime.fromisoformat(d_from)).days + 1} days)"
            except Exception:
                span = ""
        scope = _finops_scope_block(
            period_label=f"{d_from} → {d_to}{span}" if d_from and d_to else "",
            source=f"Cost warehouse (finops_daily_dimension_costs) · "
                   f"{result.get('cost_type', 'actual')} cost · grouped by {result.get('group_by', '')}",
        )
        if req.subscription_ids:
            requested = set(req.subscription_ids)
            narrowed = [s for s in scope["subscriptions"] if s["id"] in requested]
            if narrowed:
                scope["subscriptions"] = narrowed
                scope["subscription_count"] = len(narrowed)
        if req.resource_group:
            scope["resource_groups"] = req.resource_group
        scope["currency"] = result.get("currency") or scope.get("currency") or "USD"
        result["scope"] = scope
        # Echo the effective subscription set so downstream consumers never see an
        # empty list that implies "no scope" when the query actually covered them all.
        if not result.get("subscription_ids"):
            result["subscription_ids"] = [s["id"] for s in scope["subscriptions"]]
    return result
...
async def finops_cost_insights():
    ...
    modern_by_id: dict = {}
    # The opportunity's own monthly_cost was captured during the scan, when per-resource
    # cost was throttled to 0 — fall back to the hydrated cost so savings aren't all $0.
    cost_by_id = {str(r.get("resource_id") or "").lower(): float(r.get("cost_current_month") or 0)
                  for r in resources}
    try:
        for mo in (data.modernization_opportunities or []):
            ...
            if not rid:
                continue
            monthly = float(d.get("monthly_cost", 0) or 0) or cost_by_id.get(rid, 0.0)
            sav = round(monthly * (d.get("estimated_savings_pct", 0) or 0) / 100.0, 2)
...
def _finops_scope_block(period_label: str = "", source: str = "") -> Dict[str, Any]:
    """Provenance for a FinOps figure: which estate slice, which window, which currency.

    Charts and exports are meaningless without it — a burndown of "$382" tells a reader
    nothing unless it also says whose subscriptions and which month it covers.
    """
    subs: List[Dict[str, str]] = []
    try:
        configured = list(settings_svc.get_subscription_ids() or [])
    except Exception:
        configured = []
    names: Dict[str, str] = {}
    for resolver in (_sub_name_map, _resolve_all_subscription_names):
        try:
            for sid, nm in (resolver() or {}).items():
                if sid and nm and sid not in names:
                    names[sid] = nm
        except Exception:
            continue
    subs = [{"id": sid, "name": names.get(sid) or sid} for sid in configured]

    now = datetime.now(tz=timezone.utc)
    if not period_label:
        period_label = f"Month to date — {now:%B %Y} (1–{now.day} {now:%b})"
    return {
        "subscriptions": subs,
        "subscription_count": len(subs),
        "resource_groups": "All resource groups (no filter applied)",
        "period": period_label,
        "currency": "USD",
        "source": source or "Azure Cost Management daily actuals",
...
async def finops_budget_scenario(model: Dict[str, Any] = Body(default={})):
    ...
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(_pool, lambda: _bs.build_burndown(cm, budget, growth))
    if isinstance(result, dict):
        result["scope"] = _finops_scope_block(
            source="Azure Cost Management daily actuals (all configured subscriptions)")
    return result
...
async def finops_chargeback_compute(model: Dict[str, Any] = Body(...)):
    ...
    centers via ordered rules, distribute the shared/unmatched pool (proportional /
    even / weighted / headcount), and apply an optional overhead markup. Computed
    against the dashboard resource cache (per-resource cost + tags), so it is instant
    and immune to Cost Management throttling."""
    _require_finops()
    dash = _finops_dash_dict()
    if dash is None:
        return {"cost_centers": [], "total_spend_usd": 0.0, "chargeback_total_usd": 0.0,
                "shared_pool_usd": 0.0, "unallocated_usd": 0.0, "cost_center_count": 0,
                "data_source": "cache_unavailable",
                "generated_at": datetime.now(timezone.utc).isoformat()}
    loop = asyncio.get_event_loop()
    out = await loop.run_in_executor(_pool, lambda: finops_adv_svc.compute_chargeback(dash, model))
    out["data_source"] = "dashboard_cache"
    return out


@app.post("/api/finops/unit-economics", tags=["FinOps"])
async def finops_unit_economics(model: Dict[str, Any] = Body(...)):
    """Unit economics / business value — divide the (optionally scoped) cloud cost by
    user-defined value drivers (customers, transactions, revenue) to produce
    cost-per-unit, its MoM change, and variance vs target. Computed from the resource
    cache, so it is instant and throttling-immune."""
    _require_finops()
    dash = _finops_dash_dict()
    if dash is None:
        return {"drivers": [], "scope_current_usd": 0.0, "scope_previous_usd": 0.0,
                "resource_count": 0, "data_source": "cache_unavailable",
                "generated_at": datetime.now(timezone.utc).isoformat()}
    loop = asyncio.get_event_loop()
...
async def finops_resource_optimization():
    ...
    for r in (dash.resources or []):
        ...
        # Monthly run-rate cost: the current month is partial early on (and can read $0
        ...
        if r.rightsize_sku and mc > 0:
            oversized.append({
                **base,
                ...
                "monthly_savings": round(mc * r.rightsize_savings_pct / 100, 2),
...
def _style_bcdr_workbook(wb, max_fill_col: int = 12) -> None:
    """Repaint a BCDR / BIA workbook in the consultant PDF's dark theme so the Excel
    deliverable matches the signed-off PDF (page #0F172A, panels #1E293B, slate header
    bands #334155, blue titles #60A5FA / headings #93C5FD, light body #CBD5E1).

    Section bands and table headers (built with the legacy navy #1E3A5F fill) are
    re-coloured in place — a single-title navy row becomes a blue section heading on a
    panel band, a multi-label navy row becomes a white slate table header. Severity
    chips keep their colour. Every other cell gets the dark page background with light
    body text and a hairline row rule. Best-effort; never raises.
    """
    try:
        from openpyxl.styles import Font, PatternFill, Border, Side
    except Exception:
        return
    PAGE = PatternFill("solid", fgColor="0F172A")
    PANEL = PatternFill("solid", fgColor="1E293B")
    SLATE = PatternFill("solid", fgColor="334155")
    row_rule = Border(bottom=Side(style="thin", color="22304A"))
    sec_rule = Border(bottom=Side(style="medium", color="3B82F6"))
    NB = "Segoe UI"
    DARK_FONTS = {None, "FF000000", "00000000", "000000"}

    def _is_navy(c):
        try:
            f = c.fill
            if not (f and getattr(f, "patternType", None)):
                return False
            rgb = f.fgColor.rgb
            return isinstance(rgb, str) and rgb.upper().endswith("1E3A5F")
        except Exception:
            return False

    def _font_rgb(font):
        try:
            return font.color.rgb if (font and font.color and isinstance(font.color.rgb, str)) else None
        except Exception:
            return None
    ...
    for ws in wb.worksheets:
        ...
        last_col = min(max(ws.max_column or 1, 8), max_fill_col)
        for row in ws.iter_rows(min_row=1, max_row=last_row, min_col=1, max_col=last_col):
            cells = list(row)
            navy_labeled = sum(1 for c in cells if _is_navy(c) and c.value not in (None, ""))
            is_table_header = navy_labeled >= 2
            for cell in cells:
                try:
                    old = cell.font
                    size = old.size if (old and old.size) else 10.5
                    # 1) Legacy navy band → slate table header OR blue section heading.
                    if _is_navy(cell):
                        if is_table_header:
                            cell.fill = SLATE
                            cell.font = Font(name=NB, size=size, bold=True, color="FFFFFF")
                        else:
                            cell.fill = PANEL
                            cell.font = Font(name=NB, size=max(size, 11), bold=True, color="93C5FD")
                            cell.border = sec_rule
                        continue
                    # 2) Any other deliberate fill (severity chips) → keep, ensure light text.
                    f = cell.fill
                    if f is not None and getattr(f, "patternType", None) is not None:
                        if _font_rgb(old) in DARK_FONTS or (_font_rgb(old) or "").upper() in DARK_FONTS:
                            cell.font = Font(name=NB, size=size, bold=bool(old and old.bold), color="FFFFFF")
                        continue
                    # 3) Plain cell → dark page background + light body text.
                    is_title = bool(old and old.size and old.size >= 13)
                    is_italic = bool(old and old.italic)
                    is_bold = bool(old and old.bold)
                    has_val = cell.value not in (None, "")
                    cell.fill = PAGE
                    if is_title:
                        cell.font = Font(name=NB, size=max(size, 16), bold=True, color="60A5FA")
                    elif is_italic:
                        cell.font = Font(name=NB, size=size, italic=True, color="94A3B8")
                    else:
                        cell.font = Font(name=NB, size=size, bold=is_bold, color="CBD5E1")
                    if has_val and not is_title:
...
@app.post("/api/finops/export/generic-xlsx", tags=["FinOps"])
async def finops_export_generic_xlsx(payload: Dict[str, Any] = Body(...)):
    """Generic multi-sheet XLSX export usable by any FinOps module. Body:
    { "title": str, "sheets": [{ "name": str, "columns": [str], "rows": [[cell,...]] }] }"""
    _require_finops()
    try:
        import openpyxl
    ...
    import re
    from io import BytesIO
    title = str(payload.get("title") or "FinOps Export")
...
class FinOpsReportBody(BaseModel):
    ...
    customer: str = ""
    use_ai: bool = True
    # Custom report builder — which sections to include and what to scope it to.
    ...
    tag_key: Optional[str] = None
    tag_value: Optional[str] = None
    tagged: Optional[bool] = None        # True = tagged only, False = untagged only
    # Author's brief — free text from the wizard. Steers emphasis and what the
    # narrative must answer; it is never treated as data (see _guidance_block).
...
async def finops_exec_report_generate(body: FinOpsReportBody):
    ...
    # Scope filters for the custom report builder. Tag key/value is not on the
    # insights rows (they only carry has_tags), so resolve it against the scan
    # cache and hand the service an explicit allow-list of resource ids.
    filters: Dict[str, Any] = {
        "resource_groups": body.resource_groups or [],
        "regions": body.regions or [],
        "resource_types": body.resource_types or [],
        "tagged": body.tagged,
        "subscription_labels": [sub_names.get(s, s) for s in (body.scope or []) if s],
    }
    if body.tag_key:
        want_k = body.tag_key.strip().lower()
        want_v = (body.tag_value or "").strip().lower()
        allow: List[str] = []
        try:
            for r in _get_resources_list():
                tags = r.get("tags") or {}
                for k, v in tags.items():
                    if str(k).lower() != want_k:
                        continue
                    if want_v and str(v or "").lower() != want_v:
                        continue
            ...
            logger.warning("exec-report: tag filter could not be resolved: %s", exc)
        filters["resource_ids"] = allow
        filters["tag_label"] = f"{body.tag_key}={body.tag_value}" if want_v else body.tag_key

    # Module report types (security, resilience, modernization, inventory, advisor,
    # well-architected) are grounded in the estate scan rather than the cost warehouse.
    estate: Dict[str, Any] = {}
    if (_frs.REPORT_TYPES.get(body.report_type) or {}).get("module"):
        try:
            # Synchronous accessor — it rehydrates from the durable snapshot in-process.
            _dash = await loop.run_in_executor(_pool, _ensure_dashboard_in_cache)
            if _dash is not None:
                estate = _dash.model_dump() if hasattr(_dash, "model_dump") else dict(_dash)
            if not (estate.get("resources") or []):
                logger.warning("exec-report: estate scan has no resources — module blocks will report not-collected")
        except Exception as exc:
            logger.warning("exec-report: estate scan unavailable: %s", exc)
            estate = {}
...
@app.get("/api/data/availability", tags=["Data"])
async def data_availability():
    """What has actually been collected, per dataset: rows, coverage window, freshness,
    which modules read it, and whether it comes from the Azure SQL warehouse, the scan
    cache, live Azure calls or user input. Lets a panel say WHY it is empty."""
    from services import data_availability_service as _av
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(_pool, _av.get_availability)
...
@app.get("/api/finops/log-analytics/workspaces", tags=["FinOps Management"])
async def la_workspaces(days: int = 30, subscription_id: Optional[str] = None):
    """Log Analytics / Sentinel workspaces with billable GB, allocated cost and $/GB."""
    from services import log_analytics_cost_service as _la
    loop = asyncio.get_event_loop()
    subs = _mgmt_subs(subscription_id)
    return await loop.run_in_executor(
        _pool, lambda: _la.get_workspace_summary(days, subs))


@app.get("/api/finops/log-analytics/tables", tags=["FinOps Management"])
async def la_tables(workspace_id: Optional[str] = None, days: int = 30,
                    top: int = 50, subscription_id: Optional[str] = None):
    """Per-TABLE ingestion volume and allocated cost inside a workspace.

    Cost is apportioned by billable-GB share — Azure does not bill per table — so the
    response carries cost_basis for the UI to label."""
    from services import log_analytics_cost_service as _la
    loop = asyncio.get_event_loop()
    subs = _mgmt_subs(subscription_id)
    return await loop.run_in_executor(
        _pool, lambda: _la.get_table_costs(workspace_id, days, top, subs))


@app.post("/api/finops/log-analytics/collect", tags=["FinOps Management"])
async def la_collect(days: int = 30, subscription_id: Optional[str] = None):
    """Query every in-scope workspace's Usage table and persist per-table cost."""
    from services import log_analytics_cost_service as _la
    loop = asyncio.get_event_loop()
    subs = _mgmt_subs(subscription_id)
    run_id = f"la-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    return await loop.run_in_executor(
        _pool, lambda: _la.collect_all(subs, run_id=run_id, days=days))
...
async def get_cached_dashboard_instant():
    ...
    if best_data is not None:
        _hydrate_resource_costs(best_data)
...
async def run_bcdr_plan(project_id: str, body: BcdrPlanBody):
    ...
    # NOTE: the on-screen BCDR Plan is a fast posture PREVIEW. The saved, re-downloadable BCDR
    # deliverable is the full consultant proposal (persisted by /api/bcdr/consultant-report when
    # the user exports it). We intentionally do NOT persist the preview to history to avoid weak /
    # duplicate entries — the History tab should contain only client-ready deliverables.
...
async def bcdr_recovery_sequence(
    ...
    resource_group: Optional[str] = None,
    resource_type: Optional[str] = None,
    azure_tag: Optional[str] = None,    # "key=value"
    custom_tag: Optional[str] = None,   # "key=value"
):
    """Recovery Sequence Planner — dependency-aware recovery ordering.

    Optional query params scope the analysis BEFORE dependency edges are computed, so the
    waves, edges and RTO totals all reflect the user's chosen scope (e.g. one subscription,
    one resource group, one workload tagged Application=Orders). Without filters the
    endpoint runs on the entire estate — which is rarely meaningful for multi-subscription
    customers and is exactly what produced the misleading 'Foundation' wave in the dashboard.
    """
    data = _cache.get("data:*") or _cache.get("data")
    if not data:
        raise HTTPException(status_code=404, detail="No scan data available")
    ...
    # Apply scope filters. We do this on a list comprehension rather than at the SDK level
    # because the resources are already in-memory and the filter cost is trivial.
    def _tag_pair(spec: Optional[str]) -> Optional[tuple[str, str]]:
        if not spec or "=" not in spec:
            return None
        k, v = spec.split("=", 1)
        return (k, v)

    az_kv = _tag_pair(azure_tag)
    ct_kv = _tag_pair(custom_tag)

    # Custom-tag lookup is keyed by lowercased resource_id — pre-load only when needed.
    custom_tag_map: Dict[str, Dict[str, str]] = {}
    if ct_kv:
        try:
            from services import tagging_service as _tsvc
            custom_tag_map = _tsvc.get_all_custom_tags() or {}
        except Exception as exc:
            logger.warning("recovery-sequence: custom-tag filter skipped: %s", exc)
            ct_kv = None

    # ARM is case-insensitive for sub/RG/type and tag keys — compare lowercase so the UI
    # dropdown values match regardless of how the resource provider returned the casing.
    sub_low  = (subscription_id or "").lower()
    rg_low   = (resource_group or "").lower()
    type_low = (resource_type or "").lower()

    def _keep(r: dict) -> bool:
        if sub_low and (r.get("subscription_id") or "").lower() != sub_low:
            return False
        if rg_low and (r.get("resource_group") or "").lower() != rg_low:
            return False
        if type_low and (r.get("resource_type") or "").lower() != type_low:
            return False
        if az_kv:
            k, v = az_kv
            tags_low = {str(tk).lower(): str(tv) for tk, tv in (r.get("tags") or {}).items()}
            if tags_low.get(k.lower(), "") != v:
                return False
        if ct_kv:
            k, v = ct_kv
            rid_low = (r.get("resource_id") or "").lower()
            ctags = custom_tag_map.get(rid_low) or {}
            ctags_low = {str(tk).lower(): str(tv) for tk, tv in ctags.items()}
            if ctags_low.get(k.lower(), "") != v:
                return False
        return True

    scoped = [r for r in resources if _keep(r)]
    assessments = _get_bcdr_assessments()
    result = build_recovery_sequence_plan(scoped, assessments)
    # Echo the applied scope so the UI can render an honest "scoped to N of M" banner
    # without re-computing the filter client-side.
    result["scope"] = {
        "subscription_id":  subscription_id or None,
        "resource_group":   resource_group or None,
        "resource_type":    resource_type or None,
        "azure_tag":        azure_tag or None,
        "custom_tag":       custom_tag or None,
        "scoped_resources": len(scoped),
        "total_resources":  len(resources),
        "filtered":         len(scoped) != len(resources),
    }
    return result
...
async def bcdr_consultant_report(body: ConsultantReportBody):
    ...
    # Optional collective report: run the BIA first and ground the BCDR strategy ON it
    # (ISO 22301 — the BIA is the foundation of the BC plan). Produces ONE deliverable
    # containing both the Business Impact Analysis and the BCDR strategy.
    # Resolve subscription friendly names (GUID -> name) once - shared by the optional BIA
    # stage and injected into the consultant intake so the Scope / environment narrative
    # names the real subscription instead of a GUID fragment.
    try:
        _settings = settings_svc.get()
        sub_names = {s.get("id", ""): s.get("name", "") for s in _settings.get("subscriptions", [])}
        for _sid, _sname in (_sub_name_map() or {}).items():
            if _sname:
                sub_names[_sid] = _sname
    except Exception:
    ...
    bia_report = None
    if body.include_bia:
        from services.bia_service import generate_bia_report
        custom = {}
        try:
            raw_custom = tagging_svc.get_all_custom_tags()
            custom = {(k or "").lower(): (v or {}) for k, v in (raw_custom or {}).items()}
        except Exception as exc:
            logger.warning("Collective report: custom tag lookup failed: %s", exc)
        # The customer continuity intake doubles as BIA intake (customer_name, period, etc.).
        bia_intake = {**(body.customer_info or {}), **(body.bia_intake or {})}
        try:
            bia_report = await asyncio.to_thread(
                generate_bia_report, resources, meta, bia_intake, custom, sub_names
            )
        except RuntimeError as exc:
            raise HTTPException(status_code=503, detail=str(exc))
        except Exception as exc:
            logger.error("Collective report: BIA stage failed: %s", exc)
            raise HTTPException(status_code=500, detail=f"BIA stage failed: {exc}")

    # Carry the resolved subscription names into the consultant intake (Scope narrative).
    consultant_info = dict(body.customer_info or {})
    if sub_names:
        consultant_info["subscription_names"] = {k: v for k, v in sub_names.items() if v}

    try:
        report = await asyncio.to_thread(
            generate_consultant_report, resources, meta, consultant_info, bia_report
        )
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc))
    except Exception as exc:
    ...
    # Persist the run so the customer can revisit it (with date/time) from the project History and
    # re-download the PDF / Excel without re-running the AI. Scoped to the project when supplied.
    if body.project_id:
        try:
            report.setdefault("category", "bcdr_report")
            report.setdefault(
                "category_label",
                "Collective BCDR + BIA Proposal" if body.include_bia else "BCDR Plan & Proposal",
            )
            _m = report.get("metrics") or {}
            report["resource_count"] = _m.get("total_resources") or len(resources)
            _rec = project_svc.save_project_assessment(body.project_id, report)
            report["assessment_id"] = _rec.get("id")
        except Exception as exc:
            logger.warning("Consultant report: failed to persist run: %s", exc)
    return report
...
async def bcdr_consultant_report_xlsx(body: ConsultantReportExportBody):
    ...
    # 1b — Intake Audit (Supplied vs Assumed). Gated on intake_summary so older payloads
    # render unchanged. Mirrors the BIA's intake-audit sheet — gives the operator a
    # one-page view of what the customer stated vs what the BCDR report had to assume,
    # plus the explicit list of assumptions to close with the business.
    isum_c = rep.get("intake_summary") or {}
    assumptions_c = rep.get("assumptions") or []
    if isum_c or assumptions_c:
        ws = wb.create_sheet("Intake — Supplied vs Assumed")
        ws["A1"] = "Customer Continuity Intake — Stated vs Assumed"; ws["A1"].font = TITLE
        supplied_c = isum_c.get("supplied") or []
        not_supplied_c = isum_c.get("not_supplied") or []
        ws["A2"] = (f"{len(supplied_c)} supplied / {len(supplied_c) + len(not_supplied_c)} total "
                    f"({isum_c.get('supplied_pct', 0)}% supplied). Every recommendation later in "
                    "this workbook is anchored on the Supplied rows; anything marked '(recommended …)' "
                    "or 'Not supplied' flows from the Assumed rows.")
        ws["A2"].font = SUBTLE; ws["A2"].alignment = WRAP
        ws.merge_cells("A2:D2")
        r = 4
        if assumptions_c:
            r = section(ws, "Assumptions Used Because the Customer Did Not State the Value", r, span=4)
            r = bullets(ws, assumptions_c, r, span=4)
            r += 1
        r = section(ws, "Intake Field Audit", r, span=4)
        hdr(ws, ["Continuity Intake Field", "Status", "Customer-Stated Value", "Notes"], row=r); r += 1
        for f in supplied_c:
            ws.cell(row=r, column=1, value=f.get("label")).font = BOLD
            ws.cell(row=r, column=2, value="Supplied")
            val = f.get("value")
            ws.cell(row=r, column=3, value=str(val) if val is not None else "").alignment = WRAP
            ws.cell(row=r, column=4, value="")
            r += 1
        for f in not_supplied_c:
            ws.cell(row=r, column=1, value=f.get("label")).font = BOLD
            ws.cell(row=r, column=2, value="Not supplied — assumed")
            ws.cell(row=r, column=3, value="—")
            kind = f.get("kind") or ""
            note = ""
            if kind == "required":
                note = "Required field; blocked at intake form."
            elif kind == "recovery_target":
                note = "Group requires at least ONE supplied (default RTO / default RPO / MTD)."
            elif kind == "impact_signal":
                note = "Group requires at least ONE supplied (downtime cost / uptime SLA)."
            ws.cell(row=r, column=4, value=note).alignment = WRAP
            r += 1
        widths(ws, {"A": 44, "B": 26, "C": 38, "D": 40})
    ...
    ws = wb.create_sheet("RTO-RPO Matrix")
    # Top warning row when no customer recovery targets were supplied so the reader can't
    # mistake recommended values for stated SLAs.
    _no_targets_c = isum_c and isum_c.get("any_recovery_target_supplied") is False
    if _no_targets_c:
        warn = ws.cell(row=1, column=1, value=(
            "WARNING — Customer did not state default RTO, default RPO or MTD at intake. "
            "The 'Current' columns below read 'Not supplied' and the 'Target' columns carry "
            "the suffix '(recommended — no customer target)'. Validate with the business "
            "owner before approval."
        ))
        warn.font = Font(bold=True, color="92400E", size=10)
        warn.fill = PatternFill("solid", fgColor="FEF3C7")
        warn.alignment = Alignment(wrap_text=True, vertical="center")
        ws.merge_cells("A1:F1")
        ws.row_dimensions[1].height = 36
        hdr(ws, ["Workload", "Current RTO", "Target RTO", "Current RPO", "Target RPO", "Gap"], row=2)
        _start = 3
    else:
        hdr(ws, ["Workload", "Current RTO", "Target RTO", "Current RPO", "Target RPO", "Gap"])
        _start = 2
    for i, o in enumerate(rep.get("recovery_objectives", []), _start):
    ...
    widths(ws, {"A": 30, "B": 16, "C": 22, "D": 16, "E": 22, "F": 44})
    ...
    # 18-20 — Business Impact Analysis (only when this is a collective BCDR + BIA report)
    bia = rep.get("business_impact_analysis") or {}
    if bia:
        bes = bia.get("executive_summary", {}) or {}
        bfe = bia.get("financial_exposure", {}) or {}
        # 18 — BIA Summary
        ws = wb.create_sheet("BIA — Summary")
        ws["A1"] = "Business Impact Analysis — Summary"; ws["A1"].font = TITLE
        ws["A2"] = "Foundation of this BCDR plan · ISO 22301 / NIST SP 800-34 / ISO 27031"; ws["A2"].font = SUBTLE
        r = 4
        for k, v in [("BIA Criticality Score", f"{bia.get('criticality_score','—')} / 100"),
                     ("Criticality Label", bia.get("criticality_label", "—")),
                     ("Financial exposure — per hour", bfe.get("per_hour", "—")),
                     ("Financial exposure — per day", bfe.get("per_day", "—")),
                     ("Financial exposure — annualized", bfe.get("annualized", "—")),
                     ("Frameworks", ", ".join(bia.get("frameworks", []) or []))]:
            ws.cell(row=r, column=1, value=k).font = BOLD
            cell = ws.cell(row=r, column=2, value=v); cell.alignment = WRAP
            ws.merge_cells(start_row=r, start_column=2, end_row=r, end_column=4); r += 1
        if bes.get("headline"):
            r += 1; r = section(ws, "Headline", r); r = para(ws, bes.get("headline", ""), r)
        if bfe.get("basis"):
            r += 1; r = section(ws, "Financial Exposure Basis", r); r = para(ws, bfe.get("basis", ""), r)
        ts = bia.get("tier_summary", []) or []
        if ts:
            r += 1; r = section(ws, "Criticality Tier Mix", r)
            for t in ts:
                ws.cell(row=r, column=1, value=t.get("tier")).font = BOLD
                ws.cell(row=r, column=2, value=t.get("count", 0)); r += 1
        widths(ws, {"A": 34, "B": 40, "C": 16, "D": 20})

        # 19 — BIA Recovery Objectives + Impact-over-time
        ws = wb.create_sheet("BIA — Recovery Objectives")
        hdr(ws, ["Service / Tier", "Target RTO", "Target RPO", "Justification"])
        i = 2
        for o in (bia.get("recovery_objectives", []) or []):
            if not isinstance(o, dict):
                continue
            ws.cell(row=i, column=1, value=o.get("service") or o.get("tier") or o.get("name")).font = BOLD
            ws.cell(row=i, column=2, value=o.get("target_rto") or o.get("rto"))
            ws.cell(row=i, column=3, value=o.get("target_rpo") or o.get("rpo"))
            ws.cell(row=i, column=4, value=o.get("justification") or o.get("rationale") or "").alignment = WRAP
            i += 1
        iot = bia.get("impact_over_time", []) or []
        if iot:
            i += 1; i = section(ws, "Impact Over Time", i)
            for row_d in iot:
                if isinstance(row_d, dict):
                    horizon = row_d.get("horizon") or row_d.get("duration") or ""
                    impact = row_d.get("financial") or row_d.get("operational") or row_d.get("summary") or ""
                    ws.cell(row=i, column=1, value=horizon).font = BOLD
                    ws.cell(row=i, column=2, value=str(impact)).alignment = WRAP
                    ws.merge_cells(start_row=i, start_column=2, end_row=i, end_column=4); i += 1
        widths(ws, {"A": 30, "B": 16, "C": 16, "D": 60})

        # 20 — Per-resource BIA Matrix (deterministic; never AI-altered)
        ws = wb.create_sheet("BIA — Matrix")
        hdr(ws, ["Resource", "Type", "RG", "Location", "Tier", "Impact", "Source",
                 "Target RTO (h)", "$ Downtime/hr", "Monthly $", "Owner"])
        for i, m in enumerate(bia.get("bia_matrix", []) or [], 2):
            ws.cell(row=i, column=1, value=m.get("resource_name")).font = BOLD
            ws.cell(row=i, column=2, value=m.get("resource_type")); ws.cell(row=i, column=3, value=m.get("resource_group"))
            ws.cell(row=i, column=4, value=m.get("location")); ws.cell(row=i, column=5, value=m.get("bia_tier"))
            ws.cell(row=i, column=6, value=m.get("impact_score")); ws.cell(row=i, column=7, value=m.get("criticality_source"))
            ws.cell(row=i, column=8, value=m.get("target_rto_hours"))
            dc = ws.cell(row=i, column=9, value=m.get("downtime_cost_hr")); dc.number_format = "#,##0.00"
            mc = ws.cell(row=i, column=10, value=m.get("monthly_cost")); mc.number_format = "#,##0.00"
            ws.cell(row=i, column=11, value=m.get("business_owner"))
        widths(ws, {"A": 28, "B": 18, "C": 16, "D": 14, "E": 18, "F": 8, "G": 12,
                    "H": 13, "I": 14, "J": 12, "K": 18})

    # 21 — Qatar Regional & Regulatory DR Strategy (deterministic — Microsoft Qatar engineering)
    qp = rep.get("qatar_policy") or {}
    if qp:
        ws = wb.create_sheet("Qatar Regional Strategy")
        ws["A1"] = qp.get("title") or "Qatar Regional & Regulatory DR Strategy"
        ws["A1"].font = TITLE
        ws["A2"] = qp.get("applies_to") or ""
        ws["A2"].font = SUBTLE
        r = 4
        r = section(ws, "Region Policy & Sovereignty Principles", r)
        for line in (qp.get("principles") or []):
            ws.cell(row=r, column=1, value=f"\u2022 {line}").alignment = WRAP
            ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4); r += 1
        if qp.get("roc_supported_workloads"):
            r += 1; r = section(ws, "Azure Backup — Region-of-Choice (RoC) Supported Workloads", r)
            for line in qp.get("roc_supported_workloads", []):
                ws.cell(row=r, column=1, value=f"\u2022 {line}").alignment = WRAP
                ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4); r += 1
        if qp.get("roc_not_supported"):
            r += 1; r = section(ws, "RoC Limitations & Not Supported", r)
            for line in qp.get("roc_not_supported", []):
                ws.cell(row=r, column=1, value=f"\u2022 {line}").alignment = WRAP
                ws.merge_cells(start_row=r, start_column=1, end_row=r, end_column=4); r += 1
        if qp.get("references"):
            r += 1; r = section(ws, "References", r)
            for ref in qp.get("references", []):
                if isinstance(ref, (list, tuple)) and len(ref) >= 2:
                    label, url = ref[0], ref[1]
                else:
                    label, url = str(ref), ""
                ws.cell(row=r, column=1, value=label).font = BOLD
                ws.cell(row=r, column=2, value=url).alignment = WRAP
                ws.merge_cells(start_row=r, start_column=2, end_row=r, end_column=4); r += 1
        widths(ws, {"A": 44, "B": 40, "C": 24, "D": 24})

    # 22 — Per-Service Technical DR Build Guide (deterministic — Qatar BCDR playbook)
    pl = rep.get("service_dr_playbook") or []
    if pl:
        ws = wb.create_sheet("Per-Service DR Build Guide")
        hdr(ws, ["Resource", "Azure Service", "Current Region", "Recommended DR Region",
                 "AI Workload?", "RoC Eligible?", "Prerequisites", "Backup Approach",
                 "DR Approach (Azure-native)", "Target RTO", "Target RPO", "Region Policy Note",
                 "Reference"])
        for i, p in enumerate(pl, 2):
            ws.cell(row=i, column=1, value=p.get("resource_name")).font = BOLD
            ws.cell(row=i, column=2, value=p.get("azure_service"))
            ws.cell(row=i, column=3, value=p.get("current_region"))
            ws.cell(row=i, column=4, value=p.get("recommended_dr_region"))
            ws.cell(row=i, column=5, value="Yes" if p.get("is_ai_workload") else "No")
            ws.cell(row=i, column=6, value="Yes" if p.get("roc_eligible") else "No")
            ws.cell(row=i, column=7, value=p.get("prereqs")).alignment = WRAP
            ws.cell(row=i, column=8, value=p.get("backup_approach")).alignment = WRAP
            ws.cell(row=i, column=9, value=p.get("dr_approach")).alignment = WRAP
            ws.cell(row=i, column=10, value=p.get("target_rto"))
            ws.cell(row=i, column=11, value=p.get("target_rpo"))
            ws.cell(row=i, column=12, value=p.get("region_policy_note")).alignment = WRAP
            ws.cell(row=i, column=13, value=p.get("reference")).alignment = WRAP
        widths(ws, {"A": 26, "B": 22, "C": 16, "D": 20, "E": 10, "F": 10,
                    "G": 40, "H": 40, "I": 50, "J": 14, "K": 14, "L": 44, "M": 38})

    for sheet in wb.worksheets:
        if sheet.title != "Executive Summary":
            sheet.freeze_panes = "A2"

    _style_bcdr_workbook(wb)
    buf = BytesIO(); wb.save(buf); buf.seek(0)
...
# ── Business Impact Analysis (consultant-grade, framework-based, AI) ───────────
class BIAGenerateBody(BaseModel):
    project_id: Optional[str] = None
    resource_ids: Optional[List[str]] = None
    intake: Dict[str, Any] = {}


class BIAExportBody(BaseModel):
    report: Dict[str, Any]


def _bia_resolve_resources(project_id: Optional[str], resource_ids: Optional[List[str]]) -> List[dict]:
    """Resolve the resource set for a BIA: a project's resources, an explicit selection,
    or (when neither is supplied) the whole estate."""
    resources = _get_resources_list()
    if not resources:
        raise HTTPException(status_code=404, detail="No resource data — run a scan first")
    if project_id:
        ...
        if not project:
            raise HTTPException(status_code=404, detail="Project not found")
        ids = {r.lower() for r in (project.get("resource_ids") or [])}
        if not ids:
            raise HTTPException(status_code=400, detail="Project has no resources. Add resources first.")
        resources = [r for r in resources if (r.get("resource_id") or "").lower() in ids]
    elif resource_ids:
        ids = {(r or "").lower() for r in resource_ids}
        resources = [r for r in resources if (r.get("resource_id") or "").lower() in ids]
    if not resources:
        raise HTTPException(status_code=404, detail="None of the selected resources are in the current scan.")
    return resources


@app.post("/api/bia/generate")
async def bia_generate(body: BIAGenerateBody):
    """Generate a consultant-grade, standards-based (ISO 22301 / NIST SP 800-34 / ITIL 4 /
    ISO 27031) Business Impact Analysis for the selected resources — grounded on the live
    inventory, Azure + custom/Phase-1 tags and resource posture, with an AI narrative."""
    import asyncio
    from services.bia_service import generate_bia_report

    resources = _bia_resolve_resources(body.project_id, body.resource_ids)
    ...
    try:
        report = await asyncio.to_thread(
            generate_bia_report, resources, meta, body.intake or {}, custom, sub_names
        )
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc))
    except Exception as exc:
        logger.error("BIA generation failed: %s", exc)
        _low = str(exc).lower()
        if any(k in _low for k in ("connection error", "timeout", "getaddrinfo", "temporarily unavailable", "service unavailable", "apiconnection")):
            raise HTTPException(status_code=503, detail="AI service is unreachable. Verify the Azure OpenAI endpoint/key, then retry.")
        raise HTTPException(status_code=500, detail=f"BIA generation failed: {exc}")

    # Persist the run so it appears in the project History with date/time and can be re-opened and
    # re-downloaded (PDF + Excel) without re-running the AI. Only when scoped to a project.
    if body.project_id:
        try:
            _es = report.get("executive_summary") or {}
            _sc = _es.get("criticality_score")
            report.setdefault("category", "bia")
            report.setdefault("category_label", "Business Impact Analysis")
            if isinstance(_sc, (int, float)):
                report["overall_score"] = int(_sc)
            report["score_label"] = _es.get("criticality_label") or report.get("score_label") or ""
            report["resource_count"] = len(resources)
            _rec = project_svc.save_project_assessment(body.project_id, report)
            report["assessment_id"] = _rec.get("id")
        except Exception as exc:
            logger.warning("BIA: failed to persist run: %s", exc)
    return report


@app.post("/api/bia/export.xlsx")
async def bia_export_xlsx(body: BIAExportBody):
    """Export the Business Impact Analysis as a multi-sheet enterprise Excel workbook
    (parity with the on-screen BIA + the PDF export)."""
    rep = body.report or {}
    try:
        import openpyxl
        from openpyxl.styles import Font, PatternFill, Alignment
    ...
    def section(ws, title, row, span=6):
        cell = ws.cell(row=row, column=1, value=title); cell.font = HFONT; cell.fill = HFILL
        for c in range(2, span + 1):
            ws.cell(row=row, column=c).fill = HFILL
        return row + 1

    def bullets(ws, items, row, span=6):
        for x in (items or []):
            txt = x if isinstance(x, str) else (
                f"{x.get('action') or x.get('recommendation') or x.get('gap') or ''}" if isinstance(x, dict) else str(x))
            cell = ws.cell(row=row, column=1, value=f"• {txt}"); cell.alignment = WRAP
            ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=span); row += 1
        return row

    def para(ws, text, row, span=6):
        cell = ws.cell(row=row, column=1, value=text if text not in (None, "") else "—"); cell.alignment = WRAP
        ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=span); return row + 1
    ...
    ws = wb.active; ws.title = "Executive Summary"
    try:
        _xlsx_brand_logo(ws, "F1")
    except Exception:
        pass
    ws["A1"] = f"{cust} — Business Impact Analysis"; ws["A1"].font = TITLE
    ws["A2"] = f"Prepared by {cover.get('prepared_by','')}  ·  v{cover.get('report_version','')}  ·  {cover.get('date','')}  ·  Confidential"; ws["A2"].font = SUBTLE
    r = 4
    for k, v in [("Customer", cust), ("Assessment Period", cover.get("assessment_period", "")),
                 ("Business Criticality Score", f"{rep.get('overall_score','—')} / 100"),
                 ("Criticality Rating", rep.get("score_label", "—")),
                 ("Aggregate downtime cost/hr", es.get("aggregate_downtime_cost_per_hour", "—")),
                 ("Resources analysed", metrics.get("total_resources", 0)),
                 ("Tag coverage", f"{rep.get('grounding',{}).get('tagged_pct',0)}%"),
                 ("AI Model", rep.get("model", "—"))]:
        ws.cell(row=r, column=1, value=k).font = BOLD; ws.cell(row=r, column=2, value=v); r += 1
    r += 1; r = section(ws, "Frameworks Applied", r)
    r = bullets(ws, es.get("frameworks", []), r)
    r += 1; r = section(ws, "Purpose", r); r = para(ws, es.get("purpose", ""), r)
    r += 1; r = section(ws, "Scope", r); r = para(ws, es.get("scope", ""), r)
    r += 1; r = section(ws, "Headline", r); r = para(ws, es.get("headline", ""), r)
    r += 1; r = section(ws, "Key Findings", r); r = bullets(ws, es.get("key_findings", []), r)
    widths(ws, {"A": 30, "B": 60, "C": 18, "D": 18, "E": 18, "F": 18})

    # 1b — Intake Audit (Supplied vs Assumed). Gated on intake_summary so older payloads
    # without the audit block render unchanged. Lists every BIA intake field with its
    # status + the explicit assumptions the BIA had to make because the customer left it
    # blank — the operator can hand this sheet to the business owner to close the gaps.
    isum = rep.get("intake_summary") or {}
    assumptions = rep.get("assumptions") or []
    if isum or assumptions:
        ws = wb.create_sheet("Intake — Supplied vs Assumed")
        ws["A1"] = "BIA Intake — Stated vs Assumed"; ws["A1"].font = TITLE
        supplied = isum.get("supplied") or []
        not_supplied = isum.get("not_supplied") or []
        ws["A2"] = (f"{len(supplied)} supplied / {len(supplied) + len(not_supplied)} total "
                    f"({isum.get('supplied_pct', 0)}% supplied). "
                    "Every figure later in this workbook is anchored on the Supplied rows; "
                    "anything labelled '(recommended …)' or 'Not supplied' flows from the "
                    "Assumed rows.")
        ws["A2"].font = SUBTLE; ws["A2"].alignment = WRAP
        ws.merge_cells("A2:D2")
        r = 4
        if assumptions:
            r = section(ws, "Assumptions Used Because the Customer Did Not State the Value", r, span=4)
            r = bullets(ws, assumptions, r, span=4)
            r += 1
        r = section(ws, "Intake Field Audit", r, span=4)
        hdr(ws, ["BIA Intake Field", "Status", "Customer-Stated Value", "Notes"], row=r); r += 1
        for f in supplied:
            ws.cell(row=r, column=1, value=f.get("label")).font = BOLD
            ws.cell(row=r, column=2, value="Supplied")
            val = f.get("value")
        ...
        for f in not_supplied:
            ...
            if kind == "required":
                ...
                note = "Group requires at least ONE supplied (MTD / default RTO / default RPO)."
            elif kind == "impact_signal":
                note = "Group requires at least ONE supplied (downtime cost / revenue at risk / operational impact)."
            ws.cell(row=r, column=4, value=note).alignment = WRAP
            r += 1
        widths(ws, {"A": 44, "B": 26, "C": 38, "D": 40})
    ...
    md = rep.get("methodology", {}) or {}
    ws = wb.create_sheet("Methodology")
    ws["A1"] = "BIA Methodology"; ws["A1"].font = TITLE
    r = 3; r = section(ws, "Approach", r); r = para(ws, md.get("approach", ""), r)
    r += 1; r = section(ws, "Frameworks Applied", r)
    for fa in (md.get("frameworks_applied", []) or []):
        if isinstance(fa, dict):
            r = para(ws, f"• {fa.get('framework','')} — {fa.get('how_applied','')}", r)
    r += 1; r = section(ws, "Data Sources", r); r = bullets(ws, md.get("data_sources", []), r)
    widths(ws, {"A": 30, "B": 40, "C": 20, "D": 20, "E": 20, "F": 20})

    # 3 — Business Services
    ws = wb.create_sheet("Business Services")
    hdr(ws, ["Service / Process", "Business Function", "Criticality Tier", "Supporting Resources", "Users Affected", "Peak Periods"])
    for i, s in enumerate(rep.get("business_services", []), 2):
        ws.cell(row=i, column=1, value=s.get("service")).font = BOLD
        ws.cell(row=i, column=2, value=s.get("business_function")).alignment = WRAP
        ws.cell(row=i, column=3, value=s.get("criticality_tier"))
        ws.cell(row=i, column=4, value=", ".join(s.get("supporting_resources", []) or [])).alignment = WRAP
        ws.cell(row=i, column=5, value=s.get("users_affected"))
        ws.cell(row=i, column=6, value=s.get("peak_periods"))
    widths(ws, {"A": 30, "B": 30, "C": 24, "D": 40, "E": 22, "F": 22})

    # 4 — Criticality Tiers
    ws = wb.create_sheet("Criticality Tiers")
    hdr(ws, ["Tier", "Definition", "MTD", "RTO", "RPO", "Count", "Examples"])
    for i, t in enumerate(rep.get("criticality_tiers", []), 2):
        ws.cell(row=i, column=1, value=t.get("tier")).font = BOLD
        ws.cell(row=i, column=2, value=t.get("definition")).alignment = WRAP
        ws.cell(row=i, column=3, value=t.get("mtd")); ws.cell(row=i, column=4, value=t.get("rto"))
        ws.cell(row=i, column=5, value=t.get("rpo")); ws.cell(row=i, column=6, value=t.get("resource_count"))
        ws.cell(row=i, column=7, value=", ".join(t.get("examples", []) or [])).alignment = WRAP
    widths(ws, {"A": 26, "B": 40, "C": 14, "D": 12, "E": 12, "F": 8, "G": 40})

    # 5 — Impact Over Time
    ws = wb.create_sheet("Impact Over Time")
    hdr(ws, ["Outage Duration", "Financial", "Operational", "Reputational / Customer", "Regulatory / Legal", "Health & Safety"])
    for i, row_ in enumerate(rep.get("impact_over_time", []), 2):
        ws.cell(row=i, column=1, value=row_.get("duration")).font = BOLD
        ws.cell(row=i, column=2, value=row_.get("financial")).alignment = WRAP
        ws.cell(row=i, column=3, value=row_.get("operational")).alignment = WRAP
        ws.cell(row=i, column=4, value=row_.get("reputational")).alignment = WRAP
        ws.cell(row=i, column=5, value=row_.get("regulatory")).alignment = WRAP
        ws.cell(row=i, column=6, value=row_.get("health_safety")).alignment = WRAP
    widths(ws, {"A": 16, "B": 34, "C": 34, "D": 34, "E": 34, "F": 30})

    # 6 — Recovery Objectives
    ws = wb.create_sheet("Recovery Objectives")
    # Top warning row when no customer recovery targets were supplied — keeps the reader
    # honest that 'Recommended' values are not stated SLAs.
    _no_targets = isum and isum.get("any_recovery_target_supplied") is False
    if _no_targets:
        warn = ws.cell(row=1, column=1, value=(
            "WARNING — Customer did not state MTD, default RTO or default RPO at intake. "
            "The 'Current' columns below read 'Not supplied' and the 'Recommended' columns "
            "carry the suffix '(recommended — no customer target)'. Validate with the business "
            "owner before approval."
        ))
        warn.font = Font(bold=True, color="92400E", size=10)
        warn.fill = PatternFill("solid", fgColor="FEF3C7")
        ...
        ws.row_dimensions[1].height = 36
        hdr(ws, ["Workload", "Criticality", "MTD", "Current RTO", "Recommended RTO", "Current RPO", "Recommended RPO", "Rationale"], row=2)
        _start = 3
    else:
        hdr(ws, ["Workload", "Criticality", "MTD", "Current RTO", "Recommended RTO", "Current RPO", "Recommended RPO", "Rationale"])
        _start = 2
    for i, o in enumerate(rep.get("recovery_objectives", []), _start):
        ws.cell(row=i, column=1, value=o.get("workload")).font = BOLD
        ws.cell(row=i, column=2, value=o.get("criticality"))
        ws.cell(row=i, column=3, value=o.get("mtd")); ws.cell(row=i, column=4, value=o.get("current_rto"))
        ws.cell(row=i, column=5, value=o.get("recommended_rto")); ws.cell(row=i, column=6, value=o.get("current_rpo"))
        ws.cell(row=i, column=7, value=o.get("recommended_rpo"))
        ws.cell(row=i, column=8, value=o.get("rationale")).alignment = WRAP
    widths(ws, {"A": 28, "B": 18, "C": 14, "D": 16, "E": 22, "F": 16, "G": 22, "H": 40})

    # 7 — Dependencies & SPOF
    da = rep.get("dependency_analysis", {}) or {}
    ws = wb.create_sheet("Dependencies & SPOF")
    ws["A1"] = "Dependency Analysis"; ws["A1"].font = TITLE
    r = 3; r = section(ws, "Upstream Dependencies", r); r = bullets(ws, da.get("upstream_dependencies", []), r)
    r += 1; r = section(ws, "Downstream Dependencies", r); r = bullets(ws, da.get("downstream_dependencies", []), r)
    r += 1; r = section(ws, "Notes", r); r = para(ws, da.get("notes", ""), r)
    r += 2
    hdr(ws, ["Single Point of Failure", "Why", "Business Impact", "Mitigation"], row=r); r += 1
    for spof in (da.get("single_points_of_failure", []) or []):
        ws.cell(row=r, column=1, value=spof.get("resource")).font = BOLD
        ws.cell(row=r, column=2, value=spof.get("why")).alignment = WRAP
        ws.cell(row=r, column=3, value=spof.get("impact")).alignment = WRAP
        ws.cell(row=r, column=4, value=spof.get("mitigation")).alignment = WRAP
        r += 1
    widths(ws, {"A": 30, "B": 40, "C": 36, "D": 36, "E": 16, "F": 16})

    # 8 — Financial Exposure
    fe = rep.get("financial_exposure", {}) or {}
    ws = wb.create_sheet("Financial Exposure")
    ws["A1"] = "Financial Exposure"; ws["A1"].font = TITLE
    r = 3
    for k, v in [("Per hour", fe.get("per_hour", "—")), ("Per day", fe.get("per_day", "—")),
                 ("Annualized", fe.get("annualized", "—"))]:
        ws.cell(row=r, column=1, value=k).font = BOLD; ws.cell(row=r, column=2, value=v); r += 1
    r += 1; r = section(ws, "Basis", r); r = para(ws, fe.get("basis", ""), r)
    r += 1; r = section(ws, "Most Exposed Services", r); r = bullets(ws, fe.get("most_exposed_services", []), r)
    widths(ws, {"A": 26, "B": 50, "C": 18, "D": 18, "E": 18, "F": 18})

    # 9 — Resource Requirements
    rr = rep.get("resource_requirements", {}) or {}
    ws = wb.create_sheet("Resource Requirements")
    ws["A1"] = "Recovery Resource Requirements"; ws["A1"].font = TITLE
    r = 3; r = section(ws, "Minimum Recovery Resources", r); r = bullets(ws, rr.get("minimum_recovery_resources", []), r)
    r += 1; r = section(ws, "Vital Records / Critical Data", r); r = bullets(ws, rr.get("vital_records", []), r)
    r += 1; r = section(ws, "Staffing Roles", r); r = bullets(ws, rr.get("staffing_roles", []), r)
    r += 1; r = section(ws, "Third-Party Dependencies", r); r = bullets(ws, rr.get("third_party_dependencies", []), r)
    widths(ws, {"A": 30, "B": 40, "C": 20, "D": 20, "E": 20, "F": 20})

    # 10 — Recovery Sequence
    ws = wb.create_sheet("Recovery Sequence")
    hdr(ws, ["Order", "Service / Workload", "Resources", "Target RTO", "Rationale"])
    for i, s in enumerate(rep.get("recovery_sequence", []), 2):
        ws.cell(row=i, column=1, value=s.get("order")).font = BOLD
        ws.cell(row=i, column=2, value=s.get("service"))
        ws.cell(row=i, column=3, value=", ".join(s.get("resources", []) or [])).alignment = WRAP
        ws.cell(row=i, column=4, value=s.get("target_rto"))
        ws.cell(row=i, column=5, value=s.get("rationale")).alignment = WRAP
    widths(ws, {"A": 8, "B": 30, "C": 40, "D": 14, "E": 44})

    # 11 — Gaps & Recommendations
    ws = wb.create_sheet("Gaps & Recommendations")
    hdr(ws, ["Gap", "Business Impact", "Recommendation", "Priority", "Effort"])
    for i, g in enumerate(rep.get("gaps_and_recommendations", []), 2):
        ws.cell(row=i, column=1, value=g.get("gap")).alignment = WRAP
        ws.cell(row=i, column=2, value=g.get("business_impact")).alignment = WRAP
        ws.cell(row=i, column=3, value=g.get("recommendation")).alignment = WRAP
        ws.cell(row=i, column=4, value=g.get("priority")); ws.cell(row=i, column=5, value=g.get("effort"))
    widths(ws, {"A": 36, "B": 34, "C": 44, "D": 10, "E": 10})

    # 12 — Risk Register
    ws = wb.create_sheet("Risk Register")
    hdr(ws, ["Risk", "Probability", "Impact", "Mitigation"])
    for i, rk in enumerate(rep.get("risk_register", []), 2):
        ws.cell(row=i, column=1, value=rk.get("risk")).alignment = WRAP
        ws.cell(row=i, column=2, value=rk.get("probability")); ws.cell(row=i, column=3, value=rk.get("impact"))
        ws.cell(row=i, column=4, value=rk.get("mitigation")).alignment = WRAP
    widths(ws, {"A": 44, "B": 14, "C": 12, "D": 44})

    # 13 — Conclusion
    cc = rep.get("conclusion", {}) or {}
    ws = wb.create_sheet("Conclusion")
    ws["A1"] = "Conclusion & Next Steps"; ws["A1"].font = TITLE
    r = 3; r = section(ws, "Summary", r); r = para(ws, cc.get("summary", ""), r)
    r += 1; r = section(ws, "Immediate Actions", r); r = bullets(ws, cc.get("immediate_actions", []), r)
    r += 1; r = section(ws, "Next Steps", r); r = bullets(ws, cc.get("next_steps", []), r)
    widths(ws, {"A": 30, "B": 40, "C": 20, "D": 20, "E": 20, "F": 20})

    # 14 — Appendix: BIA Matrix (per-resource, deterministic)
    ws = wb.create_sheet("Appendix - BIA Matrix")
    hdr(ws, ["Resource", "Type", "Resource Group", "Location", "Subscription", "BIA Tier",
             "Impact Score", "Criticality Source", "DR Tier", "Target RTO (h)",
             "Monthly $ (actual)", "Est. downtime $/hr (business loss)", "Downtime basis",
             "Business Owner", "Data Class"])
    for i, m in enumerate((rep.get("appendices", {}) or {}).get("bia_matrix", []), 2):
        ws.cell(row=i, column=1, value=m.get("resource_name")).font = BOLD
        ws.cell(row=i, column=2, value=m.get("resource_type"))
        ws.cell(row=i, column=3, value=m.get("resource_group"))
        ws.cell(row=i, column=4, value=m.get("location"))
        ws.cell(row=i, column=5, value=m.get("subscription_name"))
        ws.cell(row=i, column=6, value=m.get("bia_tier"))
        ws.cell(row=i, column=7, value=m.get("impact_score"))
        ws.cell(row=i, column=8, value=m.get("criticality_source"))
        ws.cell(row=i, column=9, value=m.get("dr_tier"))
        ws.cell(row=i, column=10, value=m.get("target_rto_hours"))
        c11 = ws.cell(row=i, column=11, value=m.get("monthly_cost")); c11.number_format = '#,##0.00'
        c12 = ws.cell(row=i, column=12, value=m.get("downtime_cost_hr")); c12.number_format = '#,##0.00'
        ws.cell(row=i, column=13, value=m.get("downtime_cost_source"))
        ws.cell(row=i, column=14, value=m.get("business_owner"))
        ws.cell(row=i, column=15, value=m.get("data_classification"))
    widths(ws, {"A": 30, "B": 18, "C": 20, "D": 14, "E": 20, "F": 20, "G": 12, "H": 16,
                "I": 12, "J": 14, "K": 16, "L": 24, "M": 14, "N": 18, "O": 14})
    ...
    buf = BytesIO(); wb.save(buf); buf.seek(0)
    safe = "".join(c if (c.isalnum() or c in "-_") else "_" for c in cust)
    fname = f"{safe}-Business-Impact-Analysis-{datetime.now(timezone.utc).strftime('%Y%m%d')}.xlsx"
    return StreamingResponse(
        buf,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="{fname}"'},
...
def _ensure_dashboard_in_cache() -> Optional[DashboardData]:
    ...
    try:
        ...
        _cache["data"] = restored
        _cache["cached_at"] = ts
        _hydrate_resource_costs(restored)
        logger.info(
...
def _hydrate_resource_costs(data) -> None:
    """Fill per-resource cost from the cost warehouse when the scan stored 0.

    Azure Cost Management rate-limits the per-resource grain (sustained 429s), so a
    scan typically persists cost_current_month=0 for EVERY resource. That made the
    waste/quadrant/savings home-page charts filter down to zero rows and render an
    endless loading skeleton, and every cost column read $0. Accepts either a
    DashboardData model or a raw snapshot dict; mutates in place so every cache
    reader sees the same corrected resources. Idempotent and cheap.
    """
    if data is None:
        return
    rows = data.get("resources") if isinstance(data, dict) else (getattr(data, "resources", None) or [])
    rows = rows or []
    if not rows:
        return

    is_dict = isinstance(rows[0], dict)
    _get = (lambda r, k: r.get(k)) if is_dict else (lambda r, k: getattr(r, k, None))

    try:
        from services.finops_dashboard_service import _warehouse_resource_costs
        wh = _warehouse_resource_costs() or {}
    except Exception as exc:
        logger.debug("Resource cost hydration unavailable: %s", exc)
        return
    if not wh:
        return
    ...
    filled = 0
    for r in rows:
        # Per-resource rather than an all-or-nothing guard: a background refresh that
        # lands a few live costs must not stop the rest being filled.
        if float(_get(r, "cost_current_month") or 0):
            continue
        cost = wh.get(str(_get(r, "resource_id") or "").lower())
        if not cost:
            continue
        try:
            cost = round(float(cost), 2)
            if is_dict:
                r["cost_current_month"] = cost
            else:
                r.cost_current_month = cost
            filled += 1
            # Savings were derived from the $0 cost at scan time, so recompute them
            # with the SAME estimator rather than leaving the waste/savings charts empty.
            if not float(_get(r, "estimated_monthly_savings") or 0):
                sav, rec = estimate_savings(
                    cost,
                    float(_get(r, "final_score") or 0),
                    bool(_get(r, "is_orphan")),
                    0.0,
                    has_metrics=_get(r, "primary_utilization_pct") is not None,
                )
                if sav > 0:
                    if is_dict:
                        r["estimated_monthly_savings"] = sav
                        if not r.get("recommendation"):
                            r["recommendation"] = rec
                    else:
                        r.estimated_monthly_savings = sav
                        if not getattr(r, "recommendation", None):
                            r.recommendation = rec
        except Exception:
            continue
    if filled:
        logger.info("Hydrated per-resource cost for %d/%d resources from the cost warehouse",
                    filled, len(rows))
        _refresh_cost_kpis(data, rows, _get)
        _rebuild_savings_recommendations(data, rows, _get)
        _refresh_orphan_panel(data, rows, _get)


def _refresh_orphan_panel(data, rows, _get) -> None:
    """Re-point the orphan panel's costs at the hydrated resources.

    Built during the scan like the savings list, so it kept showing "Wasted: $0/mo"
    for orphans that do have a warehouse cost.
    """
    panel = (data.get("orphans") if isinstance(data, dict)
             else getattr(data, "orphans", None)) or []
    if not panel:
        return
    by_id = {}
    for r in rows:
        rid = str(_get(r, "resource_id") or "").lower()
        if rid:
            by_id[rid] = r
    for o in panel:
        o_is_dict = isinstance(o, dict)
        rid = str((o.get("resource_id") if o_is_dict else getattr(o, "resource_id", "")) or "").lower()
        src = by_id.get(rid)
        if not src:
            continue
        cost = float(_get(src, "cost_current_month") or 0)
        sav = float(_get(src, "estimated_monthly_savings") or 0)
        try:
            if o_is_dict:
                o["monthly_cost"] = round(cost, 2)
                o["estimated_savings"] = round(sav, 2)
            else:
                o.monthly_cost = round(cost, 2)
                o.estimated_savings = round(sav, 2)
        except Exception:
...
def _rebuild_savings_recommendations(data, rows, _get) -> None:
    """Re-derive savings_recommendations from the hydrated resources.

    The list is built during the scan from resources whose savings were computed
    against the throttled $0 costs, so it kept only the handful that had a live
    cost and left the home-page Savings panel reading "none found" while FinOps
    listed dozens. Rebuilding here keeps the panel consistent with the resources.
    """
    def _num(r, key):
        return float(_get(r, key) or 0)

    recs = []
    for r in rows:
        sav = _num(r, "estimated_monthly_savings")
        if sav <= 0:
            continue
        cost = _num(r, "cost_current_month")
        score = _num(r, "final_score")
        recs.append({
            "resource_id": _get(r, "resource_id") or "",
            "resource_name": _get(r, "resource_name") or "",
            "resource_type": _get(r, "resource_type") or "",
            "resource_group": _get(r, "resource_group") or "",
            "subscription_id": _get(r, "subscription_id") or "",
            "current_monthly_cost": round(cost, 2),
            "estimated_monthly_savings": round(sav, 2),
            "savings_pct": round(sav / cost * 100, 1) if cost > 0 else 0.0,
            "recommendation": _get(r, "recommendation") or "",
            "ai_explanation": _get(r, "ai_explanation"),
            "ai_action": _get(r, "ai_action"),
            "priority": "High" if score <= 25 else "Medium" if score <= 50 else "Low",
            "score": score,
            "advisor_count": len(_get(r, "advisor_recommendations") or []),
        })
    if not recs:
        return
    ...
    existing = (data.get("savings_recommendations") if isinstance(data, dict)
                else getattr(data, "savings_recommendations", None)) or []
    if len(existing) >= len(recs):
        return          # the scan already produced at least as much — leave it alone
    try:
        if isinstance(data, dict):
            data["savings_recommendations"] = recs
        else:
            from models.schemas import SavingsRecommendation
            data.savings_recommendations = [SavingsRecommendation(**x) for x in recs]
        logger.info("Rebuilt savings_recommendations from hydrated costs: %d → %d",
                    len(existing), len(recs))
    except Exception as exc:
        logger.debug("Could not rebuild savings_recommendations: %s", exc)


def _refresh_cost_kpis(data, rows, _get) -> None:
    """Re-derive the cost/savings KPI aggregates after per-resource costs were hydrated.

    The KPI block is computed during the scan, so it inherited the same $0 costs and
    left the headline savings cards reading $0 next to a non-zero resource count.
    """
    kpi = data.get("kpi") if isinstance(data, dict) else getattr(data, "kpi", None)
    if kpi is None:
        return
    k_is_dict = isinstance(kpi, dict)

    def _num(r, key):
        return float(_get(r, key) or 0)

    totals = {
        "total_cost_current_month": sum(_num(r, "cost_current_month") for r in rows),
        "total_potential_savings": sum(_num(r, "estimated_monthly_savings") for r in rows),
        "orphan_cost": sum(_num(r, "cost_current_month") for r in rows if _get(r, "is_orphan")),
        "not_used_cost": sum(_num(r, "cost_current_month") for r in rows
                             if str(_get(r, "score_label") or "") == "Not Used"),
    }
    for key, val in totals.items():
        # Always realign these to the hydrated resources. Leaving the scan's throttled
        # total in place made savings read as ~2500% of the bill.
        try:
            if k_is_dict:
                kpi[key] = round(val, 2)
            else:
                setattr(kpi, key, round(val, 2))
        except Exception:
            continue
...
def _get_resources_list() -> List[dict]:
    ...
    if not data:
        return []
    return _apply_ai_resource_filter([r.model_dump() for r in (data.resources or [])])
...
async def ai_networking_analysis(refresh: bool = False):
    ...
    # Scope the topology to the SAME resources the model is told it is analysing — reading
    # the cache here would pull in subscriptions the user filtered out.
    sub_ids = sorted({
        s for s in (
            (r.get("subscription_id") if isinstance(r, dict) else getattr(r, "subscription_id", None))
            for r in resources
        ) if s
    }) or None
...
async def licensing_reservation_analysis():
    ...
    # build_reservation_analysis() takes ResourceMetrics objects, not dicts.
    resources = _get_resources_objects()
    if not resources:
        raise HTTPException(status_code=404, detail="No resource data. Run a scan first.")
    return await asyncio.get_event_loop().run_in_executor(
