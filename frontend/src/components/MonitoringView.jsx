...
const SEV_COLOR = { Sev0: '#ef4444', Sev1: '#f97316', Sev2: '#eab308', Sev3: '#38bdf8', Sev4: '#64748b' };
const SEV_LABEL = { Sev0: 'Critical', Sev1: 'Error', Sev2: 'Warning', Sev3: 'Info', Sev4: 'Verbose' };
const HEALTH_COLOR = { Available: '#22c55e', Degraded: '#eab308', Unavailable: '#ef4444', Unknown: '#64748b' };

const card = { background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 16 };
const th = { textAlign: 'left', padding: '8px 10px', color: '#64748b', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.4, borderBottom: '1px solid #1e293b', position: 'sticky', top: 0, background: '#0b1220' };
const td = { padding: '8px 10px', color: '#cbd5e1', fontSize: 12, borderBottom: '1px solid #111a2e' };
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#94a3b8', marginBottom: 3 }}>
        <span>{label}</span>
        <span style={{ color: '#e2e8f0' }}>{value}{total ? ` / ${total}` : ''}{total ? ` · ${pct}%` : ''}</span>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ color: '#e2e8f0', fontSize: 20, fontWeight: 700, margin: 0 }}>Monitoring</h2>
          <p style={{ color: '#64748b', fontSize: 12, margin: '2px 0 0' }}>
            Azure Monitor · performance &amp; health for native, Arc, and on-premises resources
          </p>
        </div>
        <button onClick={loadCore} disabled={loading} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#1e293b', color: '#cbd5e1', border: '1px solid #334155', borderRadius: 8, padding: '7px 12px', fontSize: 12, cursor: loading ? 'default' : 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ ...card, flex: '1 1 320px' }}>
              <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Resource Health</div>
            ...
            <div style={{ ...card, flex: '1 1 320px' }}>
              <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600, marginBottom: 12 }}>Alerts by Severity</div>
      ...
      {tab === 'health' && (
        <DataGrid
          ...
          columns={[
            ...
            { key: 'availability_state', label: 'State', tdStyle: (v) => ({ color: HEALTH_COLOR[v] || '#94a3b8', fontWeight: 600 }) },
            { key: 'resource_group', label: 'Resource Group' },
            { key: 'subscription_id', label: 'Subscription' },
      ...
      {tab === 'alerts' && (
        <DataGrid
          ...
          columns={[
            { key: 'severity', label: 'Severity', csv: (v, row) => `${v} ${row.severity_label}`, render: (v, row) => (<span><span style={{ color: SEV_COLOR[v] || '#94a3b8', fontWeight: 700 }}>{v}</span> <span style={{ color: '#64748b' }}>{row.severity_label}</span></span>) },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {perfLoading && (
            <div style={{ ...card, textAlign: 'center', color: '#94a3b8', padding: 30 }}>
              <div style={{ width: 36, height: 36, border: '3px solid #1e293b', borderTopColor: '#38bdf8', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 12px' }} />
          ...
          {perf && (
            <>
              ...
              <div style={{ ...card, padding: 0 }}>
                <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600, padding: 12 }}>
                ...
                {!perf.la?.available && <div style={{ color: '#94a3b8', fontSize: 12, padding: '0 12px 12px' }}>{perf.la?.note || 'No Log Analytics performance data available.'}</div>}
                {perf.la?.available && (
