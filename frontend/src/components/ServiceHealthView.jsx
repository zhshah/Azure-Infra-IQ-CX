...
function Pill({ text, color }) {
  ...
  return <span style={{ color, background: (color || '#64748b') + '22', borderRadius: 5, padding: '2px 8px', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap' }}>{text}</span>;
}
...
function TimelineChart({ timeline = [], overdue = 0, noDate = 0 }) {
  ...
  if (noDate) bars.push({ label: 'No date', count: noDate, color: '#64748b' });
  if (!bars.length) return <div style={{ color: '#64748b', fontSize: 12 }}>No dated items to plot.</div>;
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, height: 170, paddingTop: 6, overflowX: 'auto' }}>
      {bars.map((b, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', flex: '1 1 0', minWidth: 46 }}>
          <div style={{ color: '#e2e8f0', fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{b.count}</div>
          ...
          <div style={{ color: '#94a3b8', fontSize: 10, marginTop: 6, textAlign: 'center', lineHeight: 1.2 }}>{b.label}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ ...card, borderColor: '#1e3a5f' }}>
        <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          <CalendarClock size={16} style={{ color: '#38bdf8' }} /> Retirements &amp; Deprecations radar
        </div>
        <p style={{ color: '#94a3b8', fontSize: 12, margin: 0, lineHeight: 1.55 }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ ...card, flex: '2 1 420px' }}>
          <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600, marginBottom: 8 }}>Upcoming timeline</div>
          <TimelineChart timeline={d.timeline} overdue={s.overdue} noDate={s.no_date} />
        </div>
        <div style={{ ...card, flex: '1 1 240px' }}>
          <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 600, marginBottom: 10 }}>By category</div>
          {Object.entries(d.by_category || {}).sort((a, b) => b[1] - a[1]).map(([c, n]) => (
            <div key={c} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <span style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700 }}>{n}</span>
            </div>
          ))}
          {Object.keys(d.by_category || {}).length === 0 && <div style={{ color: '#64748b', fontSize: 12 }}>No items.</div>}
        </div>
      </div>
      ...
      <DataGrid
        ...
        columns={[
          ...
          { key: 'deadline', label: 'Deadline', csv: (v, row) => deadlineText(row), render: (v, row) => <span style={{ color: row.days_until != null && row.days_until < 0 ? '#ef4444' : row.days_until != null && row.days_until <= 90 ? '#eab308' : '#cbd5e1', fontWeight: 600 }}>{deadlineText(row)}</span> },
          { key: 'exposed_count', label: 'Exposed', render: (v, row) => (v ? <span title={(row.exposed_resources || []).map((r) => r.resource_name).join(', ')} style={{ color: '#38bdf8', fontWeight: 700 }}>{v} resource{v === 1 ? '' : 's'}</span> : <span style={{ color: '#475569' }}>—</span>) },
      ...
      <div style={{ ...card }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}><Sparkles size={15} style={{ color: '#c4b5fd' }} /> AI briefing</div>
            <p style={{ color: '#94a3b8', fontSize: 12, margin: '2px 0 0', maxWidth: 620 }}>
          ...
          {!showAI && (
            <button onClick={() => setShowAI(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#3730a3', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
        ...
        <>
          ...
          <DataGrid
            ...
            columns={[
              ...
              { key: 'status', label: 'Status', tdStyle: (v) => ({ color: STATUS_COLOR[(v || '').toLowerCase()] || '#94a3b8', fontWeight: 600 }) },
