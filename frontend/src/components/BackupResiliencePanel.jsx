...
// Normalise a backup finding / unprotected resource into the drill drawer's resource shape.
function backupToResource(f) {
  return {
    resource_id: f.resource_id || f.id,
    resource_name: f.resource_name || f.name,
    resource_type: f.resource_type,
    resource_group: f.resource_group,
    location: f.location || f.region,
    subscription_id: f.subscription_id,
    cost_current_month: f.cost_current_month ?? f.cost_usd ?? 0,
    has_backup: false,
  };
}
...
const SEV_COLOR = { critical: "#ef4444", high: "#f97316", medium: "#eab308", low: "var(--c-64748b)" };
...
const CHART_COLORS = [
  ...
  "#34d399", "#22d3ee", "#818cf8", "#f87171", "var(--c-94a3b8)",
];
...
function DonutChart({ data, colorMap, size = 130, label }) {
  ...
  if (!total) return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: "var(--c-1e293b)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--c-475569)", fontSize: 11 }}>No data</div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <svg width={size} height={size}>
        ...
        <circle cx={cx} cy={cy} r={r * 0.55} style={{ fill: 'var(--c-0f172a)' }} />
        <text x={cx} y={cy - 4} textAnchor="middle" dominantBaseline="middle" style={{ fill: 'var(--c-e2e8f0)' }} fontSize={16} fontWeight={800}>{total}</text>
        {label && <text x={cx} y={cy + 12} textAnchor="middle" style={{ fill: 'var(--c-64748b)' }} fontSize={9}>{label}</text>}
      </svg>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center", maxWidth: 220 }}>
        {slices.map(s => (
          <div key={s.k} style={{ display: "flex", alignItems: "center", gap: 4 }}>
            ...
            <span style={{ fontSize: 10, color: "var(--c-94a3b8)" }}>{s.k}</span>
            <span style={{ fontSize: 10, color: "var(--c-e2e8f0)", fontWeight: 600 }}>{s.v}</span>
          </div>
        ))}
      </div>
    </div>
...
function HBarChart({ data, maxBars = 8, colorMap }) {
  ...
  if (!entries.length) return <div style={{ color: "var(--c-475569)", fontSize: 12, padding: 16 }}>No data</div>;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {entries.map(([k, v], i) => (
        <div key={k}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
            <span style={{ fontSize: 11, color: "var(--c-94a3b8)", maxWidth: "70%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{k}</span>
            <span style={{ fontSize: 11, color: "var(--c-e2e8f0)", fontWeight: 600 }}>{v}</span>
          </div>
...
function KPITile({ icon: Icon, label, value, sub, color = "var(--c-e2e8f0)", accent, onClick }) {
  return (
    <div onClick={onClick} style={{
      background: "var(--c-0f172a)", border: `1px solid ${accent || "var(--c-1e293b)"}`,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
        {Icon == null
          ...
          : (typeof Icon === "string" || typeof Icon === "number")
            ...
            : <Icon size={15} style={{ color: color !== "var(--c-e2e8f0)" ? color : "var(--c-64748b)" }} />}
        <span style={{ fontSize: 9, color: "var(--c-64748b)", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4 }}>{label}</span>
      </div>
      <div style={{ fontSize: 24, fontWeight: 800, color, lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 10, color: "var(--c-475569)", marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
function SevBadge({ severity }) {
  const col = SEV_COLOR[severity] || "var(--c-64748b)";
  return (
    <span style={{
...
function FilterBar({ filters, setFilters, severityOptions, categoryOptions }) {
  return (
    <div style={{
      ...
      background: "var(--c-111827)", border: "1px solid rgba(var(--rgb-slate), 0.7)", borderRadius: 10, marginBottom: 14,
    }}>
      <input
        type="text" placeholder="Search resources…" value={filters.search}
        onChange={e => setFilters(f => ({ ...f, search: e.target.value }))}
        style={{
          background: "var(--c-0c1220)", border: "1px solid var(--c-1e293b)", borderRadius: 7,
          color: "var(--c-e2e8f0)", fontSize: 11, padding: "7px 10px", outline: "none",
      ...
      <button onClick={() => setFilters({ search: "", severity: "", category: "" })}
        style={{
          ...
          cursor: "pointer", color: "var(--c-64748b)", padding: "6px 12px", fontSize: 11,
...
function FindingCard({ item, expanded, onToggle, onSelect }) {
  ...
  const col = SEV_COLOR[sev] || "var(--c-64748b)";
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
            ...
            <span style={{ color: "var(--c-f1f5f9)", fontWeight: 700, fontSize: 13 }}>{item.resource_name || item.vault_name || item.title}</span>
            <SevBadge severity={sev} />
            {item.category && (
              <span style={{ fontSize: 9, color: "var(--c-475569)", background: "var(--c-1e293b)", padding: "2px 7px", borderRadius: 10, border: "1px solid var(--c-334155)" }}>
                {item.category}
              </span>
            )}
            {item.finding_type && (
              <span style={{ fontSize: 9, color: 'var(--c-60a5fa)', background: "#1e40af15", padding: "2px 7px", borderRadius: 10, border: "1px solid #1d4ed825" }}>
                {item.finding_type.replace(/_/g, " ")}
              </span>
            )}
          </div>
          <div style={{ color: "var(--c-475569)", fontSize: 10, marginBottom: 4 }}>
            {item.resource_group || item.location || ""}
            {item.resource_type && <span> · {item.resource_type.split("/").pop()}</span>}
            {item.location && <span> · {item.location}</span>}
          </div>
          <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.6 }}>{asText(item.description)}</div>
          {item.last_backup_status && (
            <div style={{ color: "var(--c-475569)", fontSize: 10, marginTop: 4 }}>Last backup: {item.last_backup_status}</div>
          )}
          <button onClick={onToggle} style={{ background: "none", border: "none", color: "var(--c-475569)", cursor: "pointer", fontSize: 10, padding: 0, marginTop: 6, display: "flex", alignItems: "center", gap: 3 }}>
          ...
          {expanded && (
            <div style={{ marginTop: 10, background: "var(--c-1e293b)", borderRadius: 8, padding: "12px 14px", border: "1px solid var(--c-334155)" }}>
              ...
              <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.6 }}>{asText(item.recommendation)}</div>
            </div>
          )}
        </div>
      </div>
...
function ListRow({ item, onExpand }) {
  ...
  return (
    <div onClick={onExpand} style={{
      ...
      gap: 8, padding: "10px 14px", borderBottom: "1px solid var(--c-1e293b)",
      cursor: "pointer", alignItems: "center",
    }}>
      <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 600 }}>
          <ResourceIconImg resourceType={item.resource_type} size={14} />
          {item.resource_name || item.vault_name || item.title}
        </span>
      </div>
      <div style={{ color: "var(--c-64748b)", fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {item.category || item.finding_type || ""}
      </div>
      <div><SevBadge severity={sev} /></div>
      <div style={{ color: "var(--c-475569)", fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {item.location || ""}
      </div>
      <div style={{ color: "var(--c-475569)", fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {item.resource_group || ""}
...
function ViewToggle({ mode, setMode }) {
  const btn = (m, label) => (
    <button onClick={() => setMode(m)} style={{
      ...
      border: `1px solid ${mode === m ? "#3b82f6" : "var(--c-334155)"}`,
      color: mode === m ? "#60a5fa" : "var(--c-64748b)",
...
function TabBar({ active, setActive, tabs, counts }) {
  return (
    <div style={{
      display: "flex", gap: 2, padding: "4px", background: "var(--c-0f172a)",
      borderRadius: 12, border: "1px solid var(--c-1e293b)", marginBottom: 16, flexWrap: "wrap",
    }}>
      {tabs.map(t => {
        ...
        return (
          <button key={t.key} onClick={() => setActive(t.key)} style={{
            background: isActive ? "var(--c-1e293b)" : "transparent",
            border: isActive ? "1px solid var(--c-334155)" : "1px solid transparent",
            borderRadius: 10, padding: "8px 16px", fontSize: 12, cursor: "pointer",
            color: isActive ? "var(--c-e2e8f0)" : "var(--c-64748b)", fontWeight: isActive ? 700 : 500,
            display: "flex", alignItems: "center", gap: 6, transition: "all 0.15s",
          }}>
            {t.label}
            {count != null && count > 0 && (
              <span style={{
                background: isActive ? "#3b82f625" : "var(--c-1e293b)",
                color: isActive ? "#60a5fa" : "var(--c-475569)",
...
function VaultCard({ vault, type }) {
  ...
  return (
    <div style={{
      background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: "14px 16px",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
            ...
            <div style={{ color: "var(--c-f1f5f9)", fontSize: 13, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{vault.name}</div>
          </div>
          <div style={{ color: "var(--c-475569)", fontSize: 10, marginLeft: 26 }}>
            {vault.resourceGroup} · {vault.location}
          </div>
          {vault.sku && <div style={{ color: "var(--c-475569)", fontSize: 10, marginLeft: 26 }}>SKU: {vault.sku}</div>}
        </div>
        <span style={{
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        ...
        {type === "rsv" && (
          <span style={{
            ...
            color: crrEnabled ? "#22c55e" : "var(--c-64748b)",
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
        ...
        <span style={{ color: "var(--c-f1f5f9)", fontSize: 13, fontWeight: 700 }}>{item.resource_name}</span>
      ...
      <div style={{ color: "var(--c-475569)", fontSize: 10, marginBottom: 4 }}>
        Source: <span style={{ color: "var(--c-94a3b8)" }}>{item.source_region}</span>
        <span style={{ margin: "0 6px", color: "var(--c-334155)" }}>→</span>
        Vault: <span style={{ color: "var(--c-94a3b8)" }}>{item.vault_name}</span> ({item.vault_region})
      </div>
      <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.6 }}>{asText(item.description)}</div>
    </div>
  );
...
function ASRRow({ item }) {
  ...
  return (
    <div style={{
      ...
      gap: 8, padding: "10px 14px", borderBottom: "1px solid var(--c-1e293b)", alignItems: "center",
    }}>
      <span style={{ color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.resource_name}</span>
      <span style={{ color: "var(--c-94a3b8)", fontSize: 10 }}>{item.source_region}</span>
      <span style={{ color: "var(--c-94a3b8)", fontSize: 10 }}>{item.target_region}</span>
      <span style={{ fontSize: 9, fontWeight: 600, color: healthCol }}>{item.replication_health}</span>
      <span style={{ fontSize: 9, color: "var(--c-64748b)" }}>{item.protection_state}</span>
...
function ChartCard({ title, children }) {
  return (
    <div style={{
      background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 14,
      padding: "16px 18px", display: "flex", flexDirection: "column", gap: 10,
      overflow: "hidden", minWidth: 0,
    }}>
      <div style={{ color: "var(--c-94a3b8)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4 }}>{title}</div>
...
function Paginator({ page, totalPages, setPage }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 12, alignItems: "center" }}>
      <button disabled={page === 0} onClick={() => setPage(p => p - 1)} style={{ background: "var(--c-1e293b)", border: "1px solid var(--c-334155)", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: page === 0 ? "default" : "pointer", color: page === 0 ? "var(--c-475569)" : "var(--c-e2e8f0)" }}>← Prev</button>
      <span style={{ fontSize: 11, color: "var(--c-64748b)" }}>Page {page + 1} of {totalPages}</span>
      <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} style={{ background: "var(--c-1e293b)", border: "1px solid var(--c-334155)", borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: page >= totalPages - 1 ? "default" : "pointer", color: page >= totalPages - 1 ? "var(--c-475569)" : "var(--c-e2e8f0)" }}>Next →</button>
    </div>
  );
}
...
export default function BackupResiliencePanel({ backupCoverage }) {
  ...
  if (loading) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--c-475569)", background: "var(--c-0f172a)", borderRadius: 16, border: "1px solid var(--c-1e293b)" }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 600, color: "var(--c-94a3b8)", marginBottom: 6 }}>Loading Enhanced Backup Analysis…</div>
  ...
  if (error && !enhanced) {
    return (
      <div style={{ textAlign: "center", padding: "40px 20px", color: "#ef4444", background: "var(--c-0f172a)", borderRadius: 16, border: "1px solid var(--c-1e293b)" }}>
        ...
        <div style={{ fontSize: 12, color: "var(--c-94a3b8)" }}>{error}</div>
        {/* Fallback to basic coverage */}
        {coverage && <div style={{ marginTop: 16, color: "var(--c-64748b)", fontSize: 11 }}>Showing basic backup coverage from last scan below.</div>}
      </div>
    );
  }
    ...
    <div style={{ fontFamily: "inherit" }}>
      ...
      <div style={{
        ...
        <KPITile icon={Monitor} label="Unprotected VMs" value={kpis.vms_not_backed_up || 0}
          ...
          onClick={() => {
            const list = (enhanced?.unprotected_resources || [])
              .filter(r => (r.resource_type || "").toLowerCase().includes("virtualmachines"));
            openResourceDrill("Unprotected VMs (not backed up)", (list.length ? list : (enhanced?.unprotected_resources || [])).map(backupToResource));
          }}
      ...
      {tab === "overview" && (
        <div>
          ...
          <div style={{
            ...
            background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 16,
            padding: "20px 24px", marginBottom: 16,
          }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <CoverageArc pct={kpis.vm_backup_coverage_pct} />
              <div style={{ color: "var(--c-475569)", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>VM Coverage</div>
            </div>
            <div>
              <div style={{ color: "var(--c-f1f5f9)", fontSize: 18, fontWeight: 800, marginBottom: 8 }}>
                Azure Backup & DR Estate
              </div>
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 12 }}>
                <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>Protected Items: </span><span style={{ color: "#22c55e", fontWeight: 700 }}>{kpis.total_protected_items || 0}</span></div>
                <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>Unprotected: </span><span style={{ color: "#ef4444", fontWeight: 700 }}>{kpis.total_unprotected_resources || 0}</span></div>
                <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>SQL DBs: </span><span style={{ color: 'var(--c-60a5fa)', fontWeight: 700 }}>{kpis.total_sql_dbs || 0}</span></div>
                <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>PostgreSQL: </span><span style={{ color: 'var(--c-60a5fa)', fontWeight: 700 }}>{kpis.total_pg_servers || 0}</span></div>
                <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>Cosmos DB: </span><span style={{ color: 'var(--c-60a5fa)', fontWeight: 700 }}>{kpis.total_cosmos_accounts || 0}</span></div>
              </div>
              ...
              {kpis.same_region_backups > 0 && (
                <div style={{
                  ...
                  <div style={{ color: "var(--c-94a3b8)", fontSize: 11, lineHeight: 1.5 }}>
          ...
          {allFindings.length > 0 && (
            <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 14, padding: "16px 18px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <span style={{ color: "var(--c-94a3b8)", fontSize: 11, fontWeight: 700, textTransform: "uppercase" }}>Top Findings</span>
          ...
          {coverage?.categories?.length > 0 && (
            <div style={{ marginTop: 16, background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 14, padding: "16px 18px" }}>
              <div style={{ color: "var(--c-94a3b8)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", marginBottom: 12 }}>Coverage by Resource Category</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 10 }}>
                {coverage.categories.map(cat => {
                  const pct = cat.coverage_pct;
                  const barColor = pct >= 80 ? "#22c55e" : pct >= 50 ? "#eab308" : "#ef4444";
                  return (
                    <div key={cat.category_key} style={{ background: "var(--c-1e293b)", borderRadius: 10, padding: "10px 12px", border: `1px solid ${barColor}20` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 11, color: "var(--c-e2e8f0)", fontWeight: 600 }}>{cat.icon} {cat.category}</span>
                        <span style={{ fontSize: 10, color: barColor, fontWeight: 700 }}>{Math.round(pct)}%</span>
                      </div>
                      <div style={{ height: 4, background: "var(--c-0f172a)", borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ height: "100%", width: `${pct}%`, background: barColor, borderRadius: 2, transition: "width 0.6s" }} />
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4, color: "var(--c-475569)", fontSize: 9 }}>
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <span style={{ fontSize: 11, color: "var(--c-64748b)" }}>{filteredFindings.length} finding(s)</span>
              <button onClick={() => exportCSV(filteredFindings, "backup-findings.csv")}
                style={{ background: "var(--c-1e293b)", border: "1px solid var(--c-334155)", borderRadius: 8, padding: "4px 10px", fontSize: 10, color: "var(--c-94a3b8)", cursor: "pointer" }}>
                Export CSV
              </button>
            </div>
          </div>
          ...
          {filteredFindings.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px", color: "var(--c-475569)", background: "var(--c-0f172a)", borderRadius: 12, border: "1px solid var(--c-1e293b)" }}>
              <div style={{ marginBottom: 8 }}>{React.createElement(CheckCircle, { size: 32, style: { color: "#22c55e" } })}</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: "var(--c-94a3b8)" }}>No findings match current filters</div>
            </div>
          ) : viewMode === "cards" ? (
            ...
            <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, overflow: "hidden" }}>
              {/* Header */}
              <div style={{
                display: "grid", gridTemplateColumns: "2fr 1fr 90px 1fr 1fr",
                gap: 8, padding: "8px 14px", background: "var(--c-1e293b)", borderBottom: "1px solid var(--c-334155)",
              }}>
                {["Resource", "Category", "Severity", "Location", "Resource Group"].map(h => (
                  <span key={h} style={{ fontSize: 9, fontWeight: 700, color: "var(--c-64748b)", textTransform: "uppercase" }}>{h}</span>
                ))}
              </div>
              {pageFindings.map((f, i) => (
          ...
          {expandedId && viewMode === "list" && (() => {
            ...
            return (
              <div style={{
                background: "var(--c-0f172a)", border: "1px solid var(--c-334155)", borderRadius: 12,
                padding: 16, marginTop: 10,
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ color: "var(--c-f1f5f9)", fontSize: 14, fontWeight: 700 }}>{item.resource_name || item.vault_name || item.title}</span>
                  <button onClick={() => setExpandedId(null)} style={{ background: "none", border: "none", color: "var(--c-64748b)", cursor: "pointer", fontSize: 14 }}>✕</button>
                </div>
                <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.6, marginBottom: 10 }}>{asText(item.description)}</div>
                {item.recommendation && (
                  <div style={{ background: "var(--c-1e293b)", borderRadius: 8, padding: 12, border: "1px solid var(--c-334155)" }}>
      ...
      {tab === "vaults" && (
        <div>
          ...
          {/* RSV list */}
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", marginBottom: 8 }}>
      ...
      {tab === "roc" && (
        <div>
          {/* RoC Info Banner */}
          <div style={{
            background: "linear-gradient(135deg, var(--c-1e3a5f), var(--c-0f172a))",
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ color: 'var(--c-60a5fa)', fontSize: 16, fontWeight: 800, marginBottom: 6 }}>
                  Azure Backup — Region of Choice (RoC)
                </div>
                <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.7, marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                  <div style={{ background: "#22c55e15", border: "1px solid #22c55e25", borderRadius: 10, padding: "8px 14px" }}>
                    ...
                    <div style={{ color: "var(--c-e2e8f0)", fontSize: 14, fontWeight: 700 }}>{roc.default_primary_display || "Sweden Central"}</div>
                  </div>
                  <div style={{ background: "#3b82f615", border: "1px solid #3b82f625", borderRadius: 10, padding: "8px 14px" }}>
                    ...
                    <div style={{ color: "var(--c-e2e8f0)", fontSize: 14, fontWeight: 700 }}>{roc.default_secondary_display || "Switzerland North"}</div>
                  </div>
                  <div style={{ background: "#f9731615", border: "1px solid #f9731625", borderRadius: 10, padding: "8px 14px" }}>
                    ...
                    <div style={{ color: "var(--c-e2e8f0)", fontSize: 14, fontWeight: 700 }}>{roc.total_same_region || 0}</div>
                  </div>
                  <div style={{ background: "#ef444415", border: "1px solid #ef444425", borderRadius: 10, padding: "8px 14px" }}>
                    ...
                    <div style={{ color: "var(--c-e2e8f0)", fontSize: 14, fontWeight: 700 }}>{roc.roc_critical_count || 0}</div>
                  </div>
                </div>
          ...
          {enhanced?.roc_findings?.length > 0 ? (
            ...
            <div style={{ textAlign: "center", padding: "40px", color: "#22c55e", background: "var(--c-0f172a)", borderRadius: 12, border: "1px solid var(--c-1e293b)" }}>
              <div style={{ marginBottom: 8 }}>{React.createElement(CheckCircle, { size: 32, style: { color: "#22c55e" } })}</div>
              <div style={{ fontSize: 14, fontWeight: 600 }}>All backups are stored in a different region from source</div>
              <div style={{ fontSize: 11, color: "var(--c-64748b)", marginTop: 4 }}>No Region of Choice warnings detected</div>
            </div>
          )}
        </div>
      )}
      ...
      {tab === "dr" && (
        <div>
          ...
          {/* DR info banner */}
          <div style={{
            background: "var(--c-0f172a)", border: "1px solid #3b82f625", borderRadius: 14,
            padding: "16px 18px", marginBottom: 16,
          }}>
            <div style={{ color: 'var(--c-60a5fa)', fontSize: 12, fontWeight: 700, marginBottom: 6, display: "flex", alignItems: "center", gap: 4 }}>
              {React.createElement(Lightbulb, { size: 12 })} Cross-Region DR with Azure Site Recovery (ASR)
            </div>
            <div style={{ color: "var(--c-94a3b8)", fontSize: 11, lineHeight: 1.7 }}>
              ...
              <strong style={{ color: 'var(--c-60a5fa)' }}> Recovery Services Vault </strong> based replication provides
              ...
              <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, overflow: "hidden" }}>
                ...
                {enhanced.vms_without_dr.slice(0, 50).map((vm, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{vm.resource_name}</span>
                    <span style={{ color: "var(--c-94a3b8)", fontSize: 10 }}>{vm.location}</span>
                    <span style={{ color: "var(--c-64748b)", fontSize: 10 }}>{vm.vm_size}</span>
                    <span style={{ color: "var(--c-64748b)", fontSize: 10 }}>{vm.os_type}</span>
                  </div>
                ))}
                {enhanced.vms_without_dr.length > 50 && (
                  <div style={{ padding: "8px 14px", color: "var(--c-475569)", fontSize: 11 }}>
      ...
      {tab === "ransomware" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {!ransomwareData ? (
            <div style={{ color: "var(--c-64748b)", padding: 24, textAlign: "center" }}>Loading ransomware readiness assessment…</div>
          ) : (
            <>
              {/* Score Header */}
              <div style={{ background: "var(--c-0f172a)", border: `1px solid ${ransomwareData.score >= 80 ? "#22c55e" : ransomwareData.score >= 60 ? "#eab308" : "#ef4444"}30`, borderRadius: 12, padding: "20px 24px", display: "flex", alignItems: "center", gap: 24 }}>
                <div style={{ position: "relative", width: 100, height: 100 }}>
                  <svg width={100} height={100} viewBox="0 0 100 100">
                    <circle cx={50} cy={50} r={42} fill="none" style={{ stroke: 'var(--c-1e293b)' }} strokeWidth={8} />
                    ...
                    <text x={50} y={46} textAnchor="middle" style={{ fill: 'var(--c-f1f5f9)' }} fontSize={22} fontWeight={800}>{ransomwareData.score}</text>
                    <text x={50} y={62} textAnchor="middle" style={{ fill: 'var(--c-64748b)' }} fontSize={10}>/ 100</text>
                  </svg>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 18, fontWeight: 700, color: "var(--c-f1f5f9)", display: "flex", alignItems: "center", gap: 6 }}>{React.createElement(Shield, { size: 18, style: { color: "#3b82f6" } })} Ransomware Readiness</span>
                    <span style={{
                  ...
                  <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.6 }}>{ransomwareData.summary}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {(ransomwareData.checks || []).map((c, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: "var(--c-0f172a)", border: `1px solid ${statusColor}25`, borderRadius: 10, padding: "14px 18px", borderLeft: `3px solid ${statusColor}` }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 16 }}>{c.icon}</span>
                          <span style={{ color: "var(--c-f1f5f9)", fontSize: 13, fontWeight: 700 }}>{c.check}</span>
                          <span style={{ fontSize: 12 }}>{statusIcon}</span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 60, height: 5, background: "var(--c-1e293b)", borderRadius: 3, overflow: "hidden" }}>
                      ...
                      <div style={{ color: "var(--c-94a3b8)", fontSize: 11, lineHeight: 1.5, marginBottom: 4 }}>{c.description}</div>
                      <div style={{ color: "var(--c-64748b)", fontSize: 11 }}>{c.detail}</div>
                    </div>
                  );
                })}
              </div>
              ...
              {(ransomwareData.recommendations || []).length > 0 && (
                <div style={{ background: "var(--c-0f172a)", border: "1px solid #ef444425", borderRadius: 10, padding: 16 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {ransomwareData.recommendations.map((r, i) => (
                      <div key={i} style={{ display: "flex", gap: 8, fontSize: 12, color: "var(--c-e2e8f0)", alignItems: "flex-start" }}>
      ...
      {tab === "rpo_rto" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {!rpoRtoData ? (
            <div style={{ color: "var(--c-64748b)", padding: 24, textAlign: "center" }}>Loading RPO/RTO compliance matrix…</div>
          ) : (
            <>
              {/* KPI Strip */}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[
                  { label: "Assessed", value: rpoRtoData.total_assessed, color: "var(--c-f1f5f9)" },
                  ...
                  <div key={i} style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 10, padding: "10px 14px", minWidth: 100, flex: 1 }}>
                    <div style={{ color: "var(--c-475569)", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>{k.label}</div>
              ...
              <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: 16 }}>
                <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", marginBottom: 12 }}>RPO/RTO Targets by Business Tier</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                  {(rpoRtoData.tier_summary || []).map((t, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: "var(--c-1e293b)", borderRadius: 8, padding: 12, borderLeft: `3px solid ${compColor}` }}>
                        <div style={{ color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 700, marginBottom: 4 }}>{t.label}</div>
                        <div style={{ display: "flex", gap: 12, marginBottom: 6 }}>
                          <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>RPO: </span><span style={{ color: 'var(--c-38bdf8)', fontSize: 12, fontWeight: 600 }}>{t.target_rpo}</span></div>
                          <div><span style={{ color: "var(--c-64748b)", fontSize: 10 }}>RTO: </span><span style={{ color: 'var(--c-a78bfa)', fontSize: 12, fontWeight: 600 }}>{t.target_rto}</span></div>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          <span style={{ color: "var(--c-94a3b8)", fontSize: 11 }}>{t.total} resources</span>
                          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            <div style={{ width: 40, height: 4, background: "var(--c-0f172a)", borderRadius: 2, overflow: "hidden" }}>
                ...
                <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: 16 }}>
                  ...
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                      <thead>
                        <tr style={{ borderBottom: "1px solid var(--c-1e293b)" }}>
                          {["Resource", "Category", "Tier", "Target RPO", "Target RTO", "Current Backup", "Severity"].map((h, i) => (
                            <th key={i} style={{ padding: "6px 8px", textAlign: "left", color: "var(--c-475569)", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {rpoRtoData.entries.filter(e => e.compliance_status !== "compliant").slice(0, 50).map((e, i) => {
                          const sevColor = SEV_COLOR[e.gap_severity] || "var(--c-64748b)";
                          return (
                            <tr key={i} style={{ borderBottom: "1px solid #1e293b08" }}>
                              <td style={{ padding: "6px 8px" }}>
                                <div style={{ color: "var(--c-f1f5f9)", fontWeight: 600 }}>{e.resource_name}</div>
                                <div style={{ color: "var(--c-475569)", fontSize: 10 }}>{e.resource_group}</div>
                              </td>
                              <td style={{ padding: "6px 8px", color: "var(--c-94a3b8)" }}>{e.category}</td>
                              <td style={{ padding: "6px 8px" }}>
                                <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--c-38bdf8)', textTransform: "capitalize" }}>{e.business_tier}</span>
                              </td>
                              <td style={{ padding: "6px 8px", color: 'var(--c-38bdf8)' }}>{e.target_rpo_hours}h</td>
                              <td style={{ padding: "6px 8px", color: 'var(--c-a78bfa)' }}>{e.target_rto_hours}h</td>
                              <td style={{ padding: "6px 8px", color: "var(--c-94a3b8)" }}>{e.backup_method}</td>
                              <td style={{ padding: "6px 8px" }}>
          ...
          {enhanced?.job_failures?.length > 0 && (
            <div style={{ gridColumn: "1 / -1" }}>
              <ChartCard title={`Failed / Warning Backup Jobs (${enhanced.job_failures.length})`}>
                <div style={{ background: "var(--c-0f172a)", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{
                  ...
                  {enhanced.job_failures.slice(0, 20).map((j, i) => {
                    ...
                    return (
                      <div key={i} style={{
                        display: "grid", gridTemplateColumns: "2fr 1fr 80px 1fr 1fr",
                        gap: 8, padding: "8px 14px", borderBottom: "1px solid var(--c-1e293b)", alignItems: "center",
                      }}>
                        <span style={{ color: "var(--c-f1f5f9)", fontSize: 11, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{j.entity_name}</span>
                        <span style={{ color: "var(--c-94a3b8)", fontSize: 10 }}>{j.operation}</span>
                        <span style={{ color: statusCol, fontSize: 10, fontWeight: 600 }}>{j.status}</span>
                        <span style={{ color: "var(--c-64748b)", fontSize: 10 }}>{j.start_time?.slice(0, 16)}</span>
                        <span style={{ color: "var(--c-64748b)", fontSize: 10 }}>{j.backup_type}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {tab === "executive" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <button onClick={handleExcelDownload} style={{ padding: "8px 16px", background: "#22c55e", color: "var(--c-0f172a)", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              ⬇ Download Excel Report
            </button>
          </div>
          {!execSummary ? <div style={{ color: "var(--c-64748b)", padding: 24 }}>Loading executive summary…</div> : (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
                {Object.entries(execSummary.kpis || {}).map(([k, v]) => (
                  <div key={k} style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 10, padding: 14 }}>
                    <div style={{ fontSize: 9, color: "var(--c-64748b)", textTransform: "uppercase", fontWeight: 700 }}>{k.replace(/_/g, " ")}</div>
                    <div style={{ fontSize: 22, fontWeight: 800, color: "var(--c-e2e8f0)", marginTop: 4 }}>{typeof v === "number" ? (v % 1 ? `${v.toFixed(1)}%` : v) : v}</div>
                  </div>
                ))}
              </div>
              {execSummary.key_findings?.length > 0 && (
                <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 10, padding: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-f1f5f9)", marginBottom: 10 }}>Key Findings</div>
                  {execSummary.key_findings.map((f, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                      <span style={{ color: SEV_COLOR[f.severity] || "var(--c-64748b)", fontWeight: 700, fontSize: 11 }}>●</span>
                      <span style={{ fontSize: 11, color: "var(--c-cbd5e1)" }}>{f.text}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      {tab === "timeline" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {!timeline ? <div style={{ color: "var(--c-64748b)", padding: 24 }}>Loading timeline…</div> : (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {(cat.items || []).map((item, j) => (
                    <div key={j} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                      <span style={{ fontSize: 12, color: item.required ? "#ef4444" : "var(--c-64748b)", flexShrink: 0 }}>{item.required ? "◉" : "○"}</span>
                      <div>
                        <div style={{ fontSize: 11, color: "var(--c-e2e8f0)", fontWeight: 600 }}>{item.item}</div>
                        {item.description && <div style={{ fontSize: 10, color: "var(--c-64748b)" }}>{asText(item.description)}</div>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {!strategyRef ? <div style={{ color: "var(--c-64748b)", padding: 24 }}>Loading strategy reference…</div> : (
            <>
              ...
              {strategyRef.decision_matrix && (
                <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 10, padding: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-f1f5f9)", marginBottom: 10 }}>Decision Matrix</div>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                      <thead>
                        <tr>
                          {Object.keys(strategyRef.decision_matrix[0] || {}).map(h => (
                            <th key={h} style={{ textAlign: "left", padding: "6px 10px", color: "var(--c-64748b)", borderBottom: "1px solid var(--c-1e293b)", fontWeight: 700, fontSize: 10, textTransform: "uppercase" }}>{h.replace(/_/g, " ")}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
