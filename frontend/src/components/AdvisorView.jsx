...
const IMPACT_COLOR = { High: '#ef4444', Medium: '#f97316', Low: 'var(--c-64748b)' };
...
function AdvisorView() {
  ...
  const FilterBar = data ? (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
      <span style={{ color: 'var(--c-94a3b8)', fontSize: 12, marginRight: 2 }}>Recommendation type:</span>
      {['all', ...cats].map((c) => {
        const active = catFilter === c;
        const color = c === 'all' ? '#38bdf8' : (CAT_COLOR[c] || 'var(--c-64748b)');
        const count = c === 'all' ? (d.total || 0) : (d.by_category?.[c] || 0);
        return (
          <button key={c} onClick={() => setCatFilter(c)} style={{
            fontSize: 12, padding: '5px 12px', borderRadius: 16, cursor: 'pointer',
            background: active ? color + '22' : 'var(--c-0f172a)', color: active ? color : 'var(--c-94a3b8)',
            border: `1px solid ${active ? color : 'var(--c-1e293b)'}`, fontWeight: active ? 700 : 500,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {tab === 'recommendations' && data && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ ...card }}>
            <div style={{ color: 'var(--c-e2e8f0)', fontSize: 13, fontWeight: 600, marginBottom: 12 }}>By Category</div>
            {Object.entries(d.by_category || {}).map(([k, v]) => (
              <Bar key={k} label={catLabel(k)} value={v} total={d.total || 0} color={CAT_COLOR[k] || 'var(--c-64748b)'} />
            ))}
          </div>
          <DataGrid
            ...
            maxHeight="52vh"
            columns={[
              { key: 'impact', label: 'Impact', tdStyle: (v) => ({ color: IMPACT_COLOR[v] || 'var(--c-94a3b8)', fontWeight: 600 }) },
      ...
      {tab === 'ai' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {catFilter !== 'all' && (
            <div style={{ fontSize: 12, color: 'var(--c-38bdf8)', background: '#0c4a6e22', border: '1px solid var(--c-0c4a6e)', borderRadius: 8, padding: '7px 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
