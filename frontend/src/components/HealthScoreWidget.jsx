...
function ScoreDonut({ score, grade, size = 100 }) {
  const color = GRADE_COLOR[grade] || "var(--c-64748b)";
  const r = 38, circ = 2 * Math.PI * r;
  const offset = circ - (circ * Math.min(score, 100)) / 100;
  return (
    <svg width={size} height={size} viewBox="0 0 100 100">
      <circle cx="50" cy="50" r={r} fill="none" style={{ stroke: 'var(--c-1e293b)' }} strokeWidth="8" />
      ...
      <text x="50" y="60" textAnchor="middle" style={{ fill: 'var(--c-64748b)' }} fontSize="10" fontWeight="600">{grade}</text>
    </svg>
  );
}
...
export default function HealthScoreWidget() {
  ...
  const color = GRADE_COLOR[data.grade] || "var(--c-64748b)";

  return (
    <div style={{
      background: "var(--c-0d1117)", border: `1px solid ${color}30`, borderRadius: 14,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            <span style={{ color: "var(--c-f1f5f9)", fontWeight: 700, fontSize: 14 }}>Infrastructure Health</span>
          </div>
      ...
      {data.breakdown && Object.keys(data.breakdown).length > 0 && (
        <div style={{ marginTop: 10 }}>
          <button onClick={() => setExpanded(!expanded)} style={{
            background: "none", border: "none", color: "var(--c-475569)", fontSize: 10, cursor: "pointer",
            ...
            <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
              {Object.entries(data.breakdown).map(([key, val]) => (
                <div key={key} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "var(--c-94a3b8)", fontSize: 10, minWidth: 100 }}>{DIM_LABEL[key] || key}</span>
                  <div style={{ flex: 1, height: 4, background: "var(--c-1e293b)", borderRadius: 2, overflow: "hidden" }}>
                    <div style={{
                      width: `${val}%`, height: "100%", borderRadius: 2,
                      background: val >= 80 ? "#22c55e" : val >= 60 ? "#eab308" : "#ef4444",
                    }} />
                  </div>
                  <span style={{ color: "var(--c-e2e8f0)", fontSize: 10, fontWeight: 700, minWidth: 28, textAlign: "right" }}>{val}%</span>
                </div>
              ))}
            </div>
          )}
