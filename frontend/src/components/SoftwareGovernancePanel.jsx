...
const EOL_COLOR  = { 'end-of-life': '#ef4444', 'approaching-eol': '#f59e0b', supported: '#22c55e', unknown: '#64748b' }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        ...
        <button onClick={loadFleet} disabled={fleetLoading} style={btn('#0c1f33', '#1d4ed8', '#60a5fa')}>
          <RefreshCw size={12} className={fleetLoading ? 'animate-spin' : ''} /> {fleetLoading ? 'Scanning fleet…' : 'Scan fleet'}
        </button>
      </div>

      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid #1e293b', paddingBottom: 8, flexWrap: 'wrap' }}>
        {TABS.map(t => {
          const Icon = t.icon
          return (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer',
              background: tab === t.key ? '#2563eb' : 'transparent', color: tab === t.key ? '#fff' : '#94a3b8', border: 'none',
            }}><Icon size={14} /> {t.label}</button>
          )
        })}
      </div>

      {error && <div style={{ background: '#1a0e0e', border: '1px solid #7f1d1d', borderRadius: 8, padding: '8px 12px', color: '#fca5a5', fontSize: 12 }}>{error}</div>}
      ...
      {tab === 'overview' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {!fleet?.has_data ? (
            <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 24, textAlign: 'center' }}>
              <Boxes size={30} style={{ color: '#334155', marginBottom: 8 }} />
              <div style={{ color: '#e2e8f0', fontSize: 14, fontWeight: 600 }}>No software inventory yet</div>
              <p style={{ color: '#64748b', fontSize: 12, maxWidth: 470, margin: '6px auto 12px' }}>
              ...
              <button onClick={loadFleet} style={{ ...btn('#0d2b1f', '#166534', '#4ade80'), display: 'inline-flex' }}>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 12 }}>
                ...
                <KPI label="Unique software" value={sum.unique_software} sub="across the fleet" color="#94a3b8" />
              </div>
              ...
              {fleet.category_breakdown && Object.keys(fleet.category_breakdown).length > 0 && (
                <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
                  <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Software by category</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {Object.entries(fleet.category_breakdown).sort((a, b) => b[1] - a[1]).map(([c, n]) => (
                      <button key={c} onClick={() => { setCatFilter(c); setTab('catalog') }} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>
                        <CatBadge category={c} />
                        <span style={{ color: '#cbd5e1', fontSize: 12, fontWeight: 700 }}>{n}</span>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
                <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Servers needing attention</div>
                    ...
                    <tbody>
                      {(fleet.per_server || []).filter(p => !p.no_data).slice(0, 12).map((p, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                          <td style={{ padding: '6px 8px', color: '#e2e8f0', fontWeight: 600 }}>{p.hostname}</td>
                          <td style={{ padding: '6px 8px', color: '#64748b' }}>{p.os || '—'}</td>
                          <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{p.app_count}</td>
                          <td style={{ padding: '6px 8px', color: p.blocked.length ? '#ef4444' : '#475569' }}>{p.blocked.length ? p.blocked.join(', ') : '—'}</td>
                          <td style={{ padding: '6px 8px', color: p.missing_required.length ? '#f59e0b' : '#475569' }}>{p.missing_required.length ? p.missing_required.join(', ') : '—'}</td>
                      ...
                      {(fleet.per_server || []).filter(p => !p.no_data).length === 0 && (
                        <tr><td colSpan={6} style={{ padding: 10, color: '#64748b', fontSize: 12 }}>No scored servers yet.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
        ...
        <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12, alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
              <Search size={12} style={{ position: 'absolute', left: 8, top: 9, color: '#475569' }} />
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search software / publisher…"
                style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 6, color: '#cbd5e1', fontSize: 12, padding: '6px 8px 6px 26px', width: 220 }} />
            </div>
            ...
            <span style={{ color: '#64748b', fontSize: 12, marginLeft: 'auto' }}>{filteredCatalog.length} of {catalog.length}</span>
          </div>
          {catalog.length === 0 ? (
            <div style={{ color: '#64748b', fontSize: 12, padding: 10 }}>No software catalog yet — run a fleet scan from the Overview tab.</div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead><tr>
                  {['Software', 'Publisher', 'Category', 'Risk', 'License', 'EOL', 'Servers', 'Flags'].map(h => (
                    <th key={h} style={{ textAlign: 'left', color: '#475569', padding: '6px 8px', borderBottom: '1px solid #1e293b', position: 'sticky', top: 0, background: '#111827', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {filteredCatalog.map((c, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                      <td style={{ padding: '6px 8px', color: '#e2e8f0', fontWeight: 600 }}>{c.name}</td>
                      <td style={{ padding: '6px 8px', color: '#64748b' }}>{c.publisher || '—'}</td>
                      ...
                      <td style={{ padding: '6px 8px', color: c.license === 'commercial' ? '#c4b5fd' : '#94a3b8' }}>{c.license}</td>
                      <td style={{ padding: '6px 8px' }}><span style={{ color: EOL_COLOR[c.eol_status] || '#64748b' }}>{c.eol_status}</span></td>
                      <td style={{ padding: '6px 8px', color: '#60a5fa', fontWeight: 700 }} title={(c.servers || []).join(', ')}>{c.server_count}</td>
      ...
      {tab === 'policy' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {templates.length > 0 && (
            <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
              <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, marginBottom: 4 }}>Policy templates</div>
              <p style={{ color: '#64748b', fontSize: 11, margin: '0 0 10px' }}>One-click starting points — load, review, then Save.</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 10 }}>
                {templates.map(t => (
                  <div key={t.key} style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: 12 }}>
                    <div style={{ color: '#e2e8f0', fontSize: 12, fontWeight: 700, marginBottom: 4 }}>{t.label}</div>
                    <p style={{ color: '#64748b', fontSize: 11, lineHeight: 1.5, margin: '0 0 10px', minHeight: 48 }}>{t.description}</p>
                    <button onClick={() => applyTemplate(t)} style={btn('#0c1f33', '#1d4ed8', '#60a5fa')}><Sparkles size={12} /> Load template</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
              <span style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}><ListChecks size={15} /> Software Policy</span>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                {savedMsg && <span style={{ color: '#4ade80', fontSize: 11 }}>{savedMsg}</span>}
                <button onClick={loadPolicy} disabled={loading} style={btn('#1e293b', '#334155', '#94a3b8')}><RefreshCw size={12} className={loading ? 'animate-spin' : ''} /> Reload</button>
                <button onClick={savePolicy} disabled={saving} style={btn('#0c1f33', '#1d4ed8', '#60a5fa')}><Save size={12} /> {saving ? 'Saving…' : 'Save & re-scan'}</button>
              </div>
            </div>
              ...
              {[
                ...
                <div key={c.key}>
                  ...
                  <textarea value={(policy[c.key] || []).join('\n')} onChange={editList(c.key)} rows={7} placeholder="One per line…"
                    style={{ width: '100%', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 6, color: '#cbd5e1', fontSize: 12, padding: 8, fontFamily: 'monospace', resize: 'vertical' }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
            <span style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700 }}>Ad-hoc check (paste a machine’s software)</span>
            <p style={{ color: '#64748b', fontSize: 11, margin: '2px 0 8px' }}>Test the policy against any list without scanning — one app per line.</p>
            <textarea value={inventoryText} onChange={e => setInventoryText(e.target.value)} rows={5}
              style={{ width: '100%', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 6, color: '#cbd5e1', fontSize: 12, padding: 8, fontFamily: 'monospace', resize: 'vertical' }} />
            <button onClick={evaluate} disabled={evaluating} style={{ ...btn('#0d2b1f', '#166534', '#4ade80'), marginTop: 10 }}>
            ...
            {evaluation && (
              <div style={{ marginTop: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: '#64748b', fontSize: 12 }}>{evaluation.total_installed} apps</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 12 }}>
                  ...
                  <Findings title="Unauthorized" color="#94a3b8" icon={AlertTriangle} items={(evaluation.unauthorized || []).map(u => u.name)} empty="None" />
                </div>
              </div>
            )}
          </div>
      ...
      {tab === 'ai' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
            <div>
              <div style={{ color: '#e2e8f0', fontSize: 13, fontWeight: 700 }}>AI software intelligence</div>
              <p style={{ color: '#64748b', fontSize: 11, margin: '2px 0 0', maxWidth: 520 }}>
            ...
            <button onClick={runAI} disabled={aiBusy} style={btn('#1e1b4b', '#6d28d9', '#c4b5fd')}><Brain size={13} /> {aiBusy ? 'Analysing…' : 'Analyse fleet software'}</button>
          </div>
          {aiResult && (
            <div style={{ background: 'linear-gradient(180deg,#0c1322,#0a0f1a)', border: '1px solid #1e3a5f', borderRadius: 10, padding: 16 }}>
              <div style={{ color: '#c4b5fd', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                <Brain size={15} /> AI Software Intelligence
                <span style={{ fontSize: 9, color: '#64748b', background: '#0f172a', border: '1px solid #1e293b', borderRadius: 4, padding: '1px 6px' }}>{aiResult.provider}</span>
              </div>
              {aiResult.summary && <p style={{ color: '#cbd5e1', fontSize: 12, lineHeight: 1.5, marginTop: 0 }}>{aiResult.summary}</p>}
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    ...
                    <tbody>
                      {aiResult.items.map((it, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                          <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{it.name}</td>
                          <td style={{ padding: '6px 8px' }}><span style={{ color: EOL_COLOR[it.eol_status] || '#64748b' }}>{it.eol_status}</span></td>
                          <td style={{ padding: '6px 8px' }}><span style={{ color: RISK_COLOR[it.security_risk] || '#64748b' }}>{it.security_risk}</span></td>
                          <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{it.license_note || '—'}</td>
                          <td style={{ padding: '6px 8px', color: '#60a5fa' }}>{it.azure_target || '—'}</td>
                          <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{it.recommendation || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
  ...
  return (
    <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: 12 }}>
      ...
      {items.length === 0
        ...
        : <ul style={{ margin: 0, paddingLeft: 16, color: '#cbd5e1', fontSize: 11, lineHeight: 1.6 }}>{items.map((it, i) => <li key={i}>{it}</li>)}</ul>}
    </div>
  )
}

const sel = { background: '#0f172a', border: '1px solid #1e293b', borderRadius: 6, color: '#cbd5e1', fontSize: 12, padding: '6px 8px', cursor: 'pointer' }
...
const CAT_META = {
  ...
  'runtime':          { label: 'Runtime',            color: '#94a3b8' },
  'browser':          { label: 'Browser',            color: '#94a3b8' },
  'dev-tool':         { label: 'Dev tool',           color: '#a78bfa' },
  'productivity':     { label: 'Productivity',       color: '#94a3b8' },
  'utility':          { label: 'Utility',            color: '#94a3b8' },
  'other':            { label: 'Other',              color: '#64748b' },
}

function CatBadge({ category }) {
  const m = CAT_META[category] || { label: category, color: '#64748b' }
...
function KPI({ label, value, sub, color }) {
  return (
    <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 14, borderLeft: `3px solid ${color}` }}>
...
function WhyItMatters({ open, onToggle }) {
  return (
    <div style={{ background: 'linear-gradient(180deg,#0c1322,#0b1018)', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
      <button onClick={onToggle} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: 'none', cursor: 'pointer', color: '#e2e8f0', fontSize: 13, fontWeight: 700, padding: 0 }}>
      ...
      {open && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12, marginTop: 12 }}>
          {WHY_PILLARS.map((p, i) => {
            const Icon = p.icon
            return (
              <div key={i} style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                  <Icon size={15} style={{ color: p.color }} />
                  <span style={{ color: '#e2e8f0', fontSize: 12, fontWeight: 700 }}>{p.title}</span>
                </div>
                <p style={{ color: '#94a3b8', fontSize: 11, lineHeight: 1.55, margin: 0 }}>{p.body}</p>
              </div>
            )
          })}
        </div>
