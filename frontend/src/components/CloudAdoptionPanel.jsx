...
const IMPACT_COLOR = { high: "#22c55e", medium: "#3b82f6", low: "#64748b" };
  ...
  return (
    <div style={{
      ...
      <svg viewBox={`0 0 ${svgW} ${svgH}`} width="100%" style={{ maxHeight: 450, overflow: "visible" }}>
        {/* Header */}
        <text x={padding.left} y={16} fill="#475569" fontSize="9" fontWeight="600">SERVICE CATEGORY</text>
        <text x={svgW - padding.right + 5} y={16} fill="#475569" fontSize="9" fontWeight="600">COVERAGE</text>
        ...
        {sorted.map((cat, i) => {
          ...
          return (
            <g key={cat.category_key}>
              {/* Label */}
              <text x={padding.left - 8} y={y + barH / 2 + 4} fill="#94a3b8" fontSize="10" textAnchor="end" fontWeight="500">
                {cat.category}
              </text>
              {/* Background bar */}
              <rect x={padding.left} y={y} width={maxBarW} height={barH} rx={4} fill="#1e293b" />
              {/* Progress bar */}
              ...
              {pct < 100 && (
                <text x={padding.left + barW + 8} y={y + barH / 2 + 3} fill="#64748b" fontSize="9">
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap" }}>
        <svg width={160} height={160} viewBox="0 0 160 160">
          ...
          <text x={cx} y={cy - 4} textAnchor="middle" fill="#f1f5f9" fontSize="18" fontWeight="800">
            {totalGaps}
          </text>
          <text x={cx} y={cy + 12} textAnchor="middle" fill="#64748b" fontSize="9" fontWeight="600">
            OPPORTUNITIES
          </text>
        </svg>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
          {totals.map(t => (
            <div key={t.label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 600 }}>{t.label}</div>
      ...
      <div style={{ display: "flex", gap: 24, flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <div style={{
          ...
          <svg width={80} height={80} viewBox="0 0 80 80">
            ...
            <text x={40} y={52} textAnchor="middle" fill="#64748b" fontSize="8" fontWeight="600">
              ADOPTED
            </text>
          </svg>
          <div style={{ color: "#64748b", fontSize: 9, fontWeight: 700, textTransform: "uppercase", marginTop: 4 }}>
        ...
        <div style={{
          ...
          <div style={{ color: "#64748b", fontSize: 10, fontWeight: 700, textTransform: "uppercase", marginTop: 2 }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
            ...
            <span style={{ color: "#475569", fontSize: 14 }}>modernization opportunities across {categories.length} categories</span>
            <span style={{
          ...
          <div style={{ marginTop: 14, color: "#475569", fontSize: 11, lineHeight: 1.6 }}>
            Each opportunity identifies an Azure managed service that can <strong style={{ color: "#94a3b8" }}>replace manual infrastructure</strong>,
    ...
    <div style={{
      ...
      {cards.map(card => (
        <div key={card.title} style={{
          background: "#0f172a", border: `1px solid ${card.color}20`,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div>
              <div style={{ color: "#f1f5f9", fontSize: 13, fontWeight: 700 }}>{card.title}</div>
              <div style={{ color: "#475569", fontSize: 10 }}>{card.subtitle}</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
            <div>
              ...
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Opportunities</div>
            </div>
            <div>
              <div style={{ color: "#22c55e", fontSize: 22, fontWeight: 800 }}>
                ${Math.round(card.monthlySavings).toLocaleString()}
              </div>
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>/mo savings</div>
            </div>
          </div>
    ...
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(175px, 1fr))", gap: 10 }}>
        {categories.map((cat) => {
          ...
          const impactColor = IMPACT_COLOR[cat.acr_impact] || "#64748b";
          const theme = ADOPTION_THEME[cat.category_key] || {};
          return (
            <div
            ...
            >
              ...
              <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600, marginBottom: 2, lineHeight: 1.3 }}>
                {cat.category}
              </div>
              {theme.theme && (
                <div style={{ color: "#475569", fontSize: 9, marginBottom: 6 }}>{theme.theme}</div>
              )}
              {/* Progress bar */}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}>
                ...
                <span style={{ color: "#475569" }}>{cat.gaps} gap{cat.gaps !== 1 ? "s" : ""}</span>
              </div>
...
function AdoptionCard({ gap }) {
  ...
  const impactColor = IMPACT_COLOR[gap.acr_impact] || "#64748b";
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, color: "#f1f5f9", fontWeight: 700, fontSize: 14 }}>
          ...
          <div style={{ color: "#475569", fontSize: 10, marginBottom: 6 }}>
            ...
            {gap.resource_monthly_cost > 0 && (
              <>
                <span style={{ color: "#334155", margin: "0 5px" }}>·</span>
                <span style={{ color: "#64748b" }}>Current: ${Math.round(gap.resource_monthly_cost).toLocaleString()}/mo</span>
              </>
            )}
          </div>

          {/* Description */}
          <div style={{ color: "#94a3b8", fontSize: 12, lineHeight: 1.6, marginBottom: 8 }}>
            {gap.description}
          </div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
            ...
            {gap.estimated_monthly_acr > 0 && (
              <span style={{
                background: "#14532d18", color: "#4ade80",
            ...
            {theme.approach && (
              <span style={{
                ...
                border: "1px solid #334155", fontStyle: "italic",
              }}>{theme.approach}</span>
            )}
          </div>
          ...
          {open && (
            <div style={{ marginTop: 10, background: "#1e293b", borderRadius: 10, padding: "14px 16px", border: "1px solid #334155" }}>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <ol style={{ margin: 0, paddingLeft: 18 }}>
                    {gap.implementation_steps.map((step, i) => (
                      <li key={i} style={{ color: "#94a3b8", fontSize: 12, lineHeight: 1.7, marginBottom: 2 }}>
                        {step}
                      </li>
                    ))}
                  </ol>
              ...
              {gap.az_cli_snippet && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ color: "#60a5fa", fontSize: 11, fontWeight: 600, marginBottom: 6 }}>
                    Azure CLI
                  </div>
                  <pre style={{
...
function AIAdoptionAnalysis({ data, loading, error, onRun }) {
  if (!data && !loading) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px", background: "#0f172a", borderRadius: 16, border: "1px solid #1e293b" }}>
        <div style={{ fontSize: 48, marginBottom: 12, color: '#60a5fa' }}>⬢</div>
        <div style={{ color: "#f1f5f9", fontSize: 16, fontWeight: 700, marginBottom: 8 }}>
          AI-Powered Cloud Adoption Analysis
        </div>
        <div style={{ color: "#64748b", fontSize: 12, marginBottom: 20, maxWidth: 500, margin: "0 auto 20px" }}>
        ...
        {error && (
          <div style={{ background: "#7f1d1d20", border: "1px solid #dc262630", borderRadius: 8, padding: "8px 14px", marginBottom: 16, color: "#fca5a5", fontSize: 11 }}>
            {error}
          </div>
        )}
  ...
  if (loading) {
    return (
      <div style={{ textAlign: "center", padding: "60px 20px", background: "#0f172a", borderRadius: 16, border: "1px solid #1e293b" }}>
        <div style={{ fontSize: 36, marginBottom: 12, animation: "pulse 2s infinite", color: '#60a5fa' }}>○</div>
        <div style={{ color: "#94a3b8", fontSize: 14, fontWeight: 600 }}>Analyzing your environment...</div>
        <div style={{ color: "#475569", fontSize: 11, marginTop: 6 }}>AI is evaluating migration paths and modernization opportunities</div>
      </div>
    );
  }
  ...
  const maturityColor = { "Cloud-Native": "#22c55e", "Advanced": "#3b82f6", "Intermediate": "#eab308", "Beginner": "#ef4444" }[maturity_level] || "#64748b";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ color: "#f1f5f9", fontSize: 14, fontWeight: 700 }}>AI Cloud Adoption Assessment</div>
            <div style={{ color: "#475569", fontSize: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "auto auto 1fr", gap: 20, background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: "20px 24px" }}>
        <div style={{ textAlign: "center" }}>
          <svg width={90} height={90} viewBox="0 0 90 90">
            ...
            <text x={45} y={57} textAnchor="middle" fill="#64748b" fontSize="8">SCORE</text>
          </svg>
        </div>
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 6 }}>
          <div style={{ color: "#64748b", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>Maturity Level</div>
          <div style={{ color: maturityColor, fontSize: 18, fontWeight: 800 }}>{maturity_level || "—"}</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ color: "#64748b", fontSize: 9, fontWeight: 700, textTransform: "uppercase", marginBottom: 6 }}>Executive Summary</div>
          <div style={{ color: "#94a3b8", fontSize: 12, lineHeight: 1.6 }}>{executive_summary}</div>
        </div>
      </div>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Current IaaS/mo</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#22c55e", fontSize: 22, fontWeight: 800 }}>${(cost_comparison.projected_monthly_paas || 0).toLocaleString()}</div>
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Projected PaaS/mo</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#3b82f6", fontSize: 22, fontWeight: 800 }}>{cost_comparison.projected_savings_pct || 0}%</div>
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Cost Reduction</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#8b5cf6", fontSize: 22, fontWeight: 800 }}>{cost_comparison.operational_hours_saved_monthly || 0}h</div>
              <div style={{ color: "#64748b", fontSize: 9, fontWeight: 600, textTransform: "uppercase" }}>Ops Hours Saved/mo</div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ color: "#64748b", fontSize: 10 }}>Current (IaaS-heavy)</span>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ color: "#64748b", fontSize: 10 }}>Projected (Modernized)</span>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {iaas_to_paas_opportunities.map((opp, i) => {
              const approachColor = { "Rehost": "#64748b", "Refactor": "#3b82f6", "Rearchitect": "#8b5cf6", "Replace": "#06b6d4" }[opp.migration_approach] || "#64748b";
              return (
                <div key={i} style={{ background: "#1e293b", borderRadius: 12, padding: "14px 16px", border: "1px solid #334155" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
                    <span style={{ color: "#f1f5f9", fontSize: 13, fontWeight: 700 }}>{opp.current_service}</span>
                  ...
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                    {(opp.benefits || []).map((b, j) => (
                      <span key={j} style={{ background: "#0f172a", color: "#94a3b8", fontSize: 9, padding: "2px 7px", borderRadius: 8, border: "1px solid #334155" }}>{b}</span>
                    ))}
                    {opp.effort_weeks && (
                      <span style={{ background: "#0f172a", color: "#eab308", fontSize: 9, padding: "2px 7px", borderRadius: 8, border: "1px solid #eab30825" }}>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12 }}>
            {migration_waves.map((wave, i) => {
              ...
              return (
                <div key={i} style={{ background: "#1e293b", borderRadius: 12, padding: "14px", borderLeft: `3px solid ${color}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <span style={{ color, fontSize: 12, fontWeight: 700 }}>Wave {wave.wave}</span>
                    <span style={{ color: "#475569", fontSize: 9 }}>{wave.timeframe}</span>
                  </div>
                  <div style={{ color: "#e2e8f0", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>{wave.theme}</div>
                  {(wave.actions || []).map((a, j) => (
                    <div key={j} style={{ color: "#94a3b8", fontSize: 10, lineHeight: 1.6 }}>• {a}</div>
                  ))}
            ...
            {cloud_native_gaps.map((gap, i) => (
              <div key={i} style={{ background: "#1e293b", borderRadius: 10, padding: "12px 14px", border: "1px solid #334155" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                  <span style={{ color: "#f1f5f9", fontSize: 12, fontWeight: 600 }}>{gap.gap}</span>
                ...
                <div style={{ color: "#64748b", fontSize: 11, marginBottom: 4 }}>{gap.impact}</div>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 10 }}>
            {modernization_recommendations.map((rec, i) => {
              const prioColor = { P1: "#ef4444", P2: "#f97316", P3: "#eab308" }[rec.priority] || "#64748b";
              return (
                <div key={i} style={{ background: "#1e293b", borderRadius: 10, padding: "12px 14px", border: `1px solid ${prioColor}20`, borderLeft: `3px solid ${prioColor}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    <span style={{ color: prioColor, fontSize: 9, fontWeight: 700 }}>{rec.priority}</span>
                    <span style={{ color: "#94a3b8", fontSize: 9 }}>{rec.category}</span>
                    {rec.monthly_value > 0 && <span style={{ color: "#22c55e", fontSize: 9, fontWeight: 700, marginLeft: "auto" }}>${rec.monthly_value}/mo</span>}
                  </div>
                  <div style={{ color: "#e2e8f0", fontSize: 12, fontWeight: 600, marginBottom: 4 }}>{rec.title}</div>
                  <div style={{ color: "#64748b", fontSize: 11, lineHeight: 1.5 }}>{rec.description}</div>
  ...
  if (!opps) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 40, marginBottom: 12, color: '#60a5fa' }}>☁</div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid #1e293b", paddingBottom: 8 }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
            ...
            border: activeTab === t.key ? "1px solid #334155" : "1px solid transparent",
            borderRadius: 8, padding: "6px 14px", color: activeTab === t.key ? "#f1f5f9" : "#64748b",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
              {[
                ...
                <div key={s.phase} style={{
                  background: "#1e293b", borderRadius: 10, padding: "14px",
      ...
      {activeTab === "findings" && (
        <>
          ...
          <div style={{
            ...
            border: "1px solid #1e293b", borderRadius: 12, marginBottom: 16,
          }}>
            {/* Severity */}
            <div style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ color: "#475569", fontSize: 10, fontWeight: 700, textTransform: "uppercase", marginRight: 4 }}>Priority:</span>
              {[["all","All","#64748b"],["critical","Critical","#ef4444"],["high","High","#f97316"],["medium","Medium","#eab308"],["low","Low","#64748b"]].map(([v,l,c]) => (
                <button key={v} onClick={() => setFilterSev(v)} style={{
                  background: filterSev === v ? `${c}20` : "transparent",
                  border: `1px solid ${filterSev === v ? c : "#334155"}`,
                  color: filterSev === v ? c : "#64748b",
            ...
            {/* Impact */}
            <div style={{ display: "flex", gap: 4, alignItems: "center", borderLeft: "1px solid #1e293b", paddingLeft: 10 }}>
              <span style={{ color: "#475569", fontSize: 10, fontWeight: 700, textTransform: "uppercase", marginRight: 4 }}>Impact:</span>
              {[["all","All"],["high","High"],["medium","Medium"],["low","Low"]].map(([v,l]) => {
                const c = IMPACT_COLOR[v] || "#64748b";
                return (
                  <button key={v} onClick={() => setFilterImpact(v)} style={{
                    background: filterImpact === v ? `${c}20` : "transparent",
                    border: `1px solid ${filterImpact === v ? c : "#334155"}`,
                    color: filterImpact === v ? c : "#64748b",
                    borderRadius: 20, padding: "2px 10px", fontSize: 11,
            ...
            {filterCat && (
              <button onClick={() => setFilterCat(null)} style={{
                background: "#1e3a5f18", color: "#60a5fa",
          ...
          <div style={{ color: "#475569", fontSize: 11, marginBottom: 12 }}>
            ...
            {filterCat && <span style={{ color: "#60a5fa", marginLeft: 6 }}>
