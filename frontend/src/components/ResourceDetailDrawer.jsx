  ...
  return (
    ...
    >
      ...
      {count !== undefined && count > 0 && (
        <span style={{
          background: active ? "#3b82f633" : "#1e293b",
...
function KVRow({ label, value }) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, padding: "5px 0", borderBottom: "1px solid #1e293b" }}>
      <span style={{ color: "#64748b", fontSize: 12, minWidth: 160, flexShrink: 0 }}>{label}</span>
      <span style={{ color: "#e2e8f0", fontSize: 12, wordBreak: "break-all" }}>{String(value)}</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        background: "#0f172a", borderRadius: 10, padding: "14px 18px", marginBottom: 16,
      }}>
        <div style={{
        ...
        <div>
          <div style={{ color: "#f1f5f9", fontWeight: 700, fontSize: 15 }}>{r.resource_name}</div>
          <div style={{ color: "#94a3b8", fontSize: 12 }}>{r.resource_type}</div>
          <div style={{ color: "#64748b", fontSize: 12 }}>{r.location} · {r.resource_group}</div>
        </div>
      ...
      <div style={{ background: "#111827", borderRadius: 8, padding: "12px 16px", marginBottom: 12 }}>
        <div style={{ color: "#64748b", fontSize: 11, fontWeight: 700, marginBottom: 8, textTransform: "uppercase", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {!metricsError && !metricsLoading && !metrics && (
          <div style={{ color: "#64748b", fontSize: 12 }}>No live metrics available for this resource type.</div>
        )}
      </div>

      {/* Key attributes */}
      <div style={{ background: "#111827", borderRadius: 8, padding: "12px 16px", marginBottom: 12 }}>
        <div style={{ color: "#64748b", fontSize: 11, fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>Resource Details</div>
...
function SecurityTab({ gaps, defenderFindings, portalUrl }) {
  ...
  if (!allFindings.length) {
    return <p style={{ color: "#64748b", fontSize: 13, padding: "20px 0" }}>No security findings for this resource.</p>;
  }
  return (
    <div>
      {allFindings.map((f, i) => (
...
function BCDRTab({ assessment }) {
  if (!assessment) {
    return <p style={{ color: "#64748b", fontSize: 13, padding: "20px 0" }}>No BCDR assessment available for this resource.</p>;
  }
  const a = assessment;
  return (
    <div>
      <div style={{ background: "#0f172a", borderRadius: 8, padding: "12px 16px", marginBottom: 12 }}>
      ...
      {a.sa_bcdr_guidance_summary && (
        <div style={{ background: "#111827", borderRadius: 8, padding: "12px 16px", marginBottom: 12 }}>
          <div style={{ color: "#64748b", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>BCDR Guidance</div>
          <p style={{ color: "#cbd5e1", fontSize: 13, margin: 0 }}>{a.sa_bcdr_guidance_summary}</p>
        </div>
      )}
      {a.sa_current_gap_summary && (
        <div style={{ background: "#1e1b2e", border: "1px solid #f97316aa", borderRadius: 8, padding: "12px 16px" }}>
          <div style={{ color: "#fb923c", fontSize: 11, fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>Current Gap</div>
          <p style={{ color: "#fed7aa", fontSize: 13, margin: 0 }}>{a.sa_current_gap_summary}</p>
        </div>
      )}
    </div>
  );
...
function AdvisorTab({ recommendations }) {
  if (!recommendations?.length) {
    return <p style={{ color: "#64748b", fontSize: 13, padding: "20px 0" }}>No Advisor recommendations for this resource.</p>;
  }
      ...
      {recommendations.map((rec, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{ color: "#f1f5f9", fontWeight: 600, fontSize: 13, marginBottom: detail ? 4 : 0, lineHeight: 1.4 }}>{title}</div>
            {detail && <p style={{ color: "#cbd5e1", fontSize: 12, margin: "2px 0 8px", lineHeight: 1.5 }}>✅ {detail}</p>}
            {rec.portal_url && (
...
function AIFindingsTab({ findings }) {
  if (!findings?.length) {
    return <p style={{ color: "#64748b", fontSize: 13, padding: "20px 0" }}>No cached AI findings reference this resource.</p>;
  }
  const typeLabel = {
      ...
      {findings.map((f, i) => (
        <div key={i} style={{
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
            ...
            {f.severity && <Badge text={f.severity} color={SEVERITY_COLOR[f.severity?.toLowerCase()] || "#94a3b8"} />}
            <span style={{ color: "#f1f5f9", fontWeight: 600, fontSize: 13 }}>{f.title}</span>
          </div>
          {f.detail && <p style={{ color: "#94a3b8", fontSize: 12, margin: "4px 0" }}>{f.detail}</p>}
          {f.recommendation && (
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: "#111827", overflowX: "auto",
        ...
        <div style={{ padding: "16px 20px", flex: 1 }}>
          ...
          {error && (
            <div style={{ color: "#ef4444", background: "#1e0a0a", borderRadius: 8, padding: 16, fontSize: 13 }}>
