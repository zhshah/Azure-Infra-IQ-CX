...
function AdoptionHeatmap({ scores = [] }) {
  ...
  return (
    <div>
      <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 10 }}>
        Service Adoption Scorecard
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 6 }}>
        {scores.map((s, i) => {
          const bg    = s.adopted ? "var(--c-052e16)" : s.partial ? "var(--c-1c1a05)" : "var(--c-1a0a0a)";
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ color: "var(--c-e2e8f0)", fontSize: 11, fontWeight: 600, marginBottom: 3, lineHeight: 1.3 }}>
                {s.category}
              </div>
              <div style={{
...
function GapCard({ gap }) {
  ...
  const impColor  = IMPACT_COLOR[gap.business_impact]  || "var(--c-64748b)";
  const effColor  = EFFORT_COLOR[gap.estimated_effort] || "var(--c-64748b)";
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
            <span style={{ color: "var(--c-f1f5f9)", fontWeight: 700, fontSize: 14 }}>{gap.category}</span>
            <span style={{
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 6 }}>
            ...
            {gap.azure_services.length > 4 && (
              <span style={{ color: "var(--c-475569)", fontSize: 10, padding: "2px 4px" }}>
            ...
            <>
              ...
              {open && (
                <div style={{
                  ...
                  background: "var(--c-0c1a2e)", borderRadius: 8,
                  color: "var(--c-64748b)", fontSize: 12, lineHeight: 1.7,
...
export default function InnovationGapPanel({ innovationGaps = [], serviceAdoptionScores = [] }) {
  ...
  return (
    <div style={{ background: "var(--c-0d1117)", border: "1px solid var(--c-1e293b)", borderRadius: 16, padding: "20px 24px" }}>
      ...
      {/* Tab bar */}
      <div style={{ display: "flex", gap: 4, background: "var(--c-0f172a)", borderRadius: 8, padding: 4, width: "fit-content", marginBottom: 14, border: "1px solid var(--c-1e293b)" }}>
        {[{ id: "gaps", label: "🔍 Gaps" }, { id: "scorecard", label: "📊 Adoption Scorecard" }].map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
            background: activeTab === t.id ? "var(--c-1e293b)" : "none",
            border: "none", color: activeTab === t.id ? "var(--c-f1f5f9)" : "var(--c-64748b)",
      ...
      {activeTab === "scorecard" ? (
        ...
        <>
          {/* Impact filter */}
          <div style={{ display: "flex", gap: 4, background: "var(--c-0f172a)", borderRadius: 8, padding: 4, width: "fit-content", marginBottom: 12, border: "1px solid var(--c-1e293b)" }}>
            {FILTER_OPTIONS.map(f => (
              <button key={f} onClick={() => setImpactFilter(f)} style={{
                background: impactFilter === f ? "var(--c-1e293b)" : "none",
                border: "none",
                color: impactFilter === f ? (IMPACT_COLOR[f] || "var(--c-f1f5f9)") : "var(--c-64748b)",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {filtered.length === 0 && (
              <div style={{ color: "var(--c-475569)", fontSize: 13, padding: "16px 0", textAlign: "center" }}>
