...
const FIVE_R = {
  ...
  Retain:      { color: "var(--c-64748b)", Icon: Pin, desc: "Keep as-is" },
};
...
// ── Helpers ──────────────────────────────────────────────────────────────────
function KpiTile({ label, value, sub, color = "var(--c-f1f5f9)" }) {
  return (
    <div style={{
      background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)",
      borderRadius: 10, padding: "12px 16px", minWidth: 130, flex: 1,
    }}>
      <div style={{ color: "var(--c-475569)", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 4 }}>{label}</div>
...
function RiskBar({ score }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ width: 40, height: 5, background: "var(--c-1e293b)", borderRadius: 3, overflow: "hidden" }}>
...
function DonutSmall({ pct, color, size = 48 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={size/2} cy={size/2} r={r} fill="none" style={{ stroke: 'var(--c-1e293b)' }} strokeWidth={4} />
...
function MigrationSteps({ steps = [] }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 8 }}>Migration Steps</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {steps.map((step, i) => {
          const pc = PHASE_COLOR[step.phase] || "var(--c-64748b)";
          return (
            <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 22 }}>
                ...
                {i < steps.length - 1 && <div style={{ width: 1, flex: 1, minHeight: 6, background: "var(--c-1e293b)", marginTop: 2 }} />}
              </div>
              <div style={{ flex: 1, paddingBottom: 2 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 1 }}>
                  ...
                  {step.effort_days > 0 && <span style={{ fontSize: 9, color: "var(--c-475569)" }}>{step.effort_days}d</span>}
                </div>
                <div style={{ color: "var(--c-e2e8f0)", fontSize: 11, fontWeight: 600 }}>{step.title}</div>
                {step.detail && <div style={{ color: "var(--c-64748b)", fontSize: 10, lineHeight: 1.5 }}>{step.detail}</div>}
                {step.az_cli && (
                  <div style={{ background: "var(--c-020617)", border: "1px solid var(--c-1e293b)", borderRadius: 6, padding: "4px 8px", marginTop: 3, fontFamily: "monospace", fontSize: 9, color: 'var(--c-38bdf8)', whiteSpace: "pre-wrap", overflowX: "auto" }}>
                    {step.az_cli}
                  </div>
                )}
              </div>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
      <svg width={120} height={120} viewBox="0 0 120 120">
        {summary.map((r, i) => {
          ...
          cumPct += pct;
          const col = FIVE_R[r.category]?.color || "var(--c-64748b)";
          return <circle key={i} cx={60} cy={60} r={50} fill="none" stroke={col} strokeWidth={14}
            strokeDasharray={`${dash} ${circ - dash}`} strokeDashoffset={offset} />;
        })}
        <text x={60} y={56} textAnchor="middle" style={{ fill: 'var(--c-f1f5f9)' }} fontSize={18} fontWeight={800}>{total}</text>
        <text x={60} y={72} textAnchor="middle" style={{ fill: 'var(--c-475569)' }} fontSize={9}>resources</text>
      </svg>
        ...
        {summary.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
              ...
              <span style={{ color: "var(--c-e2e8f0)", fontWeight: 600, minWidth: 70 }}>{r.category}</span>
              <span style={{ color: "var(--c-64748b)" }}>{r.count}</span>
...
function OpportunityRow({ opp, onClick }) {
  ...
  const waveColor = WAVE_COLOR[opp.migration_wave] || "var(--c-64748b)";
  return (
    <tr onClick={onClick} style={{ cursor: "pointer", borderBottom: "1px solid var(--c-1e293b)" }}
    ...
    >
      <td style={{ padding: "8px 10px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 600 }}>{opp.resource_name}</div>
            <div style={{ color: "var(--c-475569)", fontSize: 10 }}>{opp.resource_group}</div>
          </div>
        </div>
      </td>
      ...
      <td style={{ padding: "8px 6px" }}>
        <span style={{ color: "var(--c-94a3b8)", fontSize: 11 }}>{opp.current_config?.split("(")[0]?.trim()}</span>
      </td>
      <td style={{ padding: "8px 6px" }}>
        <span style={{ color: 'var(--c-38bdf8)', fontSize: 11 }}>{opp.target_service}</span>
      </td>
      <td style={{ padding: "8px 6px" }}>
        <Badge label={`W${opp.migration_wave}`} color={waveColor} />
      </td>
      <td style={{ padding: "8px 6px" }}>
        <Badge label={opp.complexity} color={COMPLEXITY_COLOR[opp.complexity] || "var(--c-64748b)"} />
      </td>
      <td style={{ padding: "8px 6px", textAlign: "right" }}>
        <div style={{ color: "var(--c-f1f5f9)", fontSize: 12, fontWeight: 600 }}>${Math.round(opp.monthly_cost).toLocaleString()}</div>
      </td>
      <td style={{ padding: "8px 6px", textAlign: "right" }}>
      ...
      <td style={{ padding: "8px 6px", textAlign: "center" }}>
        <span style={{ color: "var(--c-475569)", fontSize: 11 }}>{opp.estimated_effort_days || "—"}d</span>
      </td>
    </tr>
  );
}
...
function DetailPanel({ opp, onClose }) {
  ...
  return (
    <div style={{
      ...
      background: "var(--c-0d1117)", borderLeft: "1px solid var(--c-1e293b)", zIndex: 1000,
      overflowY: "auto", padding: "20px 24px",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h3 style={{ color: "var(--c-f1f5f9)", margin: 0, fontSize: 16, fontWeight: 700 }}>{opp.resource_name}</h3>
        <button onClick={onClose} style={{ background: "none", border: "none", color: "var(--c-64748b)", cursor: "pointer", fontSize: 18 }}>✕</button>
      </div>

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
        <Badge label={opp.five_r || "Retain"} color={fiveR.color} />
        <Badge label={opp.complexity} color={COMPLEXITY_COLOR[opp.complexity] || "var(--c-64748b)"} />
        <Badge label={`Wave ${opp.migration_wave}`} color={WAVE_COLOR[opp.migration_wave] || "var(--c-64748b)"} />
        <Badge label={opp.migration_category || "compute"} color="var(--c-64748b)" />
      </div>

      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
        <KpiTile label="Current Cost" value={`$${Math.round(opp.monthly_cost).toLocaleString()}`} sub="/month" color="var(--c-f1f5f9)" />
        ...
        <KpiTile label="Effort" value={`${opp.estimated_effort_days || 0}d`} color="var(--c-94a3b8)" />
      </div>

      <div style={{ marginBottom: 14 }}>
        <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>Current → Target</div>
        <div style={{ background: "var(--c-1e293b)", borderRadius: 8, padding: "10px 14px", marginBottom: 6 }}>
          <div style={{ color: "var(--c-94a3b8)", fontSize: 12 }}>{opp.current_config}</div>
        </div>
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 6 }}>
          <span style={{ color: "var(--c-475569)", fontSize: 16 }}>↓</span>
        </div>
        <div style={{ background: "var(--c-0c1a2e)", border: "1px solid #1e40af60", borderRadius: 8, padding: "10px 14px" }}>
          <div style={{ color: 'var(--c-38bdf8)', fontSize: 12, fontWeight: 600 }}>{opp.target_service}</div>
        </div>
      </div>

      <div style={{ marginBottom: 14 }}>
        <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>Rationale</div>
        <div style={{ color: "var(--c-94a3b8)", fontSize: 12, lineHeight: 1.7 }}>{opp.reason}</div>
      </div>

      {opp.benefits?.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>Benefits</div>
...
export default function MigrationDashboard({ legacyOpps = [] }) {
  ...
  if (loading) {
    return (
      <div style={{ background: "var(--c-0d1117)", border: "1px solid var(--c-1e293b)", borderRadius: 16, padding: "40px 24px", textAlign: "center" }}>
        <div style={{ color: 'var(--c-38bdf8)', fontSize: 24, marginBottom: 8 }}>🔄</div>
        <div style={{ color: "var(--c-94a3b8)", fontSize: 14 }}>Analysing migration opportunities...</div>
        <div style={{ color: "var(--c-475569)", fontSize: 11, marginTop: 4 }}>Evaluating 12 migration patterns with 5R classification</div>
      </div>
    );
  }
  ...
  if (!data || opps.length === 0) {
    return (
      <div style={{ background: "var(--c-0d1117)", border: "1px solid var(--c-1e293b)", borderRadius: 16, padding: "40px 24px", textAlign: "center" }}>
        ...
        <div style={{ color: "var(--c-475569)", fontSize: 12, marginTop: 4 }}>Your estate is already well-optimised or using modern Azure services</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "var(--c-0d1117)", border: "1px solid var(--c-1e293b)", borderRadius: 16, padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {VIEW_MODES.map(m => (
            <button key={m.id} onClick={() => setViewMode(m.id)} style={{
              background: viewMode === m.id ? "var(--c-1e293b)" : "transparent",
              border: `1px solid ${viewMode === m.id ? "var(--c-334155)" : "var(--c-1e293b)"}`,
              color: viewMode === m.id ? "var(--c-f1f5f9)" : "var(--c-475569)",
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <KpiTile label="Est. Effort" value={`${data.total_effort_days || opps.reduce((s, o) => s + (o.estimated_effort_days || 0), 0)}d`} color="var(--c-94a3b8)" />
        {data.iaas_pct > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 10, padding: "8px 14px", minWidth: 130 }}>
            <DonutSmall pct={data.paas_pct} color="#38bdf8" />
            <div>
              <div style={{ color: "var(--c-475569)", fontSize: 9, fontWeight: 700, textTransform: "uppercase" }}>PaaS Ratio</div>
              <div style={{ color: "var(--c-f1f5f9)", fontSize: 11 }}>{Math.round(data.paas_pct)}% PaaS · {Math.round(data.iaas_pct)}% IaaS</div>
            </div>
          </div>
        )}
      </div>
      ...
      {viewMode === "overview" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          {(data.five_r_summary || []).length > 0 && (
            <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 12 }}>5R Classification</div>
          ...
          {(data.category_summary || []).length > 0 && (
            <div style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 12 }}>By Category</div>
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                          <span style={{ color: "var(--c-e2e8f0)", fontSize: 12, fontWeight: 600, textTransform: "capitalize" }}>{c.category.replace("_", " ")}</span>
                          <span style={{ color: "#22c55e", fontSize: 11, fontWeight: 600 }}>${Math.round(c.potential_savings).toLocaleString()}/mo</span>
                        </div>
                        <div style={{ height: 4, background: "var(--c-1e293b)", borderRadius: 2 }}>
                          <div style={{ width: `${pct}%`, height: "100%", background: "#38bdf8", borderRadius: 2 }} />
                        </div>
                        <div style={{ color: "var(--c-475569)", fontSize: 10, marginTop: 1 }}>{c.count} opportunities · ${Math.round(c.total_cost).toLocaleString()}/mo current</div>
                      </div>
                    </div>
                  );
                })}
          ...
          {/* Wave Summary */}
          <div style={{ gridColumn: "1 / -1", background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 12, padding: "16px 20px" }}>
            <div style={{ color: "var(--c-64748b)", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 12 }}>Migration Waves</div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {(data.wave_groups || []).map((wg, i) => {
                const wc = WAVE_COLOR[wg.wave] || "var(--c-64748b)";
                return (
                  <div key={i} style={{ flex: 1, minWidth: 160, background: `${wc}08`, border: `1px solid ${wc}25`, borderRadius: 10, padding: "14px 16px", cursor: "pointer" }}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      ...
                      <span style={{ color: "var(--c-f1f5f9)", fontSize: 22, fontWeight: 800 }}>{wg.total_resources}</span>
                    </div>
                    <div style={{ color: "var(--c-64748b)", fontSize: 10, marginBottom: 6 }}>{wg.description}</div>
                    <div style={{ display: "flex", gap: 10 }}>
                      <div><span style={{ color: "#22c55e", fontSize: 12, fontWeight: 700 }}>${Math.round(wg.total_savings).toLocaleString()}</span><span style={{ color: "var(--c-475569)", fontSize: 10 }}>/mo</span></div>
                      <div><span style={{ color: "var(--c-94a3b8)", fontSize: 12, fontWeight: 600 }}>{wg.total_effort_days}d</span><span style={{ color: "var(--c-475569)", fontSize: 10 }}> effort</span></div>
                    </div>
                  </div>
                );
              })}
              ...
              <div key={i}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, background: `${wc}08`, border: `1px solid ${wc}30`, borderRadius: 10, padding: "10px 14px", marginBottom: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: "var(--c-64748b)", fontSize: 11 }}>{wg.description}</div>
                  </div>
                  <div style={{ display: "flex", gap: 16, flexShrink: 0 }}>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "#22c55e", fontWeight: 700, fontSize: 14 }}>${Math.round(wg.total_savings).toLocaleString()}/mo</div>
                      <div style={{ color: "var(--c-475569)", fontSize: 10 }}>savings</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "var(--c-f1f5f9)", fontWeight: 600, fontSize: 14 }}>{wg.total_resources}</div>
                      <div style={{ color: "var(--c-475569)", fontSize: 10 }}>items</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "var(--c-94a3b8)", fontWeight: 600, fontSize: 14 }}>~{wg.total_effort_days}d</div>
                      <div style={{ color: "var(--c-475569)", fontSize: 10 }}>effort</div>
                    </div>
                  </div>
                </div>
                  ...
                  {(wg.items || []).map((o, j) => {
                    ...
                    return (
                      <div key={j} onClick={() => setSelected(o)} style={{
                        ...
                        padding: "10px 14px", cursor: "pointer", borderLeft: `3px solid ${COMPLEXITY_COLOR[o.complexity] || "var(--c-64748b)"}`,
                      }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          <ResourceIconImg resourceType={o.resource_type} size={16} />
                          <span style={{ color: "var(--c-f1f5f9)", fontSize: 13, fontWeight: 600 }}>{o.resource_name}</span>
                          <Badge label={o.five_r || "Retain"} color={fR.color} />
                          <Badge label={o.complexity} color={COMPLEXITY_COLOR[o.complexity] || "var(--c-64748b)"} />
                          <RiskBar score={o.risk_score || 0} />
                        </div>
                        <div style={{ display: "flex", gap: 12, fontSize: 11, color: "var(--c-64748b)" }}>
                          <span>{o.current_config?.split("(")[0]?.trim()}</span>
                          <span style={{ color: "var(--c-475569)" }}>→</span>
                          <span style={{ color: 'var(--c-38bdf8)' }}>{o.target_service}</span>
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
            <input
              ...
              style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 8, color: "var(--c-f1f5f9)", padding: "5px 12px", fontSize: 12, minWidth: 180 }}
            />
            {Object.entries(FILTER_OPTIONS).map(([key, opts]) => (
              <select key={key} value={filters[key]} onChange={e => setFilters(f => ({ ...f, [key]: e.target.value }))}
                style={{ background: "var(--c-0f172a)", border: "1px solid var(--c-1e293b)", borderRadius: 8, color: "var(--c-94a3b8)", padding: "5px 10px", fontSize: 11 }}>
                {opts.map(o => <option key={o.v} value={o.v}>{o.l}</option>)}
              </select>
            ))}
            <span style={{ color: "var(--c-475569)", fontSize: 11, alignSelf: "center", marginLeft: 8 }}>{filtered.length} results</span>
          </div>
                ...
                <tr style={{ borderBottom: "1px solid var(--c-1e293b)" }}>
                  {[
                    ...
                    <th key={i} onClick={() => col.key && toggleSort(col.key)} style={{
                      ...
                      color: sort.key === col.key ? "#38bdf8" : "var(--c-475569)", fontSize: 10, fontWeight: 700,
      ...
      {viewMode === "5r" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {(data.five_r_summary?.length > 0 ? data.five_r_summary : [{ category: "Retain", count: opps.length, total_cost: 0, potential_savings: 0, description: "" }]).map((rSum, i) => {
            ...
            return (
              <div key={i} style={{ background: "var(--c-0f172a)", border: `1px solid ${meta.color}25`, borderRadius: 12, padding: "16px 20px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: "var(--c-64748b)", fontSize: 11 }}>{rSum.description || meta.desc}</div>
                  </div>
                  <div style={{ display: "flex", gap: 14 }}>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "var(--c-f1f5f9)", fontWeight: 700, fontSize: 18 }}>{rSum.count}</div>
                      <div style={{ color: "var(--c-475569)", fontSize: 10 }}>resources</div>
                    </div>
                    {rSum.potential_savings > 0 && (
                      <div style={{ textAlign: "right" }}>
                        ...
                        <div style={{ color: "var(--c-475569)", fontSize: 10 }}>/mo savings</div>
                      </div>
                    )}
                  </div>
                </div>
                {items.length > 0 && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {items.slice(0, 8).map((o, j) => {
                      const savMo = o.monthly_cost * o.estimated_savings_pct / 100;
                      return (
                        <div key={j} onClick={() => setSelected(o)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderRadius: 8, cursor: "pointer", background: "var(--c-0d1117)", border: "1px solid var(--c-1e293b)" }}>
                          <ResourceIconImg resourceType={o.resource_type} size={14} />
                          <span style={{ color: "var(--c-e2e8f0)", fontSize: 12, fontWeight: 600, flex: 1 }}>{o.resource_name}</span>
                          <span style={{ color: "var(--c-64748b)", fontSize: 10 }}>{o.resource_group}</span>
                          <Badge label={o.complexity} color={COMPLEXITY_COLOR[o.complexity] || "var(--c-64748b)"} />
                    ...
                    {items.length > 8 && (
                      <div style={{ color: "var(--c-475569)", fontSize: 11, paddingLeft: 10, cursor: "pointer" }}
