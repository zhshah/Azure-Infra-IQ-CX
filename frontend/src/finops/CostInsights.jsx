...
import {
  ...
  Recycle, Rocket, TrendingUp, Server, ChevronDown, Activity, Landmark,
} from 'lucide-react'
...
const USAGE_COLORS = { 'Fully Used': '#22c55e', 'Actively Used': '#4ade80', 'Rarely Used': '#f59e0b', 'Not Used': '#ef4444', Unknown: '#64748b' }
const POWER_COLORS = { running: '#22c55e', deallocated: '#ef4444', stopped: '#f59e0b', 'Not applicable': '#334155' }
...
/* ── Collapsible section (expandability) ── */
function Section({ title, icon: Icon, count, children, defaultOpen = true }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: open ? 12 : 0 }}>
      <div onClick={() => setOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', userSelect: 'none', color: 'var(--c-f1f5f9)', fontSize: 14, fontWeight: 700, padding: '6px 2px', borderBottom: '1px solid var(--c-1e293b)' }}>
        {Icon && <Icon size={16} style={{ color: '#0078d4' }} />}
        <span>{title}</span>
        {count != null && <span style={{ fontSize: 11, color: 'var(--c-64748b)', fontWeight: 500 }}>({count})</span>}
        <ChevronDown size={16} style={{ marginLeft: 'auto', transform: open ? 'none' : 'rotate(-90deg)', transition: 'transform .15s', color: 'var(--c-64748b)' }} />
      </div>
      {open && <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>{children}</div>}
    </div>
  )
}
...
export default function CostInsights() {
  ...
  const agg = useMemo(() => {
    ...
    const nonZoneRedundant = filtered.filter(r => r.zone_status && !['zoneredundant', 'redundantbydefault'].includes(String(r.zone_status).toLowerCase().replace(/\s/g, ''))).reduce((s, r) => s + (r.cost_current || 0), 0)
    const untagged = filtered.filter(r => !r.has_tags).reduce((s, r) => s + (r.cost_current || 0), 0)
    const idle = filtered.filter(r => r.is_orphan || (r.days_idle || 0) >= 30).reduce((s, r) => s + (r.cost_current || 0), 0)
    return {
      total, prev, waste, savings, unprotected, nonZoneRedundant, untagged, idle,
      ...
      bySubscription: rollup(filtered, r => r.subscription_name, { top: 10 }),
      byUsage: rollup(filtered, r => r.score_label, { top: 6 }),
      byRi: rollup(filtered, r => (r.ri_covered ? 'Reserved (RI/SP)' : 'On-demand'), { top: 3 }),
      byTagStatus: rollup(filtered, r => (r.has_tags ? 'Tagged' : 'Untagged'), { top: 3 }),
      byPower: rollup(filtered, r => (r.power_state || 'Not applicable'), { top: 5 }),
      byModernComplexity: rollup(filtered.filter(r => r.modernization_type && r.modernization_complexity), r => r.modernization_complexity, { top: 5, valueFn: r => r.modernization_savings }),
      // Month-over-month movers by service (Δ = current − previous).
      movers: (() => {
        const m = {}
        for (const r of filtered) {
          const k = r.service
          if (!m[k]) m[k] = { name: k, value: 0 }
          m[k].value += (r.cost_current || 0) - (r.cost_previous || 0)
        }
        return Object.values(m).map(x => ({ ...x, value: Math.round(x.value * 100) / 100 }))
          .filter(x => Math.abs(x.value) >= 0.01)
          .sort((a, b) => Math.abs(b.value) - Math.abs(a.value)).slice(0, 12)
      })(),
      scatter: filtered
        .filter(r => r.cost_current > 0 && r.utilization_pct != null)
        .map(r => ({ util: Math.round(r.utilization_pct), cost: r.cost_current, name: r.resource_name })),
  ...
  // Cost-at-risk cards → open the resources behind each risk bucket.
  const drillRows = (label, predicate) => () => {
    const matches = filtered.filter(predicate)
    if (matches.length) openResourceDrill(`${label} (${matches.length})`, matches)
  }

  // Authoritative headline (warehouse this-month) vs the per-resource attributed sum.
  const authTotal = payload?.summary?.authoritative_total_usd
  const coveragePct = payload?.summary?.coverage_pct
  const showAuth = !anyFilter && authTotal && authTotal > 0
  const headlineValue = showAuth ? authTotal : agg.total
  const headlineLabel = showAuth ? 'Estate spend · last 30 days' : (anyFilter ? 'Cost · filtered slice' : 'Estate spend')
  const headlineSub = showAuth
    ? (coveragePct != null ? `warehouse actuals · breakdown attributes ${coveragePct}%` : 'warehouse actuals')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ ...card, display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--c-64748b)' }}>{agg.count} of {rows.length} resources · {payload?.data_source}{payload?.generated_at ? ` · as of ${new Date(payload.generated_at).toLocaleString()}` : ''}</span>
      </div>

      {/* KPI strip */}
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {[
          { label: headlineLabel, value: fmtUsd(headlineValue), sub: headlineSub, color: '#0078d4', icon: TrendingUp },
          ...
          { label: 'Idle / orphaned waste', value: fmtUsd(agg.waste), sub: 'cumulative', color: '#f59e0b', icon: Recycle },
      ...
      {/* Cost at risk — quantified $ exposure, click any to see the resources */}
      <div style={{ ...card }}>
        <div style={secLabel}><ShieldCheck size={15} style={{ color: '#ef4444' }} /> Cost at risk <span style={{ fontSize: 10, color: 'var(--c-64748b)', fontWeight: 400 }}>· spend on resources with a posture gap — click to see them</span></div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {[
            { label: 'Unprotected (no backup)', value: agg.unprotected, color: '#ef4444', onClick: drillRows('Unprotected spend', r => !r.has_backup) },
            { label: 'Not zone-redundant', value: agg.nonZoneRedundant, color: '#f59e0b', onClick: drillRows('Non zone-redundant spend', r => r.zone_status && !['zoneredundant', 'redundantbydefault'].includes(String(r.zone_status).toLowerCase().replace(/\s/g, ''))) },
            { label: 'Untagged (unallocated)', value: agg.untagged, color: '#a855f7', onClick: drillRows('Untagged spend', r => !r.has_tags) },
            { label: 'Idle / orphaned', value: agg.idle, color: '#64748b', onClick: drillRows('Idle / orphaned spend', r => r.is_orphan || (r.days_idle || 0) >= 30) },
          ].map((k, i) => (
            <div key={i} onClick={k.onClick} title="View resources" style={{ background: 'var(--c-0f172a)', border: `1px solid ${k.color}44`, borderLeft: `3px solid ${k.color}`, borderRadius: 8, padding: '10px 14px', flex: 1, minWidth: 170, cursor: 'pointer' }}>
              <div style={{ fontSize: 10, color: 'var(--c-64748b)', textTransform: 'uppercase', fontWeight: 600, marginBottom: 3 }}>{k.label}</div>
              <div style={{ color: k.color, fontSize: 20, fontWeight: 700 }}>{fmtUsd(k.value)}</div>
            </div>
          ))}
        </div>
      </div>

      {/* Chart sections (collapsible) */}
      <div style={{ fontSize: 10, color: 'var(--c-64748b)', marginTop: -4 }}>💡 Click any chart segment, bar or point to see the resources behind it. Collapse a section with its title.</div>

      <Section title="Where your money goes" icon={Layers}>
        <Panel title="Cost by category"><CostDonut data={agg.byCategory} onSlice={drillBy('Category', agg.byCategory, r => r.category)} /></Panel>
        <Panel title="Cost by service"><CostBars data={agg.byService} onSlice={drillBy('Service', agg.byService, r => r.service)} /></Panel>
        <Panel title="Cost by subscription"><CostDonut data={agg.bySubscription} onSlice={drillBy('Subscription', agg.bySubscription, r => r.subscription_name)} /></Panel>
        <Panel title="Cost by region"><CostBars data={agg.byRegion} onSlice={drillBy('Region', agg.byRegion, r => r.region)} /></Panel>
        <Panel title="Cost by resource group"><CostBars data={agg.byRG} onSlice={drillBy('Resource group', agg.byRG, r => r.resource_group)} /></Panel>
        <Panel title="Cost by SKU tier"><CostBars data={agg.bySkuTier} onSlice={drillBy('SKU tier', agg.bySkuTier, r => r.sku_tier)} /></Panel>
      </Section>

      <Section title="Resilience posture of your spend" icon={ShieldCheck}>
        <Panel title="Cost by zone-resilience posture" subtitle="From the genuine zone assessment — where your money sits on resilience">
        ...
        <Panel title="Protected vs unprotected spend">
      ...
      <Section title="Governance, usage & commitments" icon={Landmark}>
        <Panel title="Tagged vs untagged spend" subtitle="Untagged spend can't be allocated / charged back">
          <CostDonut data={agg.byTagStatus} colorFn={(n) => (n === 'Tagged' ? '#22c55e' : '#a855f7')} onSlice={drillBy('Tag status', agg.byTagStatus, r => (r.has_tags ? 'Tagged' : 'Untagged'))} />
        </Panel>
        <Panel title="Cost by usage level" subtitle="How much spend sits on rarely / never used resources">
          <CostDonut data={agg.byUsage} colorFn={(n) => USAGE_COLORS[n] || '#64748b'} onSlice={drillBy('Usage', agg.byUsage, r => r.score_label)} />
        </Panel>
        <Panel title="Reserved vs on-demand spend" subtitle="Commitment coverage (RI / Savings Plan)">
          <CostDonut data={agg.byRi} colorFn={(n) => (n.startsWith('Reserved') ? '#22c55e' : '#f59e0b')} onSlice={drillBy('Commitment', agg.byRi, r => (r.ri_covered ? 'Reserved (RI/SP)' : 'On-demand'))} />
        </Panel>
        <Panel title="Cost by power state" subtitle="Deallocated VMs still bill for disks & IPs">
          <CostDonut data={agg.byPower} colorFn={(n) => POWER_COLORS[n] || '#64748b'} onSlice={drillBy('Power state', agg.byPower, r => (r.power_state || 'Not applicable'))} />
        </Panel>
      </Section>
      ...
      <Section title="Month-over-month movers" icon={Activity} defaultOpen={false}>
        <Panel title="Biggest cost changes by service" subtitle="Δ vs previous month · red = increase, green = decrease">
          {agg.movers.length ? <CostBars data={agg.movers} colorFn={(n) => { const d = agg.movers.find(x => x.name === n); return d && d.value >= 0 ? '#ef4444' : '#22c55e' }} onSlice={drillBy('Cost mover', agg.movers, r => r.service)} /> : <div style={{ padding: 30, textAlign: 'center', color: 'var(--c-64748b)', fontSize: 12 }}>No month-over-month change data.</div>}
        </Panel>
      </Section>

      <Section title="Modernization upside" icon={Rocket} defaultOpen={false}>
        <Panel title="Savings by target service" subtitle="e.g. SQL Server on VM → SQL Managed Instance — grounded monthly saving">
        ...
        <Panel title="Savings by migration complexity" subtitle="Quick wins first — sequence the roadmap by effort">
          {agg.byModernComplexity.length ? <CostBars data={agg.byModernComplexity} colorFn={(n) => ({ Low: '#22c55e', Medium: '#f59e0b', High: '#ef4444' }[n] || '#64748b')} onSlice={drillBy('Complexity', agg.byModernComplexity, r => r.modernization_complexity)} /> : <div style={{ padding: 30, textAlign: 'center', color: 'var(--c-64748b)', fontSize: 12 }}>No modernization opportunities in this slice.</div>}
        </Panel>
      </Section>
