...
export default function CommitmentTracker() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, gap: 10 }}>
      <RefreshCw size={18} className="animate-spin" style={{ color: '#3b82f6' }} /><span style={{ color: '#94a3b8' }}>Loading commitment data…</span>
    </div>
  )
  if (error) return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ color: '#f1f5f9', fontSize: 18, fontWeight: 700, margin: 0 }}>Commitments & RI</h2>
          <p style={{ color: '#64748b', fontSize: 12, margin: 0 }}>Live from Azure ReservationManagementClient — same as Azure Portal Reservations blade</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 12, alignItems: 'start' }}>
        <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        ...
        {[
          ...
          <div key={c.label} style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 14 }}>
            <div style={{ color: '#64748b', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>{c.label}</div>
      ...
      {expiringSoon.length > 0 && (
        <div style={{ background: '#1c1003', border: '1px solid #854d0e', borderRadius: 8, padding: 12, display: 'flex', gap: 8, color: '#fbbf24', fontSize: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: 4, width: 'fit-content' }}>
        {[{ key: 'reservations', label: `Reservations (${reservations.length})` }, { key: 'recommendations', label: `Buy Recs (${recommendations.length})` }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            background: tab === t.key ? '#1e293b' : 'none', border: `1px solid ${tab === t.key ? '#334155' : 'transparent'}`,
            borderRadius: 6, padding: '5px 12px', cursor: 'pointer', color: tab === t.key ? '#e2e8f0' : '#475569', fontSize: 12,
          }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === 'reservations' && (
        <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
          {reservations.length === 0 ? (
            <div style={{ color: '#334155', fontSize: 12, textAlign: 'center', padding: 30 }}>No reservations found. This may require Reservation Reader role.</div>
          ) : (
            <>
              ...
              <tbody>
                {reservationsWithExpiry.slice(resPage * RES_PAGE, (resPage + 1) * RES_PAGE).map((r, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                    <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{r.display_name || (r.reservation_id?.slice(0, 16) + '…')}</td>
                    <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{r.resource_type}</td>
                    <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{r.quantity}</td>
                    ...
                    <td style={{ padding: '6px 8px', color: r._expDays != null && r._expDays < 90 ? '#fbbf24' : '#475569', fontSize: 11 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, fontSize: 11, color: '#64748b' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => setResPage(p => Math.max(0, p - 1))} disabled={resPage === 0}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: resPage === 0 ? 'default' : 'pointer', color: resPage === 0 ? '#334155' : '#94a3b8', fontSize: 11 }}>← Prev</button>
                  <button onClick={() => setResPage(p => Math.min(Math.ceil(reservationsWithExpiry.length / RES_PAGE) - 1, p + 1))} disabled={(resPage + 1) * RES_PAGE >= reservationsWithExpiry.length}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: (resPage + 1) * RES_PAGE >= reservationsWithExpiry.length ? 'default' : 'pointer', color: (resPage + 1) * RES_PAGE >= reservationsWithExpiry.length ? '#334155' : '#94a3b8', fontSize: 11 }}>Next →</button>
                </div>
              </div>
            )}
            </>
      ...
      {tab === 'recommendations' && (
        <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
          {recommendations.length === 0 ? (
            <div style={{ color: '#334155', fontSize: 12, textAlign: 'center', padding: 30 }}>No RI recommendations from Azure. This may require Billing Reader or Consumption access.</div>
          ) : (
            <>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
              ...
              <tbody>
                {recommendations.slice(recPage * REC_PAGE, (recPage + 1) * REC_PAGE).map((r, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                    <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{r.resource_type}</td>
                    <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{r.region}</td>
                    <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{r.term_label}</td>
                    <td style={{ padding: '6px 8px', color: '#94a3b8' }}>{r.recommended_quantity}</td>
                    ...
                    <td style={{ padding: '6px 8px', color: '#475569' }}>{r.break_even_months ? `${r.break_even_months}mo` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, fontSize: 11, color: '#64748b' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => setRecPage(p => Math.max(0, p - 1))} disabled={recPage === 0}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: recPage === 0 ? 'default' : 'pointer', color: recPage === 0 ? '#334155' : '#94a3b8', fontSize: 11 }}>← Prev</button>
                  <button onClick={() => setRecPage(p => Math.min(Math.ceil(recommendations.length / REC_PAGE) - 1, p + 1))} disabled={(recPage + 1) * REC_PAGE >= recommendations.length}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: (recPage + 1) * REC_PAGE >= recommendations.length ? 'default' : 'pointer', color: (recPage + 1) * REC_PAGE >= recommendations.length ? '#334155' : '#94a3b8', fontSize: 11 }}>Next →</button>
                </div>
              </div>
            )}
            </>
