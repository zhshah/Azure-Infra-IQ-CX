  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ color: '#f1f5f9', fontSize: 18, fontWeight: 700, margin: 0 }}>Cost Allocation</h2>
          <p style={{ color: '#64748b', fontSize: 12, margin: 0 }}>Live Azure data — group by any dimension to allocate costs</p>
        </div>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {['bar', 'donut'].map(v => (
              <button key={v} onClick={() => setView(v)} style={{
                background: view === v ? '#1e293b' : 'none', border: `1px solid ${view === v ? '#334155' : 'transparent'}`,
                borderRadius: 5, padding: '5px 10px', cursor: 'pointer', color: view === v ? '#e2e8f0' : '#475569', fontSize: 11,
              }}>
                {v === 'bar' ? 'Bar' : 'Donut'}
              </button>
            ))}
      ...
      {loading && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 20 }}>
          ...
          <span style={{ color: '#94a3b8', fontSize: 12 }}>Loading allocation data…</span>
        </div>
      )}
      {error && (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
            {[
              ...
              { label: 'Groups', value: items.length, color: '#94a3b8' },
            ].map(c => (
          ...
          {/* Drill-down table */}
          <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
            <div style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600, marginBottom: 10 }}>Breakdown</div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {items.slice(tablePage * PAGE_SIZE, (tablePage + 1) * PAGE_SIZE).map((it, i) => {
                  ...
                  return (
                    <tr key={globalIdx} style={{ borderBottom: '1px solid #0f172a' }}>
                      ...
                      <td style={{ padding: '6px 8px', color: (it.mom_delta_pct ?? 0) > 5 ? '#f87171' : (it.mom_delta_pct ?? 0) < -5 ? '#4ade80' : '#64748b' }}>
            ...
            {items.length > PAGE_SIZE && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, fontSize: 11, color: '#64748b' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => setTablePage(p => Math.max(0, p - 1))} disabled={tablePage === 0}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: tablePage === 0 ? 'default' : 'pointer', color: tablePage === 0 ? '#334155' : '#94a3b8', fontSize: 11 }}>
                    ← Prev
                  </button>
                  <button onClick={() => setTablePage(p => Math.min(Math.ceil(items.length / PAGE_SIZE) - 1, p + 1))} disabled={(tablePage + 1) * PAGE_SIZE >= items.length}
                    style={{ background: '#1e293b', border: 'none', borderRadius: 5, padding: '4px 10px', cursor: (tablePage + 1) * PAGE_SIZE >= items.length ? 'default' : 'pointer', color: (tablePage + 1) * PAGE_SIZE >= items.length ? '#334155' : '#94a3b8', fontSize: 11 }}>
                    Next →
                  </button>
                </div>
              </div>
