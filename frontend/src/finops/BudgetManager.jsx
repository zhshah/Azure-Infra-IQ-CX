...
function StatusBadge({ status }) {
  const colors = {
    ...
    at_risk:  { bg: '#1c1003', border: '#854d0e', text: '#fbbf24' },
    exceeded: { bg: '#1a0e0e', border: '#7f1d1d', text: '#f87171' },
  }
...
function CreateBudgetModal({ onClose, onCreated }) {
  ...
  const Field = ({ label, k, type = 'text', placeholder }) => (
    <div>
      <label style={{ color: '#64748b', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 4 }}>{label}</label>
      <input
        type={type} value={form[k]} placeholder={placeholder}
        onChange={e => set(k, e.target.value)}
        style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 6, color: '#e2e8f0', padding: '7px 10px', fontSize: 12, width: '100%', outline: 'none' }}
      />
    </div>
  )
  ...
  return (
    ...
    >
      <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 12, padding: 24, width: 440, maxWidth: '90vw' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ color: '#f1f5f9', fontSize: 16, fontWeight: 700, margin: 0 }}>Create Budget</h3>
    ...
    <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
        <div style={{ background: '#0f172a', borderRadius: 6, padding: '8px 10px' }}>
          <div style={{ color: '#475569', fontSize: 9, fontWeight: 600, textTransform: 'uppercase', marginBottom: 2 }}>Budget</div>
          <div style={{ color: '#3b82f6', fontSize: 14, fontWeight: 700 }}>{fmtUsd(b.amount_usd)}</div>
        </div>
        <div style={{ background: '#0f172a', borderRadius: 6, padding: '8px 10px' }}>
          <div style={{ color: '#475569', fontSize: 9, fontWeight: 600, textTransform: 'uppercase', marginBottom: 2 }}>Remaining</div>
      ...
      {!expanded ? (
        <button onClick={handleExpand} style={{ width: '100%', background: 'none', border: '1px dashed #1e293b', borderRadius: 6, padding: '5px', cursor: 'pointer', color: '#475569', fontSize: 11 }}>
        ...
        <>
          ...
          {b.cost_center && (
            <div style={{ marginTop: 8, fontSize: 10, color: '#475569' }}>
              Cost Center: <span style={{ color: '#64748b' }}>{b.cost_center}</span>
            </div>
          )}
        </>
      )}
...
export default function BudgetManager() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, gap: 10, color: '#3b82f6' }}>
      <RefreshCw size={18} className="animate-spin" /><span style={{ color: '#94a3b8' }}>Loading budgets…</span>
    </div>
  )
  if (error) return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ color: '#f1f5f9', fontSize: 18, fontWeight: 700, margin: 0 }}>Budget Manager</h2>
          <p style={{ color: '#64748b', fontSize: 12, margin: 0 }}>Azure native + custom budgets with live variance from Azure Cost Management</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => loadBudgets(true)} disabled={syncing} style={{
            ...
            color: '#94a3b8', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5,
          }}>
